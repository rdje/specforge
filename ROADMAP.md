...
next product gap for APB's sole SourceIR-to-EvidenceIR canonical loss. Genericity audit and behavioral `.f.v` signoff are complete within the governed boundary. `.7` is active: `.7a` freezes compound-inference recovery; `.7b` ships its source-grounded producer with exact 24-chain currency and zero public delta; `.7c` owns complete replay/signoff before the revision-bound 39/0/1 result changes.
