...
- current lane:
  ...
  - the active `BWFSC=100` batch has reached slice 7/100 with standalone explicit-module `?fsm:name` state graph provenance coverage across module candidates, selected renderable modules, and emitted source-document direct roots; `R6` remains active and push remains deferred until the batch completes.
