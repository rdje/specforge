...
- done:
  ...
  - the tracked fixture set now locks both graph-backed compatibility-direction lag and genuinely unresolved compatibility-direction gaps, so missing flat hints with no actor-relative graph coverage cannot be mislabeled as graph lag at either `SemanticIR` or `IntentIR`
