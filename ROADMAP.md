...
- done:
  ...
  - explicit top `.fsm` composition can now preserve width-only top boundary port records, recover missing top input/output direction from explicit top-link source/target topology, keep those recovered boundary directions visible even when an unrelated composition gate still blocks emission, and recover existing child-module input/output port directions from child endpoints in explicit top links before module renderability analysis
- remaining:
