...
oracle and `.9b` re-ingested the APB gold, so 25 chains are re-derivable and 53 stay legacy (the AHB and AXI
golds among them); re-derivation retired the SWD `29/29` (`.8c`) and now APB's temporal `1.000` (`.9b`).
