...
| `TEMPORAL-RULE-LTL-RENDER` | `active` (`.1` design done; `.2` pending) | `R6`/`R15e` | **Tier-1 headline adopt item from the `LITERATURE-GROUNDING` backlog** (`protocol-temporal-semantics.md`). Render mined `temporal_rules` in **standard LTL/MTL notation** — `G(antecedent → consequent)` with `X`/`F[min,max]` for the tick / cycle-window — grounding SpecForge's temporal model in the spec-mining formalism (Pnueli/GoldMine/Texada). Pure, derived, **opt-in** (`validate --ltl`); a new `ir/temporal_ltl.rs` renderer + atom vocabulary for all 7 predicate variants. **No IR persistence** (zero fixture churn), **no `.isf`/PSL-SVA export** (that touches the FSMGen contract → separate downstream tree). Worked: `G( PSEL==ASSERTED & PENABLE==ASSERTED & PREADY==ASSERTED -> X (drive(Completer,PBUSER) & PBUSER==VALID) )` | [docs/tasks/TEMPORAL-RULE-LTL-RENDER.md](docs/tasks/TEMPORAL-RULE-LTL-RENDER.md) |
