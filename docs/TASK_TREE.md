...
| `LITERATURE-GROUNDING` | `active` (`.1`–`.2` done; `.3`–`.13` pending) | `R0`/`R15e` | User-directed program: ground every SpecForge aspect in **verified** published research, leveraging prior art while separating genuine out-of-the-box novelty. 11 aspects; per aspect: prior-art / alignment / adopt / extend-novelty / gaps; `.13` = reach-full-potential backlog. **Anti-hallucination: every citation verifiable**. `.2` DONE — aspect 1 (document extraction) grounded in `docs/research/grounding/document-extraction.md` (web-verified: Docling 2408.09869/2501.17887, TableFormer 2203.01017, DocLayNet 2206.01062, PubTables-1M 2110.00061; TEDS/GriTS). Remaining aspects 2–11 + synthesis pending | [docs/tasks/LITERATURE-GROUNDING.md](docs/tasks/LITERATURE-GROUNDING.md) |
