...
| `REGISTER-MAP-CLASSIFIER-PRECISION` | `active` | `R8` | `REGISTER-MAP-CLASSIFIER-PRECISION.2` (`.1` done — owned + diagnosed) — fix a SYSTEMIC table-classifier precision bug found by `CORPUS-HARDENING`: the bare `has_addr_col` heuristic in `classify_table_kind` (`docling_backend.rs`) mis-classifies feature matrices / TOCs / data-frame layouts / address-assignment tables as `register_map` across I2C/eMMC/APB. Tighten to require address+field-structure; corpus-regression-verified (confirm real register headers in `.2` before changing the predicate in `.3`) | [docs/tasks/REGISTER-MAP-CLASSIFIER-PRECISION.md](docs/tasks/REGISTER-MAP-CLASSIFIER-PRECISION.md) |
