...
| `R16-WAVEFORM-CONTRACT-MINING` | `active` | `R16` | `R16-WAVEFORM-CONTRACT-MINING.3.1` (`.1`+`.2` done; `.3` honest-split `2026-05-20` based on corpus survey: no raw PDFs in tree ⇒ adapter consumes upstream typed FigureRegion; `.3.1` = contract design, `.3.2` = adapter) | [docs/tasks/R16-WAVEFORM-CONTRACT-MINING.md](docs/tasks/R16-WAVEFORM-CONTRACT-MINING.md) |
