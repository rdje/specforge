...
| --- | --- | --- | --- | --- |
| `EVIDENCE-DETERMINISM` | `active` | `R0`/`R15e` | `.1` diagnosis DONE `2026-06-09` (surfaced by `EXTRACTOR-ARCHITECTURE.5`): the EvidenceIR build is **content-level non-deterministic** — two runs of identical code give set-different `actor_signal_relations`/`extracted_statements` on relation-heavy prose docs (SWD/ADI). Root cause: `extract_actor_signal_relations` iterates `known_signals: &HashSet` + a first-seen-wins dedup → different representatives kept per run. Undermines reproducibility + eval-score stability + the byte-identical migration proofs. `.2` fix = deterministic iteration (sorted/`BTreeSet`), double-run-verified, multiset-preserving. | [docs/tasks/EVIDENCE-DETERMINISM.md](docs/tasks/EVIDENCE-DETERMINISM.md) |
| `CORPUS-PATTERN-REUSE` | `active` | `R15`/`R16` | `.1` design DONE `2026-06-09` (owner directive: reuse extraction patterns across PDFs, clustered by **derived** vendor/layout fingerprint — agnostic ADR 0006, advisory, honest). Extends the proven `CorpusMemory` prior plane; the `EXTRACTOR-ARCHITECTURE` run manifest IS the per-doc behavioral fingerprint to cluster on; LLM/VLM as an offline corpus-level pattern miner (validated before promotion, like `VERB-COVERAGE-CORPUS`). Build `.2`–`.4` sequenced behind the extractor-framework foundation; recommended foundation-first. | [docs/tasks/CORPUS-PATTERN-REUSE.md](docs/tasks/CORPUS-PATTERN-REUSE.md) |
...
| `EXTRACTION-QUALITY-GAUGE` | `active` (`.1`/`.2`/`.5` done; thesis validated) | `R15e`/`R16` | NLI-oracle not-entailed rate = per-doc extraction-quality GAUGE. CHI **~80% erroneous** (hand-validated 18/18) vs APB ~29%. Root: entity discrimination + flat unconditional constraints. **Harness pattern (LLM judges, Rust grounds) built across 3 fronts**; `.5` LLM-primary EXTRACTOR (replace, not patch) jumps CHI **precision 17%→64% (~4×)**. Recall settled: CHI's apparent "loss" was largely CORRECT field-exclusion (`DBID`/`TxnID` are flit FIELDS not signals — CHI is packet-based, B2.4) PLUS a real localized `must_be_value` gap (APB clean test: recall 4/6=67%, misses `PBUSER`/`PNSE`). Two separable problems: `.8` close the value-constraint recall; `.FIELD` the signal-vs-field ontology for packet protocols (CHI/SWD-ADI-class). Corpus spans wire-bus + packet + serial-debug classes | [docs/tasks/EXTRACTION-QUALITY-GAUGE.md](docs/tasks/EXTRACTION-QUALITY-GAUGE.md) |
