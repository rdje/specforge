...
| `DEMPSTER-FUSION-COMBINER` | `active` (`.1` design done; `.2` pending) | `R16`/`R15e` | **User pick (2nd, after prior-decay).** Grounded gap (`multimodal-fusion.md`, Dempster 1967): `ir/fusion.rs::merge_cluster` combines agreeing contracts' confidence with `min_confidence` (**caps at the weakest** — ignores corroboration). `.2` replaces it on the agreement path with a **Dempster corroboration** (`mass: High .9/Med .7/Low .5`, combined `m = 1−∏(1−mᵢ)`, mapped back) so independent agreement *raises* confidence (Medium+Medium→High). Conflict mass K = 0 here (disagreement already pre-routed to Residual), so the Zadeh guard is a documented future extension. Watch fixture churn (boosts on multi-source agreeing clusters) | [docs/tasks/DEMPSTER-FUSION-COMBINER.md](docs/tasks/DEMPSTER-FUSION-COMBINER.md) |
...
| `TEMPORAL-RULE-SVA-RENDER` | `deferred` (LOGGED `2026-06-04`; `.1` design done, `.2` deferred) | `R6`/`R15e` | **Logged per user direction; execution pending a decision.** The SpecForge-side `.isf`/IntentIR → **SVA** export (the LTL renderer's tool-consumable successor; renders `temporal_rules` as IEEE 1800 SystemVerilog Assertions via `$stable`/`$isunknown`, `|=>`/`|-> ##[min:max]`, active-high convention, dropping non-SVA predicates). `.1` design captured = the "log"; `.2` build **deferred** pending the user's choice between this and FSMGen-native LTL/MTL-in-ISF (`FSMGEN-LTL-MTL-SUGGESTION`) — if ISF gains LTL/MTL, this may be unnecessary | [docs/tasks/TEMPORAL-RULE-SVA-RENDER.md](docs/tasks/TEMPORAL-RULE-SVA-RENDER.md) |
