...
| `PDF-VARIANT-DIGESTION` | `active` (HIGH-PRIORITY program) | `R15`/`R16` | **Owner aim: SpecForge digests ANY chip-spec PDF we throw at it** (82 in the library across ARM/CXL/USB/RISC-V/Intel/AMD/JEDEC/NXP/NVMe/OpenCores; protocol/architecture/TRM/interface/manual). Defer ISF lowering. Lever A (table-kind classify: deterministic register-field `.2`/`.2c`/`.2d` + VLM classify/grid-repair `.2b`/`.2b'`) + Lever B (prose entity capture: signals `.3a`, actors `.3b`) LANDED; whole-corpus sweep proved breadth (1,908 signals / 2,953 registers / 10,632 fields / 3,077 relations) but only 4/74 had verified precision. **`.4` (precision verification, item ①) CLOSED `2026-06-08`** — `.4a` per-fact gold (RISC-V/NVMe register fields, I2C prose signals) + `.4b` VLM proposer/verifier audit (`audit-extraction`; cross-validated: RISC-V 0.250/0.375 ↔ NVMe 0.750, tracks gold quality). **`.5a` (structural doc-class routing — item ②) DONE `2026-06-08`**: `validate` reports a `protocol`/`register`/`interface`/`guide` class from the low-noise intent surfaces (conditional-rules excluded as over-produced noise — real-data correction), honest guide reporting; live over 74 docs = protocol 25 / register 11 / interface 22 / guide 16. Frontier `.5c` (owner-suggested front-matter/ToC doc-type signal — corroborate class + split a true guide from an under-extracted spec), then `.5b` (completeness gauge). Owner pivoted `2026-06-08` to FIXING the 4 quantified gaps → spun off `EXTRACTION-GAP-FIX`. keep APB/AHB/AXI/SWD at 100%. `project_pdf_variant_digestion` memory | [docs/tasks/PDF-VARIANT-DIGESTION.md](docs/tasks/PDF-VARIANT-DIGESTION.md) |
