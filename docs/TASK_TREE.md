...
| `RECALL-CHAO-ESTIMATOR` | `active` (`.1` design done; `.2` pending) | `R15e` | **Tier-2 adopt item from the `LITERATURE-GROUNDING` backlog** (`extraction-evaluation.md`). Add the **Chao (1987) heterogeneity-robust** richness estimator as a second recall N̂ alongside Lincoln–Petersen in `ir/completeness.rs::recall_estimate` — the two extractor tiers share prose (positive dependence → LP under-estimates) and Chao tolerates unequal catchability. 2-source incidence: `f1=distinct−overlap`, `f2=overlap`, `N̂_chao=distinct+f1²/(2·f2)` (worked: Chao 10 vs LP 8 on the existing test → report total **8–10**, misses **2–4** as a range). Additive (fires only when the gauge fires); `None`-gating unchanged; verified citation (DOI 10.2307/2531532); zero behavior change | [docs/tasks/RECALL-CHAO-ESTIMATOR.md](docs/tasks/RECALL-CHAO-ESTIMATOR.md) |
