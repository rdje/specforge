...
| `SIGNAL-TABLE-COLUMNLESS-RECALL` | `active` (`.1` done; `.2` **blocked** on design decision) | `R12`/`R15e` | Diagnosing the "11 unexplained CHI tables" `CORPUS-HARDENING` follow-up surfaced a CONFIRMED live recall gap: column-less `Signal\|Description` interface tables (CHI REQ/RSP/SNP/DAT channels — `REQFLITPEND`/`REQFLITV`/`REQLCRDV`) yield **0 declarations**, because both `synthesize_signal_declarations` (evidence) and `parse_explicit_signal_declaration` (semantic) require direction OR width. Reproduced by a `#[ignore]`d failing test. `.2` blocked: the fix reverses a deliberate abstract-transport precision philosophy (signals driven by transmitter/receiver) and can't be corpus-validated now (no re-ingest) — needs a user A/B/C decision | [docs/tasks/SIGNAL-TABLE-COLUMNLESS-RECALL.md](docs/tasks/SIGNAL-TABLE-COLUMNLESS-RECALL.md) |
