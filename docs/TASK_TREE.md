...
| `SWD-SERIAL-EXTRACTION` | `active` (research→impl) | `R15e`/`R16` | Serial/architecture extraction for SWD/ADI (IHI0074) → WIRE-BASED-100 100% (the path the parallel-bus model can't reach; owner decision (a) from `WIRE-BASED-100.5j`). `.1` research DONE (SWCLK/SWDIO ARE prose-describable; serial frame start/stop/park/turnaround/parity/ACK described; DP/AP in register_map tables). Frontier `.2`: capture SWCLK/SWDIO from the prose "`<role>` pin, `<SIGNAL>`" pattern. KM `swd-adi-not-signal-table-spec` | [docs/tasks/SWD-SERIAL-EXTRACTION.md](docs/tasks/SWD-SERIAL-EXTRACTION.md) |
