...
| `TEMPORAL-ANTECEDENT-RECALL` | `active` (`.1` design done; `.2` pending) | `R8`/`R15e` | **The second catch of the measure→catch→fix loop** (after `CONSTRAINT-SUBJECT-PRECISION`), surfaced by `TEMPORAL-RULE-EVAL`. The temporal condition parser (`parse_temporal_condition_predicates`, `ir/semantic.rs`) drops leading signals in a coordinated list: *"PBUSER valid when PSEL, PENABLE, **and** PREADY are asserted"* yields an antecedent of only PREADY (the shared trailing "are asserted" isn't distributed) → the rule fires on a weaker precondition than the spec. Fix = distribute a single shared trailing value across the coordinated signal list (tightly guarded). Expected eval delta (verifiable now, no re-ingest): **P 0.400→0.600, R 0.667→1.000**. The FP cohort is upstream constraint-tier (stale-artifact / `CONSTRAINT-SUBJECT-PRECISION` class), out of scope | [docs/tasks/TEMPORAL-ANTECEDENT-RECALL.md](docs/tasks/TEMPORAL-ANTECEDENT-RECALL.md) |
