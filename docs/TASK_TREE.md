...
| `EXTRACTION-QUALITY-GAUGE` | `active` (gauge done; fixes open) | `R15e`/`R16` | NLI-oracle not-entailed rate = per-doc extraction-quality GAUGE. CHI **~80% erroneous** (hand-validated 18/18) vs APB ~29%. Root: **entity discrimination is broken** (garbage actor noun-phrases; 64% of constraint subjects not recognized signals; noisy catalogs) + flat unconditional constraint model. SpecForge **far from production on hard specs**. Entity discrimination = FOUNDATIONAL fix | [docs/tasks/EXTRACTION-QUALITY-GAUGE.md](docs/tasks/EXTRACTION-QUALITY-GAUGE.md) |
