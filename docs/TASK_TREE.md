...
| `COMPLETENESS-REGION-ACCOUNTING` | `active` | `R15e` (x-cut R15c) | `COMPLETENESS-REGION-ACCOUNTING.2` (`.1` done — owned + provenance verified) — the foundational region-accounting miss detector (research `.3`): an intent-bearing source region that produced no fact = a candidate miss. First slice = intent-bearing TABLE coverage (SignalDescription/RegisterMap/TimingParameter table that yielded 0 records → `UnexplainedTableResidual`), exact via verified table→record links; surfaced in `validate` | [docs/tasks/COMPLETENESS-REGION-ACCOUNTING.md](docs/tasks/COMPLETENESS-REGION-ACCOUNTING.md) |
