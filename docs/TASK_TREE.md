...
| `KNOWLEDGE-MAP-ADOPTION` | `active` (`.1` design done; `.2` pending) | `R0` | Adopt the user's portable **`KNOWLEDGE_MAP_ARCHITECTURE.md`** bundle (the `knowledge-map/` dir) in SpecForge — an **additive, derived, question-keyed retrieval layer** that makes *archaeology* (a fresh agent re-deriving an already-logged fact) structurally impossible. Composes with `MEMORY_ARCHITECTURE.md` + `docs/decisions/` + task-trees; **replaces nothing, converts nothing**. `.2` = copy the bundle verbatim, derive `KNOWLEDGE_MAP.md` from front-mattered fact cards under `docs/knowledge/`, wire the KM gate beside the memory-arch gate (`.githooks/pre-commit` + `run_ci.sh` + `ci.yml`), register in the bootstrap, and seed 3 archaeology-trap cards (docling-cpu, llm-provider-default, temporal-eval-stale-FPs); lazy growth thereafter | [docs/tasks/KNOWLEDGE-MAP-ADOPTION.md](docs/tasks/KNOWLEDGE-MAP-ADOPTION.md) |
