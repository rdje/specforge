...
| `FSMGEN-ASSERT-LOWERING` | `active` (`.1` design + re-pin done; `.2`/`.3` pending) | `R6` | **FSMGen shipped both deltas** — `(stable/changed/rose/fell)` (`6700fbb4`) + `(within B MIN MAX)` → `##[MIN:MAX]` (`92d7036b`, `1<=MIN<=MAX` locked per our answer). Lower the obligations that **residualize today** into the ISF verification family: `Obligation::Stable` → `(assert (stable s))` / `(assert (=> g (stable s)))` (`.2`); general antecedent→consequent + min>1 → `(assert (=> A B))` / `(next B)` / `(within B MIN MAX)` (`.3`). New `AssertProperty` disposition + predicate→ISF-boolean helper + **mandatory parity oracle**; declared-only else residual; every form empirically strict-validated. `.1` = design + **re-pin `43b29f5c → 92d7036b`** (existing emission CI-green on new pin; stable forms strict-valid) | [docs/tasks/FSMGEN-ASSERT-LOWERING.md](docs/tasks/FSMGEN-ASSERT-LOWERING.md) |
