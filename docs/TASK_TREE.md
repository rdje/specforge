...
| `PURE-NLP-INTENT-EXTRACTION` | **`parked` (DEFERRED — do not activate)** | `R17+` | Owner decision (2026-06-05): a future model-based intent extractor (more PDF types/wordings/languages) — **gated**: not activatable until SpecForge is complete + proven *very well* on the current grammar approach AND `PDF-AGNOSTIC-EXTRACTION` is closed. Grammar + document-derived names is the path **now** (prove it first). A future model would generalize the *how*, still honoring ADR `0006` (never memorize names) | [docs/tasks/PURE-NLP-INTENT-EXTRACTION.md](docs/tasks/PURE-NLP-INTENT-EXTRACTION.md) |
