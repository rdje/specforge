...
| `VERB-COVERAGE-CORPUS` | `done` (CLOSED `2026-06-05`; `.1`–`.2`) | `R16`/`R15e` | **Owner request:** the normative-verb list IS the pattern engine's vocabulary — an unlisted verb = a dropped edge. `.1` **seedless LLM mine** (qwen2.5:14b) of the owner's real corpus (`/Users/richarddje/Documents/livework/chipdoc`, **82 PDFs/564 MB**, cross-vendor) → 640 grounded verbs, Claude cross-model curation → reviewed report (`docs/research/verb-coverage-corpus.md`). `.2` integrated the verbs as **actor→signal KG edges** (owner-reframed model: each verb is a KG edge label, `actor —verb→ signal`), centralized the four verb lists into `ir/normative_vocab.rs`, closed the passive-form gap, documented the extraction model in the book. Grammar only, never names (ADR 0006). Used the LLM offline to overcome the seed-dependence since the runtime NLP model is parked. CI green 1258 | [docs/tasks/VERB-COVERAGE-CORPUS.md](docs/tasks/VERB-COVERAGE-CORPUS.md) |
...
| `WIRE-BASED-100` | `active` (program) | `R15e`/`R16` | **Owner non-negotiable bar: 100% on ALL scores for wire-based specs (APB/AHB/AXI/SWD).** APB extraction already 100% (document-level recall 6/6+6/6); the per-statement 0.0 is a STALE-GOLD bug (statement_id drift after re-ingest — gold cites `statement_0202`=a caption; the real sentence moved to `0204` where the extractor correctly put it). Features: `.1` content-anchored eval (root fix), `.2` value-constraint recall, `.3` full-doc completeness, `.4` temporal, `.5` AHB/AXI/SWD | [docs/tasks/WIRE-BASED-100.md](docs/tasks/WIRE-BASED-100.md) |
