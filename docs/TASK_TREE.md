...
| `PRIOR-DECAY` | `active` (`.1` design done; `.2` pending) | `R15c`/`R15e` | **User pick (prior-decay → Dempster).** Grounded gap (`cross-document-learning.md`, Parisi arXiv:1802.07569): `CorpusMemory` priors **only accrete** — when two validated docs map the same key to different values (`manager → Initiator` vs `manager → Target`), nothing notices the contradiction (no revision). `.2` adds a **read-only** `contested_priors()` flagging same-key/different-value priors (ActorTaxonomy/SemanticPhrase/TableShape) with competing values + supports + a `Contested` (advisory-only) disposition, surfaced in `learn-priors`. Additive — no harvest/merge/consultation change (zero fixture/behavior risk); temporal staleness + consultation down-weighting deferred | [docs/tasks/PRIOR-DECAY.md](docs/tasks/PRIOR-DECAY.md) |
