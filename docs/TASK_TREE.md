...
| `INTENT-COMPLETENESS-RESEARCH` | `active` | `R15e` (cross-cutting R15c/d/R16) | `INTENT-COMPLETENESS-RESEARCH.2` (`.1` done — framework landed) — RESEARCH-FIRST program (user directive: accuracy of utmost importance, think it through before coding): theory + design + prioritized backlog for **detecting & bounding intent-capture misses**. Central reframe: a miss = an unexplained intent-bearing source region OR a violated domain closure invariant (both ground-truth-free) + capture–recapture recall estimation. Artifact: [docs/research/intent-capture-completeness.md](docs/research/intent-capture-completeness.md) | [docs/tasks/INTENT-COMPLETENESS-RESEARCH.md](docs/tasks/INTENT-COMPLETENESS-RESEARCH.md) |
