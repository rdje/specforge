...
| `SWD-SERIAL-EXTRACTION` | `active` (research→impl) | `R15e`/`R16` | Serial/architecture extraction for SWD/ADI (IHI0074) → WIRE-BASED-100 100% (the path the parallel-bus model can't reach; owner decision (a) from `WIRE-BASED-100.5j`). `.1` research, `.2` (SWCLK/SWDIO prose-capture), `.3` (typed `SerialFrameField` surface — `NAME[hi:lo]` widths double-gated to serial docs + frame phases → 5 clean SWD fields A/ACK/DATAIN/WDATA/RDATA; parallel buses emit 0 + stay 100%) all DONE. Frontier `.3b`: named request bits (APnDP/RnW) + ACK OK/WAIT/FAULT values + ordering. KM `swd-adi-not-signal-table-spec`, `prose-pin-appositive-signal-capture`, `swd-serial-frame-surface` | [docs/tasks/SWD-SERIAL-EXTRACTION.md](docs/tasks/SWD-SERIAL-EXTRACTION.md) |
