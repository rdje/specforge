...
| `R14-SIGNAL-RESOLVE` | `active` | `R14` | `R14-SIGNAL-RESOLVE.3` (verify skip-mode + book method-doc + close; `.1` design + `.2` producer/command done — lib →1164, `signal-resolve` command landed) — Tier-3 LLM `signal_relation` extraction (Qwen via Ollama): additive producer appending `ActorSignalRelation` edges to the existing `EvidenceIR.actor_signal_relations` KG field | [docs/tasks/R14-SIGNAL-RESOLVE.md](docs/tasks/R14-SIGNAL-RESOLVE.md) |
