...
| `AMBIGUITY-PHRASE-DETECTOR` | `active` (`.1` design done; `.2` pending) | `R8`/`R15e` | Grounded gap from the `LITERATURE-GROUNDING` backlog (`requirements-extraction.md`; NASA ARM ICSE'97 / Berry-Kamsties). A **flag-only** detector (`ir/ambiguity.rs`) that scans EvidenceIR statement prose for vagueness / under-specification markers (NASA ARM weak phrases — "as appropriate", "if necessary", "and/or", "TBD", "but not limited to" — + chip-spec "implementation-defined"/"vendor-specific"; modal verbs MUST/SHALL/SHOULD/MAY excluded) and surfaces them in `validate` (Info finding + `ambiguous_statements` metric). Residual-honesty: say what the spec leaves vague instead of treating it as precise. Extraction-neutral (report-only, like the region-accounting detector); residual-packet routing deferred | [docs/tasks/AMBIGUITY-PHRASE-DETECTOR.md](docs/tasks/AMBIGUITY-PHRASE-DETECTOR.md) |
