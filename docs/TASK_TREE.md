...
| `CVE-PROSE-EXTRACTION` | `active` | `R16` | `CVE-PROSE-EXTRACTION.4` (live Qwen e2e + book method-doc + close; `.1`+`.2`+`.3` done — lib →1159, `extract-contracts` command landed) — wire the live prose→`ActorContract` extractor (Qwen via Ollama) into the dormant R16 constrained-verified surface | [docs/tasks/CVE-PROSE-EXTRACTION.md](docs/tasks/CVE-PROSE-EXTRACTION.md) |
