...
| `NLP-ENRICH-TRANSPORT-DEDUP` | `active` | `R0` | `NLP-ENRICH-TRANSPORT-DEDUP.2` (`.1` done — owned + designed) — complete the text-transport consolidation flagged by `LLM-TEXT-TRANSPORT-DEDUP`: route `nlp_enrich` through the shared `llm_text::call_text_provider` (2→1 copies), behavior-preserving via a `max_tokens` parameter (nlp_enrich keeps 256) | [docs/tasks/NLP-ENRICH-TRANSPORT-DEDUP.md](docs/tasks/NLP-ENRICH-TRANSPORT-DEDUP.md) |
