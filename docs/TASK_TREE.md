...
| `NLI-INTENT-GATE` | `active` (`.1` design done; `.2` pending) | `R16`/`R15e` | **User pick "→ (b)":** make the NLI verifier an *active* IntentIR gate. A **post-build pass** on `IntentIr` (residuals exist there, unlike EvidenceIR): render each contract's obligation as a claim (`obligation_claim_text`, phrasable-only else abstain), NLI-verify against `provenance.source_text`, and **demote** NotEntailed contracts into `residual_decisions` (preserved, never deleted — a verifier error costs a review item, not a lost fact). `nli_gate_contracts` (verifier **injected** → hermetic) + `apply_nli_gate(&mut IntentIr)`; opt-in `intent --nli-verify` flag. `.2` = impl + wiring + tests + book + KM + close | [docs/tasks/NLI-INTENT-GATE.md](docs/tasks/NLI-INTENT-GATE.md) |
