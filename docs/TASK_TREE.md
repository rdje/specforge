...
| `SPEC-MINING-PROVENANCE` | `active` (`.1` design done; `.2`/`.3` pending) | `R0`/`R15e` | **User directive (2026-06-04).** (1) Name SpecForge's discipline **"specification mining"** (Ammons POPL'02) with the forward-vs-backward novelty explicit — SpecForge mines intent *from the spec*, not a spec from an implementation. (2) A per-author **adopt/defer provenance ledger** (`docs/research/grounding/adopt-defer-ledger.md`): for each leveraged author/work — *Take* (the abstraction) / *Leave-out + why* / *Instantiated-at* — in KM cards + book + the ledger. `.2` = temporal trio (Pnueli/GoldMine/Texada — the `G(ante→cons)` template our `temporal_rules`/`ir/temporal_ltl.rs` instantiate) + adopt the framing in README/book; `.3`+ = the rest of the swept authors + synthesis. Verify-every-citation carries over; advisory (no code change) | [docs/tasks/SPEC-MINING-PROVENANCE.md](docs/tasks/SPEC-MINING-PROVENANCE.md) |
