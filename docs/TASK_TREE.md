...
| `SWD-SERIAL-EXTRACTION` | `active` (research→impl) | `R15e`/`R16` | Serial/architecture extraction for SWD/ADI (IHI0074) → WIRE-BASED-100 100% (the path the parallel-bus model can't reach; owner decision (a) from `WIRE-BASED-100.5j`). `.1`–`.3b` (SWCLK/SWDIO prose-capture + 7-field `SerialFrameField` surface) + `.4` (typed `ProtocolStateRecord` FSM surface → **DBGTAPSM + 9 named TAP states + actions**; owner: the FSM is critical to SWD/JTAG) all DONE; parallel buses emit 0 + stay 100%. `.6` DONE (redone correctly): **PROVEN ISF can describe an FSM** — updated the stale (312-commit) submodule→`d31b0b91`, then empirically lowered a 6-state JTAG TAP-DR FSM clean (`storage` var + `switch` + `select` per-state + `rule trigger`); **feature request WITHDRAWN** (no gap; SpecForge emits `.isf` intent, FSMGen lowers to `.fsm`). Frontier `.4b` (FSM transitions), then `.5` (SWD golds→100% + lower the FSM via the proven idiom). KM `swd-adi-not-signal-table-spec`, `prose-pin-appositive-signal-capture`, `swd-serial-frame-surface`, `swd-protocol-fsm-surface`, `isf-fsm-via-switch-select` | [docs/tasks/SWD-SERIAL-EXTRACTION.md](docs/tasks/SWD-SERIAL-EXTRACTION.md) |
