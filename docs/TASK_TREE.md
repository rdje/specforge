...
| `ISF-SYMBOL-SURFACE-EMIT` | `active` | `R6` | `ISF-SYMBOL-SURFACE-EMIT.1` (bump `subs/fsmgen` → `c0b7eaa7` to ingest FSMGen's enum-type answer + verify; then emit the built-but-discarded `(constants)`/`(types)`/`(enums)`) | [docs/tasks/ISF-SYMBOL-SURFACE-EMIT.md](docs/tasks/ISF-SYMBOL-SURFACE-EMIT.md) |
