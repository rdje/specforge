...
| `SPEC-MINING-PROVENANCE` | `active` (`.1`+`.2` done; `.3` pending) | `R0`/`R15e` | **User directive (2026-06-04).** (1) **"forward specification mining"** adopted as SpecForge's discipline name (Ammons POPL'02; forward: spec→intent vs the literature's backward implementation→spec) — in `README.md` + `architecture-rationale.md` + KM card `spec-mining-framing`. (2) A per-author **adopt/defer provenance ledger** (`docs/research/grounding/adopt-defer-ledger.md`): *Take* / *Leave-out + why* / *Instantiated-at*. `.2` done = the framing + the **temporal trio** (Pnueli/GoldMine/Texada — the `G(ante→cons)` template our `temporal_rules`/`ir/temporal_ltl.rs` instantiate). `.3` = the rest of the swept authors + synthesis + close. Verify-every-citation; advisory (no code change); CI green 1229 | [docs/tasks/SPEC-MINING-PROVENANCE.md](docs/tasks/SPEC-MINING-PROVENANCE.md) |
