...
| `PDF-VARIANT-DIGESTION` | `active` (HIGH-PRIORITY program) | `R15`/`R16` | **Owner aim: SpecForge digests ANY chip-spec PDF we throw at it** (82 in the library across ARM/CXL/USB/RISC-V/Intel/AMD/JEDEC/NXP/NVMe/OpenCores; protocol/architecture/TRM/interface/manual). Defer ISF lowering. `.1` triage sweep (running) → per-class feature leaves prioritized by impact; copy a regression PDF + measure per class; keep APB/AHB/AXI/SWD at 100%. `project_pdf_variant_digestion` memory | [docs/tasks/PDF-VARIANT-DIGESTION.md](docs/tasks/PDF-VARIANT-DIGESTION.md) |
