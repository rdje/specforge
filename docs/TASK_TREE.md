...
| `CVE-PROSE-EXTRACTION` | `active` | `R16` | `CVE-PROSE-EXTRACTION.3` (producer + `extract-contracts` command, mock-tested; `.1` design + `.2` typed-surface/fold done — lib 1151→1154) — wire the live prose→`ActorContract` extractor (Qwen via Ollama) into the dormant R16 constrained-verified surface | [docs/tasks/CVE-PROSE-EXTRACTION.md](docs/tasks/CVE-PROSE-EXTRACTION.md) |
