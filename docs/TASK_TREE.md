...
| `REGISTER-MAP-CLASSIFIER-PRECISION` | `active` | `R8` | `REGISTER-MAP-CLASSIFIER-PRECISION.4` (`.1`–`.3` done) — fix a SYSTEMIC `register_map` over-classification found by `CORPUS-HARDENING`. `.3` LANDED + verified: gated `classify_table_kind` on body register-structure (colon bit-range / standalone access token) → I2C register_map 4→0, APB 1→0, I2C tiling-overlap symptom fixed, no regression, CI green. `.4` (next): the harder bit-range-bearing FPs (eMMC RPMB data frames `[511:316]` + register-name TOCs `[159:157]`) need caption/column-vocab exclusion + a clean eMMC re-ingest | [docs/tasks/REGISTER-MAP-CLASSIFIER-PRECISION.md](docs/tasks/REGISTER-MAP-CLASSIFIER-PRECISION.md) |
| `CORPUS-HARDENING` | `active` | `R12`/`R15e` | `CORPUS-HARDENING.2` (`.1` done — harness proven on I2C + eMMC) — standing hardening harness over the real chip-doc corpus (82 PDFs at `~/Documents/livework/chipdoc`); run specs through Docling→`evidence`→`validate`, collect completeness signals, turn recurring real misses into owned fix-trees. AMBA core (AXI/AHB/APB/CHI — home turf + R12 baselines) first. Detector validated precise on real data (I2C TP / eMMC 0-FP-of-48); first discovered issue: over-eager register-map classifier | [docs/tasks/CORPUS-HARDENING.md](docs/tasks/CORPUS-HARDENING.md) |
| `COMPLETENESS-CLOSURE-INVARIANTS` | `active` | `R15d` | `COMPLETENESS-CLOSURE-INVARIANTS.2` (`.1` done — owned) — FIRST implementation from the completeness research (user chose closure invariants): EXACT ground-truth-free miss detectors surfaced as `validate` findings — register bit-tiling (overlap + interior gap, no width speculation) then symbol closure / dangling references (gated). New `ir/completeness.rs`. | [docs/tasks/COMPLETENESS-CLOSURE-INVARIANTS.md](docs/tasks/COMPLETENESS-CLOSURE-INVARIANTS.md) |
| `INTENT-COMPLETENESS-RESEARCH` | `active` | `R15e` (cross-cutting R15c/d/R16) | `INTENT-COMPLETENESS-RESEARCH.5` (`.1`–`.4`+`.6` done — framework + ontology/coverage matrix + region-accounting design + miss-detector catalog + verified literature grounding) — RESEARCH-FIRST program for **detecting & bounding intent-capture misses**. Reframe: a miss = unexplained intent-bearing region OR violated closure invariant (ground-truth-free) + Chao-Mh capture–recapture. Next `.5` (recall + CompletenessReport), then `.7` (prioritized backlog → first impl tree, with user). Artifacts: `docs/research/{intent-capture-completeness, intent-ontology-coverage, region-accounting-design, miss-detectors-catalog, literature-grounding}.md` | [docs/tasks/INTENT-COMPLETENESS-RESEARCH.md](docs/tasks/INTENT-COMPLETENESS-RESEARCH.md) |
