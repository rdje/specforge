...
| `LITERATURE-GROUNDING` | `active` (`.1` design done; `.2`+ = deliberate "later" survey) | `R0`/`R15e` | User-directed program: ground every SpecForge aspect in **verified** published research (papers/articles/books/academia) — leverage prior art instead of reinventing, while explicitly separating genuine out-of-the-box novelty. 11 aspects enumerated (document extraction · staged IR/progressive lowering · requirements/spec mining · protocol & temporal semantics · KG/relation extraction · multimodal fusion · neuro-symbolic/bounded-LLM · cross-document learning · IE eval methodology · uncertainty/residual-honesty · spec→RTL). Per aspect: prior-art / alignment / adopt / extend-novelty / gaps. Reuses the `INTENT-COMPLETENESS-RESEARCH` multi-agent workflow pattern (which already grounded the completeness aspect); **anti-hallucination: every citation must be verifiable**. `.13` synthesizes a reach-full-potential backlog → future owned trees | [docs/tasks/LITERATURE-GROUNDING.md](docs/tasks/LITERATURE-GROUNDING.md) |
| `CONSTRAINT-SUBJECT-PRECISION` | `active` (`.1` diagnosis done; `.2` fix deferred) | `R8`/`R15e` | **The `LLM-EXTRACTION-EVAL` harness's first catch.** The eval scored APB `signal_constraint` at P=0.500 (6TP/6FP); the FPs are real over-extraction by the pattern/NLP-L2 constraint tier in 3 classes: condition-clause signals (`PREADY`/`PSEL` from "until…/if…") minted as subjects; a width parameter (`USER_RESP_WIDTH`) mis-extracted as a signal; and clock/every-mentioned-signal (`PCLK`/`PREADY`) swept into a "must be stable" clause. `.1` owned + diagnosed; `.2` (subject-precision fix + per-class tests + clean CPU re-ingest + eval re-check) deferred | [docs/tasks/CONSTRAINT-SUBJECT-PRECISION.md](docs/tasks/CONSTRAINT-SUBJECT-PRECISION.md) |
