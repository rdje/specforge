...
| `VERB-COVERAGE-CORPUS` | `active` (`.1` mine; `.2` integrate) | `R16`/`R15e` | **Owner request:** since extraction is pattern-matching (not the parked pure-NLP model), the normative-verb list IS the engine's vocabulary — an unlisted verb = a dropped constraint. Mine a comprehensive verb vocabulary from the owner's real corpus (`/Users/richarddje/Documents/livework/chipdoc`, **82 PDFs / 564 MB**, cross-vendor: I²C/Wishbone/CCIX/OpenCAPI/Intel/AMD/Avalon). PoC done (verbs beyond current list: reset/driven/generate/qualify/…). `.1` = `pdftotext` all 82 → provenance-tagged predicate frequency + gap report vs current lists (read-only, owner reviews); `.2` = curate signal-behavioral gaps + integrate (centralize grammar). Grammar only, never names (ADR 0006) | [docs/tasks/VERB-COVERAGE-CORPUS.md](docs/tasks/VERB-COVERAGE-CORPUS.md) |
