...
| `SWD-SERIAL-EXTRACTION` | `active` (research→impl) | `R15e`/`R16` | Serial/architecture extraction for SWD/ADI (IHI0074) → WIRE-BASED-100 100% (the path the parallel-bus model can't reach; owner decision (a) from `WIRE-BASED-100.5j`). `.1` research, `.2` (SWCLK/SWDIO prose-capture), `.3`+`.3b` (typed `SerialFrameField` surface → **7 ordered SWD fields** with bit-widths + ACK resp=[FAULT,OK,WAIT] + named request bits APnDP/RnW; double-gated to serial docs + frame phases; parallel buses emit 0 + stay 100%) all DONE. Frontier `.4`: DP/AP register interface; then `.5` SWD golds→100%. KM `swd-adi-not-signal-table-spec`, `prose-pin-appositive-signal-capture`, `swd-serial-frame-surface` | [docs/tasks/SWD-SERIAL-EXTRACTION.md](docs/tasks/SWD-SERIAL-EXTRACTION.md) |
