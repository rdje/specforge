...
| `SWD-SERIAL-EXTRACTION` | `active` (research→impl) | `R15e`/`R16` | Serial/architecture extraction for SWD/ADI (IHI0074) → WIRE-BASED-100 100% (the path the parallel-bus model can't reach; owner decision (a) from `WIRE-BASED-100.5j`). `.1`–`.3b` (SWCLK/SWDIO prose-capture + 7-field `SerialFrameField` surface) + `.4` (typed `ProtocolStateRecord` FSM surface → **DBGTAPSM + 9 named TAP states + actions**; owner: the FSM is critical to SWD/JTAG) all DONE; parallel buses emit 0 + stay 100%. Frontier `.6` (owner directive: ensure ISF models FSM/serial-frame elegantly or raise an ISF feature request — no hacks), then `.4b` (FSM transitions), `.5` (SWD golds→100%). KM `swd-adi-not-signal-table-spec`, `prose-pin-appositive-signal-capture`, `swd-serial-frame-surface`, `swd-protocol-fsm-surface` | [docs/tasks/SWD-SERIAL-EXTRACTION.md](docs/tasks/SWD-SERIAL-EXTRACTION.md) |
