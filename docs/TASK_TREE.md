...
| `FSMGEN-ASSERT-LOWERING` | `active` (`.1`+`.2` done; `.3` pending) | `R6` | **FSMGen shipped both deltas** — `(stable/changed/rose/fell)` (`6700fbb4`) + `(within B MIN MAX)` → `##[MIN:MAX]` (`92d7036b`, `1<=MIN<=MAX` locked per our answer). `.1` = design + **re-pin `43b29f5c → 92d7036b`** (existing emission CI-green on the new pin). `.2` = **verified-negative**: the `(stable s)` lowering would be UNFAITHFUL — SpecForge's stability obligations are phase-interval-scoped (`Obligation::Stable { during: Between{tick_phases} }`) while `(stable s)` is *unconditional* per-tick, so `(assert (stable s))` over-asserts; **the residual is correct** (KM card `stable-obligation-phase-scoped-residual`; no code change). `.3` = the genuinely-faithful enabled work: general antecedent→consequent `(assert (=> A B))` + min>1 `(within B MIN MAX)` (antecedent = representable boolean); new `AssertProperty` disposition + parity oracle; close | [docs/tasks/FSMGEN-ASSERT-LOWERING.md](docs/tasks/FSMGEN-ASSERT-LOWERING.md) | [docs/tasks/FSMGEN-ASSERT-LOWERING.md](docs/tasks/FSMGEN-ASSERT-LOWERING.md) |
