...
| `COMPLETENESS-RECALL-RELATIONS` | `active` | `R15e` | `COMPLETENESS-RECALL-RELATIONS.2` (`.1` done — owned + design) — extend per-extractor tagging + the recall gauge to actor-signal relations (Pattern @ `evidence` vs LLM @ `signal-resolve`, pre-dedup): `FactKind::ActorSignalRelation` + key + generalize `recall_estimate(_, fact_kind)`; `validate` reports relation recall too | [docs/tasks/COMPLETENESS-RECALL-RELATIONS.md](docs/tasks/COMPLETENESS-RECALL-RELATIONS.md) |
