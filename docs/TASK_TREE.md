...
| `SWD-SERIAL-EXTRACTION` | `active` (research→impl) | `R15e`/`R16` | Serial/architecture extraction for SWD/ADI (IHI0074) → WIRE-BASED-100 100% (the path the parallel-bus model can't reach; owner decision (a) from `WIRE-BASED-100.5j`). `.1` research DONE; `.2` DONE — SWCLK/SWDIO/NSRST captured from the prose "`<role>` pin, `<SIGNAL>`" appositive (`synthesize_signal_declarations_from_prose`; no parallel-bus regression). Frontier `.3`: model the serial frame (start/stop/park/turnaround/parity + WAIT/FAULT/OK ACK). KM `swd-adi-not-signal-table-spec`, `prose-pin-appositive-signal-capture` | [docs/tasks/SWD-SERIAL-EXTRACTION.md](docs/tasks/SWD-SERIAL-EXTRACTION.md) |
