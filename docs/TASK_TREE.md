...
| `EXTRACTION-GAP-FIX` | `active` | `R15`/`R16` | **Owner pivot (`2026-06-08`): fix the 4 extraction gaps `PDF-VARIANT-DIGESTION.4` quantified** — convert measured weakness into real capability (the FIX half of measure→catch→fix). **Honesty guardrail (owner non-negotiable): you cannot extract what is not there** — read the modality where the fact actually lives (NVMe mnemonic→description `.2`; RISC-V name→heading `.3`; RISC-V bits→layout graphic `.4`), or report an honest completeness RESIDUAL; NEVER fabricate a value. `.1` I2C prose acronym/condition precision filter (REMOVE ACK/NACK/DDC/SDR over-captures; precision 0.600→higher) is the frontier. Each leaf re-measures on the SAME `.4a` gold; agnostic (ADR 0006); APB/AHB/AXI/SWD stay 100% | [docs/tasks/EXTRACTION-GAP-FIX.md](docs/tasks/EXTRACTION-GAP-FIX.md) |
