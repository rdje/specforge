...
| `MEMORY-ARCHITECTURE-DOC` | `active` (`.1` done — standard authored) | `R0` | Authoring a portable, harness-agnostic **durable-memory standard** (`MEMORY_ARCHITECTURE.md`) + implementing it in-repo. Layers: resume-pointer (demoted `MEMORY.md`) + task-trees + `docs/decisions/` + git; enforcement E1–E4 (ubiquitous bootstrap pointers, self-check script, git hooks, CI gate) + an agnostic reproduce-anywhere kit so other projects achieve the same hard-to-be-non-compliant property. `.1` standard authored; `.2` decision-record layer + migrate facts; `.3` demote MEMORY.md; `.4` install enforcement kit; `.5` verify + close | [docs/tasks/MEMORY-ARCHITECTURE-DOC.md](docs/tasks/MEMORY-ARCHITECTURE-DOC.md) |
| `LLM-EXTRACTION-EVAL` | `active` (`.1` done — design; paused for MEMORY-ARCHITECTURE-DOC) | `R15d`/`R16` | Building the missing measurement piece: a labeled **precision/recall/F1** eval for the LLM extraction passes (`nlp-enrich`→SignalConstraint, `signal-resolve`→relations, `extract-contracts`→ActorContract), so model A/Bs (qwen2.5vl vs qwen3-vl, Instruct vs Thinking) are rigorous not eyeballed. `kg-bench` (151 fixtures) tests the deterministic pipeline only; the recall gauge is unsupervised — this adds supervised gold. Design: faithful runner over the real command path, closed-world scoring on labeled statements, agent-drafted+review-flagged labels. v1 = 3 text tasks (VLM enrich deferred). Leaves: scorer → labeled data → runner → baseline | [docs/tasks/LLM-EXTRACTION-EVAL.md](docs/tasks/LLM-EXTRACTION-EVAL.md) |
