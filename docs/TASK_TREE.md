...
| `PDF-AGNOSTIC-EXTRACTION` | `active` (program; `.1`–`.4`) | `R16`/`R15e` | **Project-owner SIGNOFF CRITERION:** SpecForge production code must contain **zero** vocabulary belonging to any specific chip-spec PDF — signal names, value/state names **incl. `HIGH`/`LOW`**, protocol family names (`AMBA`/`AXI`), encoding values (`NONSEQ`/`INCR4`) — all **derived from the document at hand** (spec vocab varies per spec AND per revision → hardcoding is non-agnostic + brittle). ADR `0006`. Violations: `evidence.rs` (2 denylists), `semantic.rs:signal_stop_words`. Plan: `.1` invariant + CI guard; `.2` positional/generic value path (drop value words); `.3` document-derived signal validation (`declared_signal_names`, drop family/encoding names); `.4` sweep + guard-to-zero. Test/eval fixtures may use real names (data, not logic) | [docs/tasks/PDF-AGNOSTIC-EXTRACTION.md](docs/tasks/PDF-AGNOSTIC-EXTRACTION.md) |
