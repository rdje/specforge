...
| `EXTRACTION-GAP-FIX` | `active` | `R15`/`R16` | **Owner pivot (`2026-06-08`): fix the 4 extraction gaps `PDF-VARIANT-DIGESTION.4` quantified** — convert measured weakness into real capability (the FIX half of measure→catch→fix). **Honesty guardrail (owner non-negotiable): you cannot extract what is not there** — read the modality where the fact actually lives, or report an honest completeness RESIDUAL; NEVER fabricate a value. **`.1` DONE** (I2C prose precision 0.600→1.000), **`.2` DONE** (NVMe mnemonic-from-description, field-name 0/29→28/29), **`.3` DONE** (RISC-V register-name-from-heading, 0/60→59/60), **`.4a` DONE** (tiling-gated register-diagram bit recovery — pure `ir/register_bits.rs` + `recover-register-bits` command; dmcontrol 14/14 hermetic, dmstatus/name-mismatch residual; lib 1400; no wire-based regression). **Frontier `.4b`** (live VLM bit-extent measurement on the `.4a.2` gold; needs `ollama serve`). Each leaf re-measures on the SAME `.4a` gold; agnostic (ADR 0006); APB/AHB/AXI/SWD stay 100% | [docs/tasks/EXTRACTION-GAP-FIX.md](docs/tasks/EXTRACTION-GAP-FIX.md) |
