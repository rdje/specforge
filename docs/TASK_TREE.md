...
| `LITERATURE-GROUNDING` | `active` (`.1`–`.3` done; `.4`–`.13` pending) | `R0`/`R15e` | User-directed program: ground every SpecForge aspect in **verified** published research, leveraging prior art while separating genuine out-of-the-box novelty. 11 aspects; per aspect: prior-art / alignment / adopt / extend-novelty / gaps; `.13` = reach-full-potential backlog. **Anti-hallucination: every citation verifiable**. Grounded so far (`docs/research/grounding/`): **document extraction** (Docling/TableFormer/DocLayNet/PubTables-1M; TEDS/GriTS) and **protocol & temporal semantics** (Pnueli LTL/Ammons spec-mining/Daikon/Texada/GoldMine — SpecForge's temporal_rules = LTL G(ante→cons) templates; novelty = spec→property not trace→property). 9 aspects + synthesis pending | [docs/tasks/LITERATURE-GROUNDING.md](docs/tasks/LITERATURE-GROUNDING.md) |
