...
| `R16-INTENT-CAPTURE` | `done` | `R16` | — (umbrella CLOSED `2026-05-29`; **R16 program complete** — all 6 sub-trees done at their honest scope boundaries #1/#2/#3/#4/#5/#6; the 2 remaining crux items — waveform raster/vector figure extraction + CVE prose-extractor producer wiring — are upstream-blocked, honestly-deferred FUTURE trees, not re-opened leaves) | [docs/tasks/R16-INTENT-CAPTURE.md](docs/tasks/R16-INTENT-CAPTURE.md) |
| `R16-MODULE-HARDENING` | `active` | `R16` | `R16-MODULE-HARDENING.4` (`.1`/`.2`/`.3` done — figure_region + audit + clean gaps + fidelity glue; `.4` = contract consequent arms + fusion gaps, then close) | [docs/tasks/R16-MODULE-HARDENING.md](docs/tasks/R16-MODULE-HARDENING.md) |
| `FSMGEN-REFRESH-INTEGRATE` | `active` | `R6` | `FSMGEN-REFRESH-INTEGRATE.1` (bump `subs/fsmgen` `9bfb9a20` → `88a7af9c` + verify CI; then read handoff/integration/contract/mdBook + assess adoptable ISF features) | [docs/tasks/FSMGEN-REFRESH-INTEGRATE.md](docs/tasks/FSMGEN-REFRESH-INTEGRATE.md) |
