...
| `CORPUS-HARDENING` | `active` (standing; `.1`–`.3` complete) | `R12`/`R15e` | standing hardening harness over the real chip-doc corpus (82 PDFs). DONE: `.1` harness proven (I2C/eMMC); `.2` full AMBA core run (APB/AHB/AXI/CHI) — all 4 completeness surfaces validated on real data, register classifier FP fixed; `.3` VLM+NLP-in-the-loop recall pass (recovered 6 timing+1 state+5 constraints+16 decls). Future specs/families = new leaves. Follow-up candidates: 1 CHI register overlap (review), 11 unexplained CHI tables | [docs/tasks/CORPUS-HARDENING.md](docs/tasks/CORPUS-HARDENING.md) |
