...
| `COMPLETENESS-RECALL-GAUGE` | `active` | `R15e` | `COMPLETENESS-RECALL-GAUGE.2` (`.1` done — owned + design) — the calibrated recall estimate the fact-tagging precondition unblocks: 2-extractor Lincoln–Petersen over `fact_provenance` (Pattern vs Nlp signal-constraint overlap) → estimated remaining misses as an honest LOWER BOUND (gated on ≥2 tiers + overlap; assumptions printed), surfaced in `validate` | [docs/tasks/COMPLETENESS-RECALL-GAUGE.md](docs/tasks/COMPLETENESS-RECALL-GAUGE.md) |
