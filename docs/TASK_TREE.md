...
| `PER-EXTRACTOR-FACT-TAGGING` | `active` | `R15e` | `PER-EXTRACTOR-FACT-TAGGING.2` (`.1` done — owned + design) — the recall-gauge precondition (research `.5` §3): record which extractor found each fact (Pattern@`evidence` vs Nlp@`nlp-enrich`), pre-dedup, under a canonical key so overlaps are detectable. Provenance-INDEX on `EvidenceIr` (not a 140-site struct field); first fact = `SignalConstraintRecord` | [docs/tasks/PER-EXTRACTOR-FACT-TAGGING.md](docs/tasks/PER-EXTRACTOR-FACT-TAGGING.md) |
