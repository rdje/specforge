...
| `COMPLETENESS-REPORT-SURFACE` | `active` | `R15e` | `COMPLETENESS-REPORT-SURFACE.2` (`.1` done — owned) — the framework §10 unifying headline: aggregate the separate `validate` completeness signals (convergence + register-tiling + region-accounting + prose residuals) into one `Completeness Summary` + `completeness_candidate_misses` metric. Validate-side aggregation (no new detection/IR) | [docs/tasks/COMPLETENESS-REPORT-SURFACE.md](docs/tasks/COMPLETENESS-REPORT-SURFACE.md) |
