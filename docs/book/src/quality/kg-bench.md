...
The tracked suite now includes AXI write-address, write-response, read-address, read-address-sideband-stability, read-data, read-data-last-stability, write-data, write-data-last-stability, sideband-stability, and write-address-sideband-stability timing gold paths, so the same width-only table plus prose actor-relation pattern is checked across `AWVALID` / `AWREADY` / `AWADDR`, `BVALID` / `BREADY` / `BRESP`, `ARVALID` / `ARREADY` / `ARADDR` / `ARLEN`, `RVALID` / `RREADY` / `RDATA` / `RRESP`, `WVALID` / `WREADY` / `WDATA` / `WSTRB`, explicit sideband hold obligations for `ARLEN` and `WSTRB`, read-address sideband holds for `ARSIZE` and `ARBURST`, write-address sideband holds for `AWLEN`, `AWSIZE`, and `AWBURST`, subordinate-owned read-data last-beat stability for `RLAST`, and manager-owned write-data last-beat stability for `WLAST`.
