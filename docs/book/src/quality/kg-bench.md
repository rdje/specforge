...
It also locks zero aggregate temporal rules, proving field-table rows cannot create guarded timing obligations while signal provenance and semantic-role recovery stay empty.
...
It now locks the generic handshake-completion validation metric too, so the typed completion stays counted while the alias-dependent residual and assumption trail remains visible.
...
VLM timing-annotation hints are zero for that negative fixture too, so timing notes cannot masquerade as source-column actor attribution.
The AXI width-only prose-direction fixture now keeps prose-sourced semantic hints at zero too, proving `AWVALID` / `AWREADY` role hints remain table-only while prose actor relations still recover directions.
Alias-grounded prose hints are zero for that fixture too, so phrase-alias recovery cannot masquerade as table-backed AXI width-only evidence.
...
VLM timing-annotation hints are zero there too, so timing notes cannot masquerade as table-backed AXI write-response timing evidence.
That fixture also locks one multi-predicate antecedent, proving the `BVALID` / `BREADY` guard remains visible as a compound temporal condition.
...
Alias-grounded prose hints are zero there too, so phrase-alias recovery cannot masquerade as table-backed write-address control evidence.
Visual-caption hints are zero there too, so diagram captions cannot masquerade as table-backed write-address control evidence.
VLM timing-annotation hints are zero there as well, so timing notes cannot masquerade as table-backed write-address control evidence.
...
VLM timing-annotation hints are zero there too, so timing notes cannot masquerade as table-backed read-address ID evidence.
...
The AXI read-data timing fixture keeps prose-sourced semantic hints at zero too, proving its `RVALID` / `RREADY` role hints remain table-only.
...
The AXI read-data response fixture keeps prose-sourced semantic hints at zero too, proving its `RVALID` / `RREADY` role hints remain table-only while `RRESP` stability stays recovered.
Alias-grounded prose hints are zero there too, so phrase-alias recovery cannot masquerade as table-backed read-data response evidence.
Visual-caption hints are zero there too, so diagram captions cannot masquerade as table-backed read-data response evidence.
VLM timing-annotation hints are zero there too, so timing notes cannot masquerade as table-backed read-data response evidence.
...
The AXI address/response USER sideband fixture keeps prose-sourced semantic hints at zero too, proving address and write-response request/accept role hints remain table-only while `AWUSER` / `ARUSER` / `BUSER` stability stays recovered.
Alias-grounded prose hints are zero there too, so phrase-alias recovery cannot masquerade as table-backed address/response USER sideband evidence.
Visual-caption hints are zero there too, so diagram captions cannot masquerade as table-backed address/response USER sideband evidence.
...
VLM timing-annotation hints are zero there as well, so timing notes cannot masquerade as table-backed write-address sideband evidence.
...
VLM timing-annotation hints are zero there as well, so timing notes cannot masquerade as table-backed APB setup/access evidence.
...
VLM timing-annotation hints are zero there as well, so timing notes cannot masquerade as table-backed APB address/protection evidence.
...
The response fixture keeps prose-sourced semantic hints at zero too, proving `PSEL` / `PREADY` role hints remain table-only while completed-access response obligations stay completer-owned.
Alias-grounded prose hints are zero there too, so phrase-alias recovery cannot masquerade as table-backed APB response evidence.
Visual-caption hints are zero there too, so diagram captions cannot masquerade as table-backed APB response evidence.
VLM timing-annotation hints are zero there as well, so timing notes cannot masquerade as table-backed APB response evidence.
...
Alias-grounded prose hints are zero as well, so phrase aliases cannot masquerade as the learned visual-caption prior.
VLM timing-annotation hints are zero too, so timing notes cannot masquerade as the learned visual-caption prior.
