...
The harness self-tests the missing canonical table-support diagnostic at both `SemanticIR` and `IntentIR`, plus absent expected-signal failures, so these failures identify the fixture, stage, signal-specific expectation field, missing table id or signal, and actual support or signal set.
