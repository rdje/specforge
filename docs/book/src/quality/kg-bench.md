...
The tracked suite now includes AXI write-address, write-response, and read-data timing gold paths, so the same width-only table plus prose actor-relation pattern is checked across `AWVALID` / `AWREADY` / `AWADDR`, `BVALID` / `BREADY` / `BRESP`, and `RVALID` / `RREADY` / `RDATA` / `RRESP` channel shapes.
