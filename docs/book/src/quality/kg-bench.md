...
Low-value labels such as `T0` and `Addr 1` do not become timing constraints, and waveform motion states such as `rising`, `stable`, `falling`, and `UNCHANGED` do not become symbolic signal values.
