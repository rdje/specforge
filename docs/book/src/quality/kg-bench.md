...
The tracked suite now includes AXI write-address, write-response, read-data, and write-data timing gold paths, so the same width-only table plus prose actor-relation pattern is checked across `AWVALID` / `AWREADY` / `AWADDR`, `BVALID` / `BREADY` / `BRESP`, `RVALID` / `RREADY` / `RDATA` / `RRESP`, and `WVALID` / `WREADY` / `WDATA` / `WSTRB` channel shapes.
