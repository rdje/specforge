...
When one artifact emits both graph-coverage and graph-conflict guidance for the same signal, `project-validation` preserves both replay recommendations at SemanticIR and IntentIR stages: the signal id remains a coverage target, and the actor-aware conflict id remains a disagreement target.
...
`rescan-plan` is the first explicit consumer for that schema: it dry-runs by default, prints replay inputs plus a readable action summary before the command hints, and `--execute` dispatches only whitelisted local enrichment/stage rebuild/validate commands from the structured args rather than trusting shell text.
