...
When one artifact emits both graph-coverage and graph-conflict guidance for the same signal, `project-validation` preserves both replay recommendations: the signal id remains a coverage target, and the actor-aware conflict id remains a disagreement target.
