...
Guard predicates, true-FSM transition guards, and selector-bearing branch predicates use the shared registration rule too: graph-backed guard or selector signals create size entries, while stale flat disagreement blocks and does not create a stale entry.
