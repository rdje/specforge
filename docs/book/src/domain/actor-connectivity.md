...
Symbol definitions are intentionally stricter in the active `.fsm` slice: graph-backed signal references inside symbol values still block and do not create renderable signal metadata until symbol expression lowering is deliberately widened.
