...
Waveform motion descriptors such as `rising`, `stable`, `falling`, `UNCHANGED`, `RISING_EDGE`, and `LOW_TO_HIGH` are also filtered from the signal-value path; they describe movement or persistence, not concrete sampled values.
