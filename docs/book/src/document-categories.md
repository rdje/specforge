...
| **3 — platform / system-IP** | **Partial.** Registers and their bit-fields lower in volume (thousands; CoreSight SoC-600 alone ~3,250 fields) — the same road as cat-2. | The *topology* (what connects to what, clock/reset trees) **is captured** as a typed producer→consumer connectivity + clock/reset distribution surface, but it is **sparse and not lowered**: ISF has no static-topology construct (its composition is transaction-level only) and the emit is per-actor, so cross-component topology has no home yet. The next step is measuring how complete that capture is before deciding whether to grow ISF or treat topology as the *integrator's* concern above per-module synthesis. |
