...
evidence: crates/specforge/src/ir/semantic.rs (parse_explicit_signal_declaration, the `if index != tokens.len() { return None; }` arm; build_interfaces; declared_signal_names; the signal_constraints grounding partition); docs/tasks/SIGNAL-DECLARATION-ROW-DROP.md (.4); docs/tasks/EXTRACTION-QUALITY-GAUGE.md (.3k.7)
reverify: "python3 scripts/measure_declared_signals_missing_from_semantic.py — expect 83 missing across 10 documents; the AXI-L line must show WSTRB"
---
...
Over all 77 documents carrying both an EvidenceIR and a SemanticIR (`2026-09-13`): **83 signals are
...
* **17 carry a legitimate arithmetic width.** AXI's `Signal WSTRB is output width DATA_WIDTH / 8.`
  stops after `DATA_WIDTH`, leaving `/` and `8` unconsumed. Same for `WPOISON`/`RPOISON`
  (`ceil(DATA_WIDTH / 64)`), `RUSER` (`USER_DATA_WIDTH + USER_RESP_WIDTH`) and the four `*IDCHK`
  (`ceil((ID_W_WIDTH + int(Unique_ID_Support))/8)`). The tell is that AXI's catalog carries
  **`WSTRBCHK` but not `WSTRB`** — the parity companion has a plain width and the wire it checks does
  not.
* **66 carry a width the source row never stated as one** — MMU-700's
  ...
  refusing them silently is the defect either way.
...
EvidenceIR and the SemanticIR constraint count did not move (53 → 53). That is not the fix failing:
...
only at SemanticIR will read as a no-op for any signal in this set.
