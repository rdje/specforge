...
title: SWD protocol surfaces reach SemanticIR exactly but do not yet reach IntentIR or ISF
answers:
...
evidence: docs/tasks/SWD-SERIAL-EXTRACTION.md (.7b); docs/decisions/0016-swd-protocol-projection-and-honest-isf-boundary.md; crates/specforge/src/ir/semantic.rs; crates/specforge/src/commands/validate.rs
reverify: "rg -n 'serial_frame_fields|swd_operations|protocol_states|interface_edge_timings' crates/specforge/src/ir/semantic.rs && ! rg -n 'serial_frame_fields|swd_operations|protocol_states|interface_edge_timings' crates/specforge/src/ir/intent.rs crates/specforge/src/ir/isf_ir.rs"
---
...
`SWD-SERIAL-EXTRACTION.7b` adds the same exact record types to SemanticIR. The builder clones each collection
without filtering, sorting, rewriting ids, or changing provenance; empty collections are serde-defaulted and
omitted, so older artifacts remain compatible. SemanticIR validation reports all four collection counts.

Therefore the 29/29 SWD derivation score proves fresh extraction fidelity at EvidenceIR, while `.7b` proves
lossless availability one stage later. The records still do not reach the canonical IntentIR product or ISF;
`.7c` and `.7d` own those boundaries.
