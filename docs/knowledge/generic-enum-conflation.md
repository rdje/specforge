...
**`.5.iv.a` LANDED (`2026-08-31`), then partly CORRECTED the same day — the header now SOURCES the name,
three `.5.iv` records were corrected, and two of this leaf's own published statements were then withdrawn
after the director audited them (the `RESERVED` split was `5/7` and is `6/6`; the `DataSource` worked
example is backwards — the merge DROPS the reserved rows). Report §`.5.iv.a correction`.** `derive_header_sourced_enum_name` (`ir/evidence.rs`) runs after both existing paths decline, so
it is strictly additive. Re-measuring over the population the SHIPPED scan visits changed the lever's size
(994 candidates / **285 accepted** in 9 documents, not 134 in 10 — `.5.iv` censused `encoding`-kind tables
only) and its predicate: the dominant junk is **positional headers** (`Bytes`/`Offset`/`Index`/`bits` — the
column holds a POSITION, so the table is a field LAYOUT), which removes 448 candidates alone and subsumes
the `OFFSET` and garbled-`AXADDR` classes, while **"at least one value cell must parse as an encoding
literal"** subsumes the `*_WIDTH` class *and* the glossary/notation/abbreviation class `.5.iv` never saw.
**`RESERVED`-only did not ship**: five of the twelve self-eliminate in `build_symbol_definitions`'
conflicting-value rule, the other seven are structurally identical to 31 legitimate single-member tables,
and the only remaining discriminator is a spec-assigned value word ADR 0006 forbids. The lever is **inert on
the measurable stratum** — `check_chain_currency.sh` reports 24/24 current at every stage — because 8 of its
9 documents are legacy schema-1 chains and the one rebuildable table (the SMMU guide `Table 3-1` that opened
`.5.iv`) mints nothing, exactly as `.5.iv`'s honest correction predicted in prose.
Report `docs/research/generic-enum-conflation-results.md` §`.5.iv.a`.
...
[[evidence-proof-binds-artifact-location]] (the oracle defect found while gating `.5.iv.a`).
