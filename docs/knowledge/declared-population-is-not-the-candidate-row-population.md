...
evidence: scripts/measure_declaration_name_cell_shapes.py; crates/specforge/src/ir/evidence.rs (synthesize_signal_declarations; signal_names_in_name_cell; NAME_COLUMN_OVERRIDE_MARGIN; infer_signal_table_row_width_hint); docs/tasks/PROSE-NAME-CELL-DECLARATION.md (.0)
reverify: "python3 scripts/measure_declaration_name_cell_shapes.py — expect candidate-row approximation 318 rows / 27 documents; CURRENT stratum 604 declarations, 604 joined (100.0%), 528 single-token / 72 comma-family / 2 footnote-marked / 0 bracket-suffixed / 0 repeated-token / 2 phrase; LEGACY 2085 declarations, 1251 joined (60.0%), 15 phrase"
---
...
| --- | ---: | ---: | ---: | ---: |
| current (proof-carrying) | 4 | 604 | **604 (100.0%)** | **2** |
| legacy (inspection-only) | 22 | 2,085 | 1,251 (60.0%) | 15 |
...
- **`single-token`** — one whitespace token. 528 current.
...
## What the shapes could not separate, and why it matters

Both current-stratum phrases are AHB `table_0004`: `Clock source` → `Clock`, `Reset controller` →
`Reset`. That table is **rotated** exactly like AHB `table_0033`, but it has two body rows, so the real
name column scores 2 distinct tokens against the header column's 2 and `NAME_COLUMN_OVERRIDE_MARGIN =
2` blocks the correction. Refusing the phrase silences the row; correcting the column reads it. No wire
is lost either way — `HCLK` and `HRESET` are recovered from `table_0033`, where the same rotation wins
19 vs 4. The same two rows also carry a whole sentence as their width, accepted as
