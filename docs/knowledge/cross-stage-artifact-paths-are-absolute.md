...
evidence: docs/tasks/SWD-SERIAL-EXTRACTION.md (surfaced portability finding); crates/specforge/src/ir/evidence.rs; crates/specforge/src/ir/semantic.rs; crates/specforge/src/ir/intent.rs; crates/specforge/src/ir/adapters.rs
...
artifacts still embedding the deleted `/Users/richarddje/Documents/github/specforge` root.
...
The finding arose during `SWD-SERIAL-EXTRACTION.4e`; its task tree records the clean-tree handoff requirement.
A dedicated task must define backward-compatible path resolution, migrate/verifiably rebuild affected local
artifacts, and add a fail-closed path census. Do not bulk-rewrite ignored artifacts before the resolver and
migration contract exist.
