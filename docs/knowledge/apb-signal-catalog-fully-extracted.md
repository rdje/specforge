...
signal keeps the table flagged. APB `validate` candidate_misses **5 → 3** (0016/0017 covered; `0018` stays
flagged honestly — docling trapped its signals in the header rows; 2 prose residuals remain). No fabrication:
the inventory (`collect_known_signal_names`) is read, never written. See `[[conformal-tier-agreement-degenerate]]`
for the related Pattern×Nlp disjoint-tier finding on APB.
...
APB candidate_misses = **2**, both genuine: `table_0018` (docling-garbled) + `statement_0370` (EDC
end-to-end — a non-wire system requirement, correctly left unstructured).
