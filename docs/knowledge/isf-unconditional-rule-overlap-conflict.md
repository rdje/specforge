...
evidence: crates/specforge/src/ir/isf_ir.rs (drop_unconditional_overlap_conflicts — per-signal unconditional value, drop different-value rules; unconditional_overlap_residual_packet → isf_unconditional_overlap_<name>; wired after dedup_conflicting_rules and before rule/transaction conflict residualization); subs/fsmgen/perl/FSM/Scheduler/ISF/LoweringIR.pm (_build_conflict_issues and _condition_terms_prove_disjoint); docs/tasks/KG-ISF-COMPLETENESS.md (.2a.v node + acceptance checklist); docs/knowledge/isf-rule-transaction-priority-authority.md
