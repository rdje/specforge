...
reverify: "python3 -c \"import json;d=json.load(open('generated/evidence_ir/ihi0074_a_2017_03_09_arm_debug_interface_v6_architecture_specification/evidence_ir.json'));print([v['caption_text'] for v in d['visual_evidence'] if v.get('asset_id') in ('picture_0038','picture_0039')])\""
---
...
statement exists for all 11 fields, so the rule would always produce something — and for at least 7 it would
produce the wrong phase. `APnDP` and `RnW` would take `transfer` against a gold of `request`; `Start`, `Parity`
