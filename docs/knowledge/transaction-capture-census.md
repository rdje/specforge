...
> **STATUS UPDATE — `.2g`/`.2h` LANDED + `.2i` PARKED pending FSMGEN (`2026-06-16`).** `.2g` (commit
> `bb83a32b`) added the typed `SemanticIr.transaction_phases` recogniser (`build_transaction_phases` +
> `derive_phase_name`, `ir/semantic.rs`; universal `<qualifier> phase` grammar + `PHASE_NAME_STOPWORDS`
> gate) + a `validate <semantic-ir>` phase inventory — recognition only, SemanticIR-only, `.isf`
> byte-identical. `.2h` (commit `ab5bda6a`, measurement-only) found a transaction's phase ORDER is NOT
> cleanly+universally recoverable → ordering is an honest residual. **`.2i` measurement (`2026-06-16`,
> read-only Rule-A grouping over the 4 wire docs):** grouping each transaction's `.2c` membership by each
> phase's document-global signal set is groundable **cleanly only on AHB (1 of 4)** — and noisily
> (HCLK/HWRITE ungrouped, HREADY multi-phase) — and is **empty on APB (thin membership), AXI and SWD (phase
> prose names no declared signal).** **ISF-grammar findings (empirically probed,
> `subs/fsmgen/bin/fsmgen --strict --check --json` @ `8c39827f`):** a transaction body is TOTALLY ORDERED
> (`13b`: "the scheduler links states in order … what you write is what you get"); a value-less
> `(drive SIG)` is REJECTED — *"missing actual for 'val'"* — every drive needs a concrete value;
> `(sample INPUT as name)` is value-free-ACCEPTED but `(drive INPUT)` is REJECTED ("not defined" — drives
> exist only for outputs); same-cycle concurrency exists only via a multi-pair drive block and collides with
> SpecForge's per-output top-level named drives (`isf_priority_mixed_timing_conflict`). **So lowering the
> grounded membership into an ISF transaction body would force inventing per-output VALUES + a cross-phase
> ORDER the document does not ground.** Per the no-hacks doctrine (`[[feedback_isf_no_hacks]]`) this is raised
> to FSMGEN as a question/feature request (value-less output participation / unordered-or-partial-order body /
