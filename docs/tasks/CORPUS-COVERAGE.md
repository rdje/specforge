...
- Frontier (active): `CORPUS-COVERAGE.2.33d` — repair the measured universal false-signal feedback/trust path,
  prove the real USB adapter no longer launders prose into hardware, and retain WIRE/FSMGen/KG truthfulness.
...
| 33 | `usb_3_2_revision_1_0_2017_09` USB 3.2 | 548 | **full current-binary cascade + foundational semantic-fidelity finding:** guarded CPU ingest restores the normalized bundle and produces 548 pages / 507 visuals / 283 tables / 5,830 elements; EvidenceIR 8,412 statements / 45 relations / 542 conditional rules / 102 timing constraints; IntentIR 51 actors / 2 interfaces / 2,893 behaviors / 3,015 constraints / 27 transactions / 1 register. SourceIR 6,584→5,830 is cleaner visual-label suppression (all 53 loss pages have visuals; ≤10-char affected-page fragments 543→3), not prose loss. **Semantic block:** weak candidates `AT`/`USB`/`ENHANCED`/`NO` are laundered through relation-derived directions; phantom actor `setportfeature_port_over_current` wins the tied net-producer selection and yields four ports / four enums / one storage / 29 mostly unconditional rules. Durable fact `dense-prose-false-signal-loop-reaches-isf`; repair owned by `.2.33d`. After #33: 80/2/80/79 stage census and **23 real chip-spec refreshes remain.** | emitted but **SEMANTICALLY BLOCKED** (`setportfeature_port_over_current.isf`) | **0 diagnostics** ✓ syntax only; not fidelity proof |
...
**Running strict tally (post-`.2a.v`, MEASURED on a clean full re-emit + FSMGen sweep over the CURRENT-emit set — both the prior "27/28, 1 FAIL" per-doc tally AND the first "97/107" per-`.isf` tally were inaccurate):** `adapt` emits ONE primary-actor `.isf` per doc, so the 107-file cache held 37 STALE cruft files from older binary versions (now cleaned — git-ignored cache, serves the ≥24h artifact-cleanup directive). With USB #33 added, the **71 current-emit `.isf`: 71 PASS / 0 FAIL** syntax tally remains green; #33 proves this downstream grammar tally is not a semantic-fidelity oracle (its USB output is explicitly blocked pending `.2.33d`). **Levers A, B, C, F + the rule-value gate all ✅ resolved:** Lever B (module-name HDL-sanitization, `.2a.iii`); Lever A (value width-alignment, `ISF-VALUE-WIDTH-EMIT.2`, re-verified clean `2026-06-23`); Lever F (enum value-literal emit-gate, `.2a.iv`, HBM2 strict-clean); **Lever C (unconditional-rule-overlap conflict residual, `.2a.v`, `2026-06-23` — 6 docs FAIL→PASS incl. all 3 wire golds + LPI/LTI/NVMe, 0 regressions)**; **`.2a.vi` rule-drive-value validity gate (`2026-06-23` — closed the last AXI+ACE `ihi0022_h_c` `(port expr)` FAIL by residualizing its 4 prose-valued loopback rules)**. **The ISF-emit strict-FAIL frontier is now fully CLOSED — every renderable doc emits FSMGen-`--strict`-clean `.isf`.** (The stale 107-file cache's extra "FAILs" — `agent`/`address_decoder`/`able_to`/`bit_fields`/`redistributor→…` — were secondary-actor files the current binary no longer emits.) **The earlier "Surfaced lever F" note below is now historical (resolved by `.2a.iv`).** **NEW Surfaced lever F (#28 HBM2 — ISF-emitter, kin to Lever A):** the emitter emits an enum-member value as a bare token (`TABLE.REPAIR_LANE_8` = `1000`) that FSMGen rejects (package symbol values must be width/radix-qualified scalar literals like `4'b1000`); the REPAIR_LANE values are BINARY codes whose radix was stripped (so `1000` is also semantically wrong vs binary 8) — a value-literal emission gap in the `ISF-*-EMIT` family + a related generic-`TABLE` mega-enum extraction-precision concern (one enum conflating ~10 distinct doc tables). **Surfaced lever D (upstream/non-emitter, now seen on 4 docs):** non-AMBA table/signal recognition — Wishbone-style `*_O`/`*_I` suffix-notation + prose signal-lists, OpenCAPI-style packet/command tables left `unknown` (219/246), and the RISC-V-IOMMU/VT-d in-memory STRUCTURE tables — a `PDF-VARIANT-DIGESTION` signal/table-recall family (kin to the parked `.9.10`). **NEW Surfaced lever E (#27 eMMC — KG-precision, NOT emitter):** **agent-identity precision for descriptive-prose / non-AMBA specs** — fresh Docling of prose-heavy specs mints ~90+ phantom sentence-fragment actors (`host has`/`host to`/`cache in`/`B write`) the `.1a`/`.1b` gates (measured clean on AMBA docs) don't catch; needs its OWN owned `KG-ISF-COMPLETENESS` leaf (probe-first, structural gates not denylists — owner steer), as the prose tail (USB/HBM/guides) likely recurs it. eMMC itself remains ISF-unaffected, but USB #33 overturns the broader assumption: a dense-prose false-signal feedback loop can make a phantom actor renderable and reach strict-clean ISF. `.2.33d` owns that semantic-trust repair; it is not a syntax strict-FAIL. Watch the long-tail for width-align (A) / rule-conflict (C) / non-AMBA-table-recall (D) / prose-actor-precision (E) / enum-value-literal (F) recurrence to scope those.
