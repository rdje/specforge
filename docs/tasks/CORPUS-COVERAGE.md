...
- Frontier (active): `CORPUS-COVERAGE.2` — re-ingest the 57 normalized-missing docs from the
  `.cache/local-references/chipdoc` symlink, register/TRM/ISA phase, one doc per slice (21 of 57 done after
  #21 RISC-V IOMMU). Next candidates: a CoreSight SoC-600 TRM (`100806_*`, the `.10c` family lifted CoreSight
  TMC 2→30), JEDEC eMMC (`jesd84_b50`, EXT_CSD register-heavy), or AMD-IOMMU (`48882_*`) — all genuinely
  pre-`.10`-stale register/structure docs where the `.10`/message-field families fire.
