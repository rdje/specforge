...
- Frontier (active): `CORPUS-COVERAGE.2` — re-ingest the 57 normalized-missing docs from the
  `.cache/local-references/chipdoc` symlink, register/TRM/ISA phase, one doc per slice (23 of 57 done after
  #23 CoreSight SoC-600 0701 TRM). **Measured-corrected expectation (`2026-06-23`, #22–#23):** a one-pass survey of
  ...
  `.isf` richness): CoreSight SoC-600 0200/0100 (`100806_0200`/`100806_0100`, 631/597 regs — same family as #23, so
  also likely a consolidation+transaction refresh), Intel VT-d (`5_0_…`, 103 regs), JEDEC eMMC/HBM
...
**Running strict tally (23 renderable docs, post-`.2a.iii`):** **21 clean** (incl. GIC-600, now fixed, + GIC-arch + SMMU-arch + Avalon #17 + Wishbone #18 + OpenCAPI-TL #19 + GIC-400 #20 + RISC-V-IOMMU #21 + Cortex-A76 #22 + CoreSight-SoC-600-0701 #23 — whose 3 `ATB` rule-conflicts RESIDUALIZE cleanly, unlike LPI's). **2 FAIL** = DTI (Lever A, value width-align), LPI (Lever C, rule-conflict). Lever B (module-name) ✅ resolved. **Surfaced lever D (upstream/non-emitter, now seen on 2 docs):** non-AMBA table/signal recognition — Wishbone-style `*_O`/`*_I` suffix-notation + prose signal-lists, and OpenCAPI-style packet/command tables left `unknown` (219/246) — a `PDF-VARIANT-DIGESTION` signal/table-recall family (kin to the parked `.9.10`). Watch the long-tail for width-align (A) / rule-conflict (C) / non-AMBA-table-recall (D) recurrence to scope those.
