...
- Frontier (active): `CORPUS-COVERAGE.2` — re-ingest the 57 normalized-missing docs from the
  `.cache/local-references/chipdoc` symlink, register/TRM/ISA phase, one doc per slice (22 of 57 done after
  #22 Cortex-A76 TRM). **Measured-corrected expectation (`2026-06-23`, #22):** a one-pass survey of the
  remaining 35 docs' persisted evidence shows the previously-named "genuinely pre-`.10`-stale" register
  candidates are ALREADY current-binary-equivalent — CoreSight SoC-600 ×3 carry 597/631/833 `register_records`,
  AMD-IOMMU `48882` carries 217 `message_field_records`, Intel VT-d 103 registers — i.e. their evidence was
  rebuilt from the retained `source_ir.json` (before the `normalized/` bundle was reclaimed) and already holds
  the `.10`/message-field surfaces. So the marquee-jump phase is effectively over; the remaining slices are the
  **#21/#22 confirmation+restoration class** (restore the `normalized/` bundle + confirm on the current binary +
  document honest absences / surfaced Lever-D recall gaps), not big surface jumps. Next candidates (by descending
  `.isf` richness): CoreSight SoC-600 0701 (`100806_0701`, 833 regs — biggest register `.isf`), Intel VT-d
  (`5_0_…`, 103 regs), JEDEC eMMC/HBM (`JESD84-B50`/`JESD235*`), then the thin OpenCAPI/PHY/guide tail
  (honest-absence confirmation). A genuinely pre-`.10`-stale doc with a real jump remains possible but is now the
