...
**Cumulative update after #36:** 36 current-binary refreshes are complete and 20 real chip-spec documents remain.
CoreSight Base System joins USB4 Connection Manager, USB4 Inter-Domain, USB 3.2, and HBM-gen1 as honestly blocked/non-emitting; 67 current renderable
...
**Running strict tally (post-`.2.36`, measured on the current converged output set):** `adapt` emits one
primary-actor `.isf` per renderable document. The cache now contains **67 current `.isf`: 67 PASS / 0 FAIL**;
USB #33, USB4 #34, USB4 Connection Manager #35, and CoreSight Base System #36 are honestly blocked and emit none. This confirms both that the grammar frontier remains closed and that
