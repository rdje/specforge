...
- Frontier: `CORPUS-COVERAGE.2` — select and authenticate roadmap-aligned corpus refresh #45 from the 12 remaining
  candidates, then create its owned leaf before any generated mutation.
...
**Cumulative update after #44:** 44 current-binary refreshes are complete and 12 real chip-spec documents remain.
...
PHY-mechanical output's removal reduces the current renderable set to 65; all 65 are FSMGen-strict clean. The preceding long-form cumulative paragraph is the historical
...
**Running strict tally (post-`.2.44`, measured on the current converged output set):** `adapt` emits one
primary-actor `.isf` per renderable document. The cache now contains **65 current `.isf`: 65 PASS / 0 FAIL**;
...
Definition #42, Ready Definition #43, and 32 Gbps PHY Mechanical #44 documents are honestly blocked and emit
