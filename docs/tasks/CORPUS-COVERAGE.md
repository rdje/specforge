...
**Cumulative update after #35:** 35 current-binary refreshes are complete and 21 real chip-spec documents remain.
USB4 Connection Manager joins USB4 Inter-Domain, USB 3.2, and HBM-gen1 as honestly blocked/non-emitting; 68 current renderable
...
**Running strict tally (post-`.2.35`, measured on the current converged output set):** `adapt` emits one
primary-actor `.isf` per renderable document. The cache now contains **68 current `.isf`: 68 PASS / 0 FAIL**;
USB #33, USB4 #34, and USB4 Connection Manager #35 are honestly blocked and emit none. This confirms both that the grammar frontier remains closed and that
