...
| 18 | `wbspec_b4` Wishbone B4 | 128 | **wire-signal inventory comes out EMPTY** (0 interfaces / 0 signal_records → relations 1→0 — the lone stale fragment is dropped by the `.1a` gate, NOT a regression: the stale build also had ~1); IntentIR still carries **215 free-text constraints + 2 transactions + 8 actors** (the intent is captured as obligations/behaviors, just not as the typed wire surface). Surfaces a real **signal-inventory recall gap**: Wishbone documents its signals in the `SIGNAL_O()`/`SIGNAL_I()` suffix-notation + prose signal-list style that the current signal-table/prose extractors don't recognize → an upstream `PDF-VARIANT-DIGESTION` signal-recall lever (kin to the parked `.9.10` prose-bus-line lever), NOT an emitter lever and NOT fixed in-slice | renderable but THIN (`arbiter.isf`, 1 signal) | **0 diagnostics** ✓ (strict-clean) |

**Cumulative (18 docs, protocol + register/arch phases): hundreds of registers + ~370 message fields surfaced that were ABSENT in stale evidence** — registers e.g. SMMU-arch 1→89, GIC-arch 17→90, MMU-700 13→63, GIC-600 15→33, TMC 2→30; message fields DTI 0→159, CHI-C2C 0→210. **16/18 strict-clean `.isf`** (after `.2a.iii` fixed GIC-600; #17 Avalon + #18 Wishbone clean); 2 strict-FAIL remain (DTI width-align = Lever A, LPI rule-conflict = Lever C). #17–#18 are the first interconnect specs whose refresh value is NOT marquee table-family gains: #17 Avalon = consolidation + current-binary freshness (no register-field/message-field/presence tables); **#18 Wishbone surfaced a signal-inventory RECALL GAP** (0 interfaces / 0 signal_records — its `*_O`/`*_I` suffix signal notation isn't caught by the current extractors → degenerate 1-signal `.isf`; an upstream signal-recall lever, not an emitter one). Honest per-doc outcomes — the sweep is doing its job of surfacing which doc styles extract well vs poorly. Remaining queue = the long tail (OpenCAPI×14/JEDEC/USB/RISC-V system-IP/VT-d/Cortex-A76/GIC-400/overview/guides).
...
**Running strict tally (18 renderable docs, post-`.2a.iii`):** **16 clean** (incl. GIC-600, now fixed, + GIC-arch + SMMU-arch + Avalon #17 + Wishbone #18). **2 FAIL** = DTI (Lever A, value width-align), LPI (Lever C, rule-conflict). Lever B (module-name) ✅ resolved. **New surfaced lever (D, upstream/non-emitter):** Wishbone-style `*_O`/`*_I` suffix-notation + prose signal-list signal recall (PDF-VARIANT-DIGESTION family, kin to the parked `.9.10`). Watch the long-tail for width-align (A) / rule-conflict (C) / Wishbone-style signal-recall (D) recurrence to scope those.
