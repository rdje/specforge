...
- Frontier (active): `CORPUS-COVERAGE.2` — re-ingest the 57 normalized-missing docs from the
  `.cache/local-references/chipdoc` symlink, register/TRM/ISA phase, one doc per slice (25 of 57 done after
  #25 CoreSight SoC-600 0100 TRM — the CoreSight SoC-600 cluster 0701/0200/0100 is now COMPLETE, all three genuine
  consolidation+transaction refreshes, confirming the family is uniformly the #23/#17 refresh class). **Measured-corrected expectation (`2026-06-23`, #22–#23):** a one-pass survey of
  ...
  `.isf` richness): Intel VT-d (`5_0_…`, 103 regs, 12MB — distinct vendor), JEDEC eMMC/HBM
