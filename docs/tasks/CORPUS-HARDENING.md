...
| arm AMBA CHI IHI0050_G | 11666 | 11% | 20 (overlap root-caused + **fixed at classifier**) | 2p · 591 · ✓ | **region accounting: 11 unexplained intent-bearing tables**; 1024 nlp residuals; completeness_candidate_misses=1036. *The 1 register overlap was the symptom of 8 phantom registers from 2 DVM field-encoding tables (`table_0170`/`0171`) mis-typed as register maps; `REGISTER-CLASSIFIER-ENCODING-FP` (CLOSED `2026-06-01`) reclassifies them to `encoding`. Projected by the table-classifier harness: 20→12 registers, overlap 1→0. **RE-INGESTED `2026-06-06`** (fresh Docling): `table_0170`/`0171` now classify as **`encoding`** (not register_map) in the regenerated SourceIR (368 tables: 230 unknown / 109 encoding / 18 timing / 8 signal_description / 3 feature_matrix; **register_map count = 0**) — the classifier fix is realized in the artifact. (Exact downstream register/overlap count needs a full converge run on this large doc.)* |
