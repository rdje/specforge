...
The stable landing, separate title-part directory, 64-card packing, derived 198-card maximum, limits, writer
transaction, and two-stage migration remain accepted.
...
row ceiling remains 320. This does not change ADR 0020's part surface limits: 64 worst-case migrated rows plus the
fixed scaffold remain below 24,576 bytes, and 333 remains below the 384-byte health target. For the current 158
cards, the corrected in-memory result is three parts totaling 176 lines / 34,720 bytes, with a 14,306-byte largest
part and 268-byte widest line. The 172-line / 12,361-byte landing is unchanged; the combined projection is
47,081 bytes.
