...
The decision packet above was built in three landed extraction-side gates and one follow-on measurement; every
per-leaf result is retained verbatim in
[`generic-enum-conflation-results.md`](generic-enum-conflation-results.md).

Composed by derivation: the generic name is gone as a *class* and the member-quality defect underneath it is
gated per member, both structurally and without a name list (ADR 0006). `.5.i` gated the
`derive_encoding_enum_name` fallback on independent document evidence and gated the emitter's `(types …)`
block by `emitted_enums()`, taking generic-named enums 82 → 8 and total enum records 422 → 105 across the 33
rebuildable docs, with the 8 survivors document-evidenced rather than missed. `.5.ii` then disproved this
record's own per-enum plan — a whole-enum drop destroys AXI-gold `BRESP`'s real codes — and landed a
per-member sentence-spine fragment gate at precision 1.000 / recall 1.000. `.5.iii` measured the five deferred
residual classes, found four subsumed or FP-unsafe, and landed the one that reached a wire gold: the `_WIDTH`
parameter-leak member gate, dropping exactly 7 leaks from `ihi0022_l` with an empty corpus false-positive set.
WIRE-BASED-100 held **1.000 before == after** across all three, `kg-bench` stayed 156/156, and every changed
`.isf` stayed FSMGen-`--strict` clean.

Open, and reporting into the results record: `.5.iv` measured that an encoding table's own column header may
**source** an enum name and not merely veto one — 2,540 encoding tables yield 134 that would mint an enum, and
0 of 28 same-name collision groups conflict on any shared value, so merge-by-name does not re-create the
