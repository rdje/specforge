...
fn build_signal_inventory(intent_ir: &IntentIr) -> Vec<FsmSignalCandidate> {
    ...
    if let Some(system_contract) = intent_ir.system_contract.as_ref() {
...
fn overlay_system_contract_signal_inventory(
...
fn overlay_system_contract_signal(
    inventory: &mut BTreeMap<String, SignalInventoryEvidence>,
    signal_name: &str,
    role_name: &str,
    system_contract: &SystemContractRecord,
...
fn build_module_candidates(intent_ir: &IntentIr) -> Vec<FsmExplicitModuleCandidate> {
    let topology_directions = collect_module_topology_port_directions(
        &intent_ir.explicit_tops,
        &intent_ir.explicit_modules,
    );
    intent_ir
        .explicit_modules
        .iter()
...
fn build_module_candidate(
    ...
    if let Some(system_contract) = module.system_contract.as_ref() {
        overlay_system_contract_signal_inventory(&mut signal_inventory, system_contract);
    }
    overlay_actor_port_width_inventory(&mut signal_inventory, actor_ports);
    overlay_actor_port_inventory_for_actor(
...
fn collect_module_topology_port_directions(
    ...
    let module_signal_widths = collect_explicit_module_signal_numeric_widths(explicit_modules);
...
fn merge_top_port_evidence_from_actor_ports(
    top: &ExplicitTopRecord,
    actor_ports: &[ActorPortRecord],
    ...
    {
        if !top_port_directions.contains_key(&port.signal_name) {
            continue;
        };
        ...
        let Some(width_hint) = port.width_hint.clone() else {
            continue;
        };
        let Some(width_evidence) = top_port_widths.get_mut(&port.signal_name) else {
            continue;
        };
        merge_top_port_width_evidence(
            width_evidence,
...
mod tests {
    ...
    fn register_renderable_signal_uses_graph_direction_without_stale_disagreement_entries() {
        ...
        let signals = [graph_only, flat_graph_disagreement];
    ...
    fn system_signal_renderability_uses_graph_direction_without_stale_flat_override() {
        ...
        let signals = [graph_clock, stale_reset];
        let signals_by_name = signals
            .iter()
            .map(|signal| (signal.signal_name.clone(), signal))
            .collect::<std::collections::BTreeMap<_, _>>();
    ...
    #[test]
    fn control_assignment_renderability_uses_graph_output_without_stale_flat_override() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
        ...
        stale_output.signal_name = "STALE_OUT".to_string();
        stale_output.width_hint = Some(8);
        ...
        let stale_action = ControlActionRecord::Assign {
            target: ControlAssignmentTargetRecord {
                signal_name: "STALE_OUT".to_string(),
                exposed_public_output: false,
            },
            assignment_kind: DecisionTreeAssignmentKind::Combinational,
            dual_output: None,
        ...
        assert!(!size_entries.contains_key("STALE_OUT"));
        assert!(!driven_outputs.contains("STALE_OUT"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_OUT` has conflicting canonical and graph-backed direction evidence",
            )
    ...
    #[test]
    fn registered_dual_output_assignment_renderability_uses_graph_output_without_stale_flat_override()
     {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
        ...
        graph_output.signal_name = "REG_DATA".to_string();
        graph_output.width_hint = Some(8);
        ...
        stale_output.signal_name = "STALE_REG".to_string();
        stale_output.width_hint = Some(8);
        ...
        let graph_action = ControlActionRecord::Assign {
            target: ControlAssignmentTargetRecord {
                signal_name: "REG_DATA".to_string(),
                exposed_public_output: false,
            },
            assignment_kind: DecisionTreeAssignmentKind::Sequential,
        ...
        let graph_entry = size_entries
            .get("REG_DATA")
            .expect("graph-backed registered dual-output target should create a size entry");
        assert_eq!(graph_entry.direction_hint, InterfaceSignalDirection::Output);
        assert_eq!(graph_entry.width, 8);
        assert!(driven_outputs.contains("REG_DATA"));
        assert!(sequential_targets.contains("REG_DATA"));
        assert!(blocking_reasons.is_empty());

        let stale_action = ControlActionRecord::Assign {
            target: ControlAssignmentTargetRecord {
                signal_name: "STALE_REG".to_string(),
                exposed_public_output: false,
            },
            assignment_kind: DecisionTreeAssignmentKind::Sequential,
            dual_output: Some(ControlDualOutputKind::RegisteredSignal),
        ...
        assert!(!size_entries.contains_key("STALE_REG"));
        assert!(!driven_outputs.contains("STALE_REG"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_REG` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(
            blocking_reasons
                .iter()
                .any(|reason| { reason.contains("Assignment target `STALE_REG` is not declared") })
        );
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
        ));
    ...
    #[test]
    fn dual_output_assignment_kind_blocks_nonsequential_graph_target() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
        ...
        graph_output.signal_name = "COMB_NEXT".to_string();
        graph_output.width_hint = Some(8);
        ...
        let graph_action = ControlActionRecord::Assign {
            target: ControlAssignmentTargetRecord {
                signal_name: "COMB_NEXT".to_string(),
                exposed_public_output: false,
            },
            assignment_kind: DecisionTreeAssignmentKind::Combinational,
            dual_output: Some(ControlDualOutputKind::NextSignal),
        ...
        let graph_entry = size_entries
            .get("COMB_NEXT")
            .expect("graph-backed dual-output target should still create a size entry");
        assert_eq!(graph_entry.direction_hint, InterfaceSignalDirection::Output);
        assert_eq!(graph_entry.width, 8);
        assert!(driven_outputs.contains("COMB_NEXT"));
        assert!(!sequential_targets.contains("COMB_NEXT"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Dual-output assignment lowering currently requires canonical sequential assignment intent",
            )
        ...
        assert!(required_canonical_enrichments.contains(
            "keep dual-output canonical assignments explicitly sequential in the active `.fsm` slice"
        ));
    }

    #[test]
    fn dual_output_assignment_values_use_graph_direction() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
        ...
        stale_value.signal_name = "STALE_NEXT_VALUE".to_string();
        ...
        let value_entry = size_entries
            .get("NEXT_VALUE")
            .expect("graph-backed dual-output value should create a size entry");
        ...
        let stale_action = ControlActionRecord::Assign {
            ...
            value: ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "STALE_NEXT_VALUE".to_string(),
                    kind_hint: ControlReferenceKind::Signal,
                    suffixes: Vec::new(),
                    exposed_public_output: false,
                },
        ...
        assert!(!size_entries.contains_key("STALE_NEXT_VALUE"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_NEXT_VALUE` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
    ...
    fn registered_dual_output_assignment_values_use_graph_direction() {
        ...
        graph_value.signal_name = "REG_VALUE".to_string();
        graph_value.width_hint = Some(8);
        ...
        stale_value.signal_name = "STALE_REG_VALUE".to_string();
        stale_value.width_hint = Some(8);
        ...
        let value_entry = size_entries
            .get("REG_VALUE")
            .expect("graph-backed registered dual-output value should create a size entry");
        assert_eq!(value_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(value_entry.width, 8);
        assert!(driven_outputs.contains("REG_DATA"));
        assert!(sequential_targets.contains("REG_DATA"));
        ...
        let stale_action = ControlActionRecord::Assign {
            ...
            value: ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "STALE_REG_VALUE".to_string(),
                    kind_hint: ControlReferenceKind::Signal,
                    suffixes: Vec::new(),
                    exposed_public_output: false,
                },
        ...
        assert!(!size_entries.contains_key("STALE_REG_VALUE"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_REG_VALUE` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
    ...
    #[test]
    fn registered_dual_output_assignment_value_width_cast_blocks_after_graph_signals() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
        ...
        assert_eq!(target_entry.width, 8);
        let value_entry = size_entries.get("REG_VALUE").expect(
            "graph-backed width-cast registered dual-output value should still create a size entry",
        );
        assert_eq!(value_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(value_entry.width, 8);
        assert!(driven_outputs.contains("REG_DATA"));
    ...
    #[test]
    fn control_assignment_value_width_cast_blocks_after_graph_signals() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
        ...
        let value_entry = size_entries
            .get("VALUE_IN")
            .expect("graph-backed width-cast assignment value should still create a size entry");
        assert_eq!(value_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(value_entry.width, 8);
        assert!(driven_outputs.contains("DATA_OUT"));
        assert!(!sequential_targets.contains("DATA_OUT"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "The active `.fsm` adapter slice does not yet lower width-cast signal references",
            )
    ...
    #[test]
    fn control_expression_renderability_uses_graph_direction_without_stale_flat_override() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        stale_input.signal_name = "STALE_DATA".to_string();
        stale_input.width_hint = Some(8);
        ...
        let graph_entry = size_entries
            .get("DATA_IN")
            .expect("graph-backed expression signal should create a size entry");
        assert_eq!(graph_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(graph_entry.width, 8);
        assert!(blocking_reasons.is_empty());

        let stale_expression = ControlExpressionRecord::Reference {
            reference: ControlReferenceRecord {
                base_name: "STALE_DATA".to_string(),
                kind_hint: ControlReferenceKind::Signal,
                suffixes: Vec::new(),
                exposed_public_output: false,
            },
        ...
        assert!(!size_entries.contains_key("STALE_DATA"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_DATA` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
    ...
    #[test]
    fn delayed_pulse_renderability_uses_graph_output_without_stale_flat_override() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
        ...
        stale_output.signal_name = "STALE_PULSE".to_string();
        stale_output.width_hint = Some(1);
        ...
        let graph_entry = size_entries
            .get("PULSE_OUT")
            .expect("graph-backed delayed-pulse target should create a size entry");
        assert_eq!(graph_entry.direction_hint, InterfaceSignalDirection::Output);
        assert_eq!(graph_entry.width, 1);
        ...
        let stale_action = ControlActionRecord::DelayedPulse {
            target: ControlAssignmentTargetRecord {
                signal_name: "STALE_PULSE".to_string(),
                exposed_public_output: false,
            },
            delay: 1,
            value: ControlExpressionRecord::Literal {
        ...
        assert!(!size_entries.contains_key("STALE_PULSE"));
        assert!(!driven_outputs.contains("STALE_PULSE"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_PULSE` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains("Delayed-pulse target `STALE_PULSE` is not declared")
        }));
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
        ));
    ...
    #[test]
    fn delayed_pulse_width_cast_value_keeps_graph_signal_references_signal_free() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
    ...
    fn compound_update_amount_renderability_uses_graph_direction_without_stale_flat_override() {
        ...
        let mut graph_amount = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
            false,
        );
        graph_amount.signal_name = "STEP_IN".to_string();
        graph_amount.width_hint = Some(8);

        let mut stale_amount = signal_candidate_with_direction_evidence(
            Some(InterfaceSignalDirection::Output),
            false,
            Some(InterfaceSignalDirection::Input),
            false,
        );
        stale_amount.signal_name = "STALE_STEP".to_string();
        stale_amount.width_hint = Some(8);

        let signals = [graph_output, graph_amount, stale_amount];
        let signals_by_name = signals
            .iter()
            .map(|signal| (signal.signal_name.clone(), signal))
            .collect::<std::collections::BTreeMap<_, _>>();
        ...
        let graph_action = ControlActionRecord::CompoundUpdate {
            ...
            operation: ControlCompoundUpdateOperation::Increment,
            amount: Some(ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "STEP_IN".to_string(),
                    kind_hint: ControlReferenceKind::Signal,
                    suffixes: Vec::new(),
                    exposed_public_output: false,
                },
        ...
        assert_eq!(target_entry.width, 8);
        let amount_entry = size_entries
            .get("STEP_IN")
            .expect("graph-backed compound-update amount should create a size entry");
        assert_eq!(amount_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(amount_entry.width, 8);
        assert!(driven_outputs.contains("COUNT"));
        assert!(sequential_targets.contains("COUNT"));
        assert!(blocking_reasons.is_empty());
        ...
        let stale_action = ControlActionRecord::CompoundUpdate {
            ...
            amount: Some(ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "STALE_STEP".to_string(),
                    kind_hint: ControlReferenceKind::Signal,
                    suffixes: Vec::new(),
                    exposed_public_output: false,
                },
        ...
        assert!(!size_entries.contains_key("STALE_STEP"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_STEP` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
    ...
    #[test]
    fn compound_update_amount_width_cast_blocks_after_graph_signals() {
        let mut graph_output = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Output),
        ...
        let signals = [graph_output, graph_amount];
        let signals_by_name = signals
            .iter()
            .map(|signal| (signal.signal_name.clone(), signal))
            .collect::<std::collections::BTreeMap<_, _>>();
        ...
        assert_eq!(target_entry.width, 8);
        let amount_entry = size_entries.get("STEP_IN").expect(
            "graph-backed width-cast compound-update amount should still create a size entry",
        );
        assert_eq!(amount_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(amount_entry.width, 8);
        assert!(driven_outputs.contains("COUNT"));
    ...
    #[test]
    fn transition_renderability_uses_graph_guard_direction_without_stale_flat_override() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        graph_input.signal_name = "GO".to_string();
        graph_input.width_hint = Some(1);
        ...
        stale_input.signal_name = "STALE_GO".to_string();
        stale_input.width_hint = Some(1);
        ...
        let signals_by_name = signals
            ...
            .map(|signal| (signal.signal_name.clone(), signal))
            .collect::<std::collections::BTreeMap<_, _>>();
        let state_names = ["idle".to_string(), "busy".to_string()]
            .into_iter()
        ...
        let graph_transition = FsmTransitionCandidate {
            transition_id: "transition_go".to_string(),
            source_state: "idle".to_string(),
            target_state: "busy".to_string(),
            guard: Some(DecisionTreeGuardRecord::SignalIsHigh {
        ...
        super::validate_transition_renderability(
            &graph_transition,
            &state_names,
            &signals_by_name,
            &mut size_entries,
        ...
        let graph_entry = size_entries
            .get("GO")
            .expect("graph-backed transition guard should create a size entry");
        assert_eq!(graph_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(graph_entry.width, 1);
        assert!(blocking_reasons.is_empty());

        let stale_transition = FsmTransitionCandidate {
            transition_id: "transition_stale".to_string(),
            source_state: "idle".to_string(),
            target_state: "busy".to_string(),
            guard: Some(DecisionTreeGuardRecord::SignalIsHigh {
                signal_name: "STALE_GO".to_string(),
            }),
            declaration_order: 1,
            supporting_canonical_ids: Vec::new(),
            automation_confidence: AutomationConfidence::High,
        ...
        assert!(!size_entries.contains_key("STALE_GO"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
    ...
    #[test]
    fn selector_width_cast_blocks_after_graph_signal() {
        let mut graph_selector = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let selector_expr = ControlExpressionRecord::Reference {
            reference: ControlReferenceRecord {
                base_name: "SEL".to_string(),
                kind_hint: ControlReferenceKind::Signal,
                suffixes: vec![ControlReferenceSuffix::WidthCast { width: 1 }],
        ...
        let graph_block = ControlBlockRecord {
            block_id: "selector_width_cast_block".to_string(),
            block_name: "selector_width_cast_block".to_string(),
            role: ControlBlockRole::StandaloneDecisionTree,
            declaration_order: 0,
            selector: Some(selector_expr.clone()),
            branches: vec![ControlBranchRecord {
                branch_id: "selector_width_cast_branch".to_string(),
                declaration_order: 0,
                predicate: Some(ControlExpressionRecord::Binary {
                    operator: ControlBinaryOperator::Eq,
                    left: Box::new(selector_expr),
                    right: Box::new(ControlExpressionRecord::Literal {
                        literal: "1'd1".to_string(),
                    }),
                }),
                actions: vec![ControlActionRecord::Transition {
                    target_state: "busy".to_string(),
        ...
        let graph_entry = size_entries
            .get("SEL")
            .expect("graph-backed width-cast selector should still create a size entry");
        assert_eq!(graph_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(graph_entry.width, 2);
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
        ...
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains("selector `SEL'1` that the active `.fsm` test-node slice cannot encode")
        }));
        assert!(
            required_canonical_enrichments
                .contains("add width-cast reference lowering from canonical control expressions")
        ...
        assert!(required_canonical_enrichments.contains(
            "keep selector-bearing canonical control blocks on plain signal selectors or explicit computed unary/binary selector expressions"
        ));
    }
    ...
    fn comparison_guard_renderability_uses_graph_direction_without_stale_flat_override() {
        ...
        graph_left.signal_name = "MATCH".to_string();
        graph_left.width_hint = Some(2);
        ...
        graph_right.signal_name = "EXPECTED".to_string();
        graph_right.width_hint = Some(2);

        let mut stale_right = signal_candidate_with_direction_evidence(
            Some(InterfaceSignalDirection::Output),
            false,
            Some(InterfaceSignalDirection::Input),
            false,
        );
        stale_right.signal_name = "STALE_EXPECTED".to_string();
        stale_right.width_hint = Some(2);

        let signals = [graph_left, graph_right, stale_right];
        let signals_by_name = signals
            .iter()
            .map(|signal| (signal.signal_name.clone(), signal))
            .collect::<std::collections::BTreeMap<_, _>>();
        ...
        let graph_guard = DecisionTreeGuardRecord::Comparison {
            left_signal: "MATCH".to_string(),
            operator: DecisionTreeComparisonOperator::Eq,
            right: DecisionTreeValueRecord::SignalRef {
                signal_name: "EXPECTED".to_string(),
            },
        };
        super::validate_guard_renderability(
            &graph_guard,
        ...
        let left_entry = size_entries
            .get("MATCH")
            .expect("graph-backed comparison lhs should create a size entry");
        let right_entry = size_entries
            .get("EXPECTED")
            .expect("graph-backed comparison rhs should create a size entry");
        assert_eq!(left_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(left_entry.width, 2);
        assert_eq!(right_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(right_entry.width, 2);
        assert!(blocking_reasons.is_empty());

        let stale_guard = DecisionTreeGuardRecord::Comparison {
            left_signal: "MATCH".to_string(),
            operator: DecisionTreeComparisonOperator::Eq,
            right: DecisionTreeValueRecord::SignalRef {
                signal_name: "STALE_EXPECTED".to_string(),
            },
        };
        super::validate_guard_renderability(
            &stale_guard,
        ...
        assert!(!size_entries.contains_key("STALE_EXPECTED"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_EXPECTED` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
    ...
    #[test]
    fn symbol_definition_renderability_keeps_graph_signal_references_signal_free() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "symbol_from_signal".to_string(),
            symbol_name: "SYMBOL_FROM_SIGNAL".to_string(),
            kind: SymbolDefinitionKind::Constant,
            value: Some(ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "DATA_IN".to_string(),
                    kind_hint: ControlReferenceKind::Signal,
                    suffixes: Vec::new(),
        ...
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `DATA_IN` currently resolves through signal `DATA_IN`",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "keep emitted `.fsm` symbol-definition values signal-free in the active slice"
    ...
    #[test]
    fn symbol_definition_width_cast_keeps_graph_signal_references_signal_free() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "width_cast_symbol_from_signal".to_string(),
            symbol_name: "WIDTH_CAST_SYMBOL_FROM_SIGNAL".to_string(),
            kind: SymbolDefinitionKind::Constant,
            value: Some(ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "DATA_IN".to_string(),
        ...
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `DATA_IN'4` currently resolves through signal `DATA_IN`",
            )
        }));
        assert!(
            required_canonical_enrichments
    ...
    #[test]
    fn unary_symbol_definition_renderability_keeps_graph_signal_references_signal_free() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "unary_symbol_from_signal".to_string(),
            symbol_name: "INVERTED_SYMBOL_FROM_SIGNAL".to_string(),
            kind: SymbolDefinitionKind::Constant,
            value: Some(ControlExpressionRecord::Unary {
                operator: ControlUnaryOperator::Not,
    ...
    #[test]
    fn unary_symbol_definition_width_cast_keeps_graph_signal_references_signal_free() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "unary_width_cast_symbol_from_signal".to_string(),
            symbol_name: "INVERTED_WIDTH_CAST_SYMBOL_FROM_SIGNAL".to_string(),
            kind: SymbolDefinitionKind::Constant,
            value: Some(ControlExpressionRecord::Unary {
                operator: ControlUnaryOperator::Not,
                operand: Box::new(ControlExpressionRecord::Reference {
    ...
    #[test]
    fn binary_symbol_definition_width_cast_keeps_graph_signal_references_signal_free() {
        let mut graph_left = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "binary_width_cast_symbol_from_signals".to_string(),
            symbol_name: "COMBINED_WIDTH_CAST_SYMBOL_FROM_SIGNALS".to_string(),
            kind: SymbolDefinitionKind::Constant,
            value: Some(ControlExpressionRecord::Binary {
                operator: ControlBinaryOperator::Add,
                left: Box::new(ControlExpressionRecord::Reference {
        ...
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `LEFT_IN'4` currently resolves through signal `LEFT_IN`",
            )
        }));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `RIGHT_IN'4` currently resolves through signal `RIGHT_IN`",
            )
        }));
        assert!(
            required_canonical_enrichments
    ...
    fn enum_symbol_renderability_keeps_graph_signal_references_signal_free() {
        ...
        graph_input.signal_name = "STATE_IN".to_string();
        graph_input.width_hint = Some(2);
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "enum_from_signal".to_string(),
            symbol_name: "STATE_ENUM".to_string(),
            kind: SymbolDefinitionKind::Enum,
            value: None,
            members: vec![SymbolEnumMemberRecord {
                member_name: "STATE_FROM_SIGNAL".to_string(),
                value: ControlExpressionRecord::Reference {
                    reference: ControlReferenceRecord {
                        base_name: "STATE_IN".to_string(),
                        kind_hint: ControlReferenceKind::Signal,
                        suffixes: Vec::new(),
                        exposed_public_output: false,
                    },
        ...
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `STATE_IN` currently resolves through signal `STATE_IN`",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "keep emitted `.fsm` symbol-definition values signal-free in the active slice"
    ...
    #[test]
    fn enum_symbol_width_cast_renderability_keeps_graph_signal_references_signal_free() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "enum_width_cast_from_signal".to_string(),
            symbol_name: "STATE_ENUM".to_string(),
            kind: SymbolDefinitionKind::Enum,
            value: None,
            members: vec![SymbolEnumMemberRecord {
                member_name: "STATE_WIDTH_CAST_FROM_SIGNAL".to_string(),
                value: ControlExpressionRecord::Reference {
                    reference: ControlReferenceRecord {
                        base_name: "STATE_IN".to_string(),
                        kind_hint: ControlReferenceKind::Signal,
        ...
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `STATE_IN'2` currently resolves through signal `STATE_IN`",
            )
        }));
        assert!(
            required_canonical_enrichments
    ...
    #[test]
    fn unary_enum_symbol_renderability_keeps_graph_signal_references_signal_free() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "unary_enum_from_signal".to_string(),
            symbol_name: "STATE_ENUM".to_string(),
            kind: SymbolDefinitionKind::Enum,
            value: None,
            members: vec![SymbolEnumMemberRecord {
                member_name: "INVERTED_STATE_FROM_SIGNAL".to_string(),
                value: ControlExpressionRecord::Unary {
                    operator: ControlUnaryOperator::Not,
                    operand: Box::new(ControlExpressionRecord::Reference {
                        reference: ControlReferenceRecord {
                            base_name: "STATE_IN".to_string(),
    ...
    #[test]
    fn unary_enum_symbol_width_cast_renderability_keeps_graph_signal_references_signal_free() {
        let mut graph_input = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "unary_enum_width_cast_from_signal".to_string(),
            symbol_name: "STATE_ENUM".to_string(),
            kind: SymbolDefinitionKind::Enum,
            value: None,
            members: vec![SymbolEnumMemberRecord {
                member_name: "INVERTED_WIDTH_CAST_STATE_FROM_SIGNAL".to_string(),
                value: ControlExpressionRecord::Unary {
                    operator: ControlUnaryOperator::Not,
                    operand: Box::new(ControlExpressionRecord::Reference {
                        reference: ControlReferenceRecord {
    ...
    #[test]
    fn binary_enum_symbol_renderability_keeps_graph_signal_references_signal_free() {
        let mut graph_left = signal_candidate_with_direction_evidence(
            None,
            false,
            Some(InterfaceSignalDirection::Input),
        ...
        let definition = SymbolDefinitionRecord {
            symbol_id: "binary_enum_from_signal".to_string(),
            symbol_name: "STATE_ENUM".to_string(),
            kind: SymbolDefinitionKind::Enum,
            value: None,
            members: vec![SymbolEnumMemberRecord {
                member_name: "COMBINED_STATE_FROM_SIGNALS".to_string(),
        ...
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `STATE_LEFT` currently resolves through signal `STATE_LEFT`",
            )
        }));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Symbol-definition expression `STATE_RIGHT` currently resolves through signal `STATE_RIGHT`",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "keep emitted `.fsm` symbol-definition values signal-free in the active slice"
    ...
    fn unary_expression_renderability_uses_graph_direction_without_stale_flat_override() {
        ...
        stale_input.signal_name = "STALE_FLAG".to_string();
        stale_input.width_hint = Some(1);
        ...
        let graph_entry = size_entries
            .get("FLAG")
            .expect("graph-backed unary operand should create a size entry");
        assert_eq!(graph_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(graph_entry.width, 1);
        assert!(blocking_reasons.is_empty());

        let stale_expression = ControlExpressionRecord::Unary {
            operator: ControlUnaryOperator::Not,
            operand: Box::new(ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "STALE_FLAG".to_string(),
                    kind_hint: ControlReferenceKind::Signal,
                    suffixes: Vec::new(),
                    exposed_public_output: false,
                },
        ...
        assert!(!size_entries.contains_key("STALE_FLAG"));
        assert!(blocking_reasons.iter().any(|reason| {
            reason.contains(
                "Signal `STALE_FLAG` has conflicting canonical and graph-backed direction evidence",
            )
        }));
        assert!(required_canonical_enrichments.contains(
            "resolve conflicting canonical and actor-relative graph direction evidence before lowering `.fsm`"
    ...
    fn binary_expression_renderability_uses_graph_direction_without_stale_flat_override() {
        ...
        let signals = [graph_left, graph_right, stale_input];
        let signals_by_name = signals
            .iter()
            .map(|signal| (signal.signal_name.clone(), signal))
            .collect::<std::collections::BTreeMap<_, _>>();
        ...
        let left_entry = size_entries
            .get("LEFT_FLAG")
            .expect("graph-backed binary left operand should create a size entry");
        assert_eq!(left_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(left_entry.width, 1);
        let right_entry = size_entries
            .get("RIGHT_FLAG")
            .expect("graph-backed binary right operand should create a size entry");
        assert_eq!(right_entry.direction_hint, InterfaceSignalDirection::Input);
        assert_eq!(right_entry.width, 1);
        assert!(blocking_reasons.is_empty());

        let stale_expression = ControlExpressionRecord::Binary {
            operator: ControlBinaryOperator::BitAnd,
            left: Box::new(ControlExpressionRecord::Reference {
                reference: ControlReferenceRecord {
                    base_name: "LEFT_FLAG".to_string(),
    ...
    fn clear_explicit_module_signal_shape_hints(intent_ir: &mut IntentIr, signal_names: &[&str]) {
        for module in &mut intent_ir.explicit_modules {
            for interface in &mut module.interfaces {
                for signal in &mut interface.signal_records {
                    if signal_names.contains(&signal.signal_name.as_str()) {
                        signal.direction_hint = None;
                        signal.width_hint = None;
                    }
                }
            }
        }
    ...
    fn clear_direct_signal_shape_hints(intent_ir: &mut IntentIr, signal_names: &[&str]) {
        for interface in &mut intent_ir.interfaces {
            for signal in &mut interface.signal_records {
                if signal_names.contains(&signal.signal_name.as_str()) {
                    signal.direction_hint = None;
    ...
    fn standalone_dt_blocks_graph_backed_undriven_output_inventory() -> Result<()> {
        ...
        let signal_inventory_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_signal_inventory")
            .expect("undriven graph-backed outputs should retain signal inventory guidance");
        assert_eq!(
            signal_inventory_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_dt_keeps_conflicting_actor_port_direction_unresolved() -> Result<()> {
        ...
        let signal_inventory_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_signal_inventory")
            .expect("actor-port direction conflicts should retain signal inventory guidance");
        assert_eq!(
            signal_inventory_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_dt_blocks_flat_graph_direction_disagreement() -> Result<()> {
        ...
        let signal_inventory_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_signal_inventory")
            .expect("flat/graph direction disagreements should retain signal inventory guidance");
        assert_eq!(
            signal_inventory_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_sequential_dt_recovers_system_signals_from_system_contract() -> Result<()> {
        ...
        let emitted_target_path = adapter
            ...
            .emitted_target_path
            .as_ref()
            .expect("system-contract-backed sequential adapter should emit target text");
        let emitted_text = fs::read_to_string(emitted_target_path)?;
        let fsm = adapter.fsm.expect("fsm artifact should be present");
        let clk = fsm
            .signal_inventory
    ...
    fn standalone_sequential_dt_blocks_conflicting_system_contract_signal_direction() -> Result<()>
    {
        ...
        let system_contract_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_system_contract")
            .expect("system contract direction conflict should surface system guidance");
        assert_eq!(
            system_contract_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_sequential_dt_blocks_conflicting_system_contract_signal_width() -> Result<()> {
        ...
        let system_contract_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_system_contract")
            .expect("system contract width conflict should surface system guidance");
        assert_eq!(
            system_contract_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_dt_blocks_parametric_actor_port_width_with_diagnostic() -> Result<()> {
        ...
        assert!(
            data_in
                .supporting_canonical_ids
                .iter()
                .any(|id| id == "graph_controller_DATA_IN")
        );
        assert_eq!(data_in.automation_confidence, AutomationConfidence::High);
        assert!(!data_in.width_hint_conflicted);
        assert!(!fsm.renderability.is_renderable);
    ...
    fn structured_fsm_blocks_flat_graph_direction_disagreement() -> Result<()> {
        ...
        let signal_inventory_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_signal_inventory")
            .expect("structured flat/graph conflict should surface signal-inventory guidance");
        assert_eq!(
            signal_inventory_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_explicit_module_recovers_inputs_from_module_control_reads() -> Result<()> {
        ...
        let renderable_module = module
            .renderable_module
            .as_ref()
            .expect("graph-backed explicit module should produce a renderable module");
        for (signal_name, width) in [("DATA_IN", 8), ("GO", 1), ("DONE", 1)] {
            let size_entry = renderable_module
                .size_entries
                .iter()
    ...
    #[test]
    fn standalone_explicit_module_recovers_control_input_width_from_actor_port_graph() -> Result<()>
    {
        let tempdir = tempdir()?;
        let mut intent_ir = build_standalone_explicit_module_fsm_intent_ir(tempdir.path())?;
        clear_explicit_module_signal_shape_hints(&mut intent_ir, &["DATA_IN"]);
    ...
    #[test]
    fn standalone_explicit_module_blocks_conflicting_control_input_actor_port_widths() -> Result<()>
    {
        let tempdir = tempdir()?;
        let mut intent_ir = build_standalone_explicit_module_fsm_intent_ir(tempdir.path())?;
        clear_explicit_module_signal_shape_hints(&mut intent_ir, &["DATA_IN"]);
        intent_ir.actor_ports = vec![
            ...
            actor_port_with_numeric_width("monitor", "DATA_IN", ActorRelativeDirection::Input, 16),
        ];
        intent_ir.write_to_disk()?;
        ...
        let signal_inventory_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_signal_inventory")
            .expect("explicit module width conflict should surface signal guidance");
        assert_eq!(
            signal_inventory_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_explicit_module_blocks_flat_graph_direction_disagreement() -> Result<()> {
        ...
        let signal_inventory_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_signal_inventory")
            .expect("explicit module flat/graph conflict should surface signal guidance");
        assert_eq!(
            signal_inventory_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn standalone_explicit_module_recovers_system_signals_from_system_contract() -> Result<()> {
        ...
        let clk = module
            ...
            .iter()
            .find(|signal| signal.signal_name == "clk")
            .expect("clock should remain in the module inventory");
        let rst_n = module
            .signal_inventory
            .iter()
            .find(|signal| signal.signal_name == "rst_n")
    ...
    #[test]
    fn renderable_top_document_preserves_mixed_child_root_order_and_kind() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
        ...
        let child_kinds = renderable_top
            .children
            .iter()
            .map(|child| (child.instance_name.as_str(), child.child_root_kind))
            .collect::<Vec<_>>();
        let direct_root_kinds = renderable_document
            .direct_roots
            .iter()
            .map(|root| (root.module_name.as_str(), root.root_kind))
            .collect::<Vec<_>>();

        assert_eq!(
            child_kinds,
            vec![
                ("producer", FsmRootKind::Dt),
                ("controller", FsmRootKind::Fsm)
            ]
        );
        assert_eq!(
            direct_root_kinds,
        ...
        let top_index = emitted_text
            .find("(?top:wrapper")
            .expect("emitted text should contain top root");
        let producer_root_index = emitted_text
            .find("(?dt:producer_core")
            .expect("emitted text should contain producer direct root");
        let controller_root_index = emitted_text
            .find("(?fsm:controller_core")
            .expect("emitted text should contain controller direct root");
        assert!(top_index < producer_root_index);
        assert!(producer_root_index < controller_root_index);
        assert!(emitted_text.contains("(?dtc:producer producer_core)"));
        assert!(emitted_text.contains("(?fsmc:controller controller_core)"));
    ...
    #[test]
    fn top_composition_recovers_top_port_width_from_actor_ports() -> Result<()> {
        let tempdir = tempdir()?;
        let mut intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
    ...
    fn top_root_kind_confidence_follows_top_link_evidence() -> Result<()> {
        ...
        let (top_port_support_ids, link_support_ids) = {
            ...
            assert!(
                !raw_link.supporting_statement_ids.is_empty(),
        ...
        let renderable_link = renderable_top
            ...
            .expect("child-to-child top link should be present in renderable top root");
        ...
        assert!(
            link_support_ids
                .iter()
                .any(|id| recovered_link.supporting_statement_ids.contains(id))
        );
        assert_eq!(
            renderable_link.automation_confidence,
            AutomationConfidence::High
        );
        assert!(
            link_support_ids
    ...
    fn top_composition_blocks_link_from_unemitted_child_port() -> Result<()> {
        ...
        let (top_port_support_ids, child_support_ids, top_link_support_ids) = intent_ir
            .explicit_tops
            .iter()
            .find(|top| top.top_name == "datapath")
            .and_then(|top| {
                let result_port = top
                    ...
                    .iter()
                    .find(|port| port.port_name == "result_data")?;
                let producer_child = top
                    .children
                    .iter()
                    .find(|child| child.instance_name == "producer")?;
                let blocked_link = top.links.iter().find(|link| {
                    link.source.instance_name.as_deref() == Some("producer")
                ...
                Some((
                    ...
                    producer_child.supporting_statement_ids.clone(),
                    super::explicit_top_link_supporting_ids(blocked_link),
                ))
            })
    ...
    fn top_composition_blocks_conflicting_top_actor_port_direction() -> Result<()> {
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("top actor-port direction conflict should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    #[test]
    fn top_composition_blocks_conflicting_top_actor_port_width() -> Result<()> {
        let tempdir = tempdir()?;
        let mut intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
            "conflicting_top_actor_port_width.md",
            "# Conflicting Top Actor Port Width\nTop wrapper.\n\nTop wrapper port ext_data is output width 8.\n\nTop wrapper child producer uses module producer_core.\n\nModule producer_core signal output_data is output width 8.\n\nModule producer_core block produce: output_data = 8'3.\n",
        )?;
        intent_ir.actor_ports = vec![actor_port_with_numeric_width(
            "wrapper",
            "ext_data",
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("top actor-port width conflict should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn top_composition_keeps_conflicting_top_port_direction_unresolved() -> Result<()> {
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("blocked top-link role mismatch should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn top_composition_keeps_duplicate_top_port_direction_conflict_unresolved() -> Result<()> {
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("duplicate top-port direction blocker should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn top_composition_recovers_child_directions_from_actor_ports() -> Result<()> {
        ...
        for (module, signal_name, direction_hint) in [
            ...
            (consumer, "result_data", InterfaceSignalDirection::Output),
        ] {
            let renderable_module = module
                .renderable_module
                .as_ref()
    ...
    fn top_composition_recovers_child_width_from_top_link_topology() -> Result<()> {
        ...
        let renderable_module = producer
            .renderable_module
            .as_ref()
            .expect("top-link-width-backed producer should be renderable");
    ...
    #[test]
    fn top_composition_recovers_child_width_from_sibling_child_link_topology() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
        ...
        let emitted_target_path = adapter
            ...
            .emitted_target_path
            .as_ref()
            .expect("child-child-width-backed top adapter should emit target text");
        let emitted_text = fs::read_to_string(emitted_target_path)?;
        let fsm = adapter.fsm.expect("fsm artifact should be present");
        let consumer = fsm
            .module_candidates
    ...
    fn top_composition_recovers_child_width_through_transitive_topology() -> Result<()> {
        ...
        let (
            ...
            let producer_to_consumer_link = explicit_top
                ...
                .expect("producer to consumer topology link should be present");
            let producer_to_top_link = explicit_top
                .links
                .iter()
                .find(|link| {
                    link.source.instance_name.as_deref() == Some("producer")
    ...
    fn top_composition_blocks_conflicting_top_port_widths_from_child_links() -> Result<()> {
        ...
        let (top_port_support_ids, child_support_id_sets, child_link_support_id_sets) = {
            let explicit_top = intent_ir
                .explicit_tops
                .iter()
                .find(|top| top.top_name == "datapath")
            ...
            let child_support_id_sets = explicit_top
                .children
                .iter()
                .filter(|child| ["producer8", "producer16"].contains(&child.instance_name.as_str()))
                .map(|child| {
                    (
                        child.instance_name.clone(),
                        child.source_module_name.clone(),
            ...
            assert!(
                child_support_id_sets
                    .iter()
                    .all(|(_, _, support_ids)| !support_ids.is_empty()),
                "conflicting child declarations should have provenance"
            );
            let link_support_id_sets = explicit_top
                .links
                .iter()
        ...
        for support_ids in &child_link_support_id_sets {
            assert!(child_links.iter().any(|link| {
                support_ids
                    .iter()
                    .any(|id| super::explicit_top_link_supporting_ids(link).contains(id))
            }));
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("top child-link width conflict should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn top_composition_blocks_conflicting_child_topology_widths() -> Result<()> {
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("child topology width conflict should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn top_composition_blocks_conflicting_child_link_topology_directions() -> Result<()> {
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("child topology direction conflict should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    fn top_composition_blocks_conflicting_actor_port_directions() -> Result<()> {
        let tempdir = tempdir()?;
        let mut intent_ir = build_explicit_top_composition_intent_ir(tempdir.path())?;
        let (top_port_support_ids, child_support_ids, link_support_ids, output_data_support_ids) = {
            let explicit_top = intent_ir
                .explicit_tops
                .iter()
                .find(|top| top.top_name == "datapath")
            ...
            let output_link = explicit_top
                ...
                .expect("producer to consumer output link should be present");
            ...
            (
                result_port.supporting_statement_ids.clone(),
                producer_child.supporting_statement_ids.clone(),
                super::explicit_top_link_supporting_ids(output_link),
                output_data_support_ids,
            )
        };
        assert!(
            !top_port_support_ids.is_empty()
                ...
                && !output_data_support_ids.is_empty(),
            "top port, child, link, and child output signal provenance should be present"
        );
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("child actor-port direction conflict should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    #[test]
    fn top_composition_blocks_multi_child_without_links_guidance() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
        ...
        assert!(
            top_candidate
                ...
                .iter()
                .any(|reason| reason
                    .contains("Multi-child top-root lowering requires explicit link records"))
        );
        assert!(
            top_candidate
                .renderability
    ...
    fn top_composition_blocks_top_without_child_guidance() -> Result<()> {
        ...
        let signal_inventory_port = fsm
            ...
            .iter()
            .find(|signal| signal.signal_name == "result_data")
            .expect("top output port should remain in selected top inventory");
        assert_eq!(
            signal_inventory_port.direction_hint,
            Some(InterfaceSignalDirection::Output)
        );
    ...
    #[test]
    fn top_composition_blocks_top_without_port_guidance() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
            "top_without_port.md",
        ...
        let child_support_ids = {
            let explicit_top = intent_ir
                ...
                .expect("explicit top should be present");
            assert!(
                explicit_top.ports.is_empty(),
                "fixture should expose the no-top-port blocker"
            );
            explicit_top
                .children
                .iter()
        ...
        let top_candidate = fsm
            ...
            .find(|top| top.top_name == "wrapper")
            .expect("top candidate should remain visible");
        assert!(top_candidate.ports.is_empty());
        assert_eq!(top_candidate.children.len(), 1);
        let child = top_candidate
            .children
            .iter()
            .find(|child| child.instance_name == "producer")
            .expect("producer child candidate should remain visible");
        assert_eq!(child.source_module_name, "producer_core");
        ...
        assert!(
            top_candidate
                ...
                .blocking_reasons
                .iter()
                .any(|reason| reason.contains("at least one explicit top port record"))
        );
        assert!(
            top_candidate
                .renderability
                ...
                .iter()
                .any(|enrichment| enrichment
                    == "carry explicit top-port records before lowering `?top:name`")
        );
        assert!(top_candidate.renderable_top.is_none());
        assert!(!fsm.renderability.is_renderable);
        assert!(fsm.renderable_document.is_none());
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("top without top ports should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    #[test]
    fn top_composition_blocks_link_to_undeclared_top_target_guidance() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
            "link_to_undeclared_top_target.md",
            "# Link To Undeclared Top Target\nTop datapath.\n\nTop datapath port status is output width 1.\n\nTop datapath child producer uses module producer_core.\n\nTop datapath link producer.output_data -> result_data.\n\nModule producer_core signal output_data is output width 8.\n\nModule producer_core block produce: output_data = 8'3.\n",
        )?;
        let (top_port_support_ids, child_support_ids, link_support_ids) = {
            ...
            assert!(
                explicit_top
                    .ports
                    .iter()
                    .all(|port| port.port_name != "result_data"),
                "fixture should leave the top-link target undeclared"
            );
            let status_port = explicit_top
                .ports
                .iter()
                .find(|port| port.port_name == "status")
                .expect("declared status top port should be present");
            let producer_child = explicit_top
                .children
                .iter()
                .find(|child| child.instance_name == "producer")
            ...
            let output_link = explicit_top
                ...
                .expect("producer to missing top target link should be present");
            (
                status_port.supporting_statement_ids.clone(),
                producer_child.supporting_statement_ids.clone(),
                super::explicit_top_link_supporting_ids(output_link),
            )
        };
        ...
        let top_candidate = fsm
            ...
            .find(|top| top.top_name == "datapath")
            .expect("top candidate should remain visible");
        let status_port = top_candidate
            .ports
            .iter()
            .find(|port| port.port_name == "status")
            .expect("declared status top port should remain visible");
        assert_eq!(
            status_port.direction_hint,
            Some(InterfaceSignalDirection::Output)
        );
        assert_eq!(status_port.width_hint, Some(WidthHint::Numeric(1)));
        assert!(
            top_port_support_ids
                .iter()
                .any(|id| status_port.supporting_statement_ids.contains(id))
        );
        assert_eq!(
            status_port.automation_confidence,
            AutomationConfidence::High
        );
        let signal_inventory_port = fsm
            .signal_inventory
            .iter()
            .find(|signal| signal.signal_name == "status")
            .expect("declared status top port should remain in selected top inventory");
        assert_eq!(
            signal_inventory_port.direction_hint,
            Some(InterfaceSignalDirection::Output)
        );
        ...
        assert!(
            top_candidate
                ...
                .iter()
                .any(|reason| reason.contains(
                    "Top link target `result_data` does not resolve to an explicit top port"
                ))
        );
        assert!(
            top_candidate
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("blocked undeclared top target should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
    ...
    #[test]
    fn top_composition_blocks_link_from_unemitted_child_source_guidance() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
            "link_from_unemitted_child_source.md",
            "# Link From Unemitted Child Source\nTop datapath.\n\nTop datapath port result_data is output width 8.\n\nTop datapath child producer uses module producer_core.\n\nTop datapath link producer.side_data -> result_data.\n\nModule producer_core signal output_data is output width 8.\n\nModule producer_core signal side_data is output width 8.\n\nModule producer_core block produce: output_data = 8'3.\n",
        )?;
        let (top_port_support_ids, producer_child_support_ids, link_support_ids) = {
            ...
            let producer_child = explicit_top
                ...
                .find(|child| child.instance_name == "producer")
                .expect("producer child should be present");
            let unemitted_source_link = explicit_top
                .links
                .iter()
                .find(|link| {
                    link.source.instance_name.as_deref() == Some("producer")
            ...
            (
                result_port.supporting_statement_ids.clone(),
                producer_child.supporting_statement_ids.clone(),
                super::explicit_top_link_supporting_ids(unemitted_source_link),
            )
        };
        assert!(
            !top_port_support_ids.is_empty()
    ...
    #[test]
    fn top_composition_blocks_child_target_direction_role_guidance() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
            "child_target_direction_role.md",
            "# Child Target Direction Role\nTop datapath.\n\nTop datapath port drive_data is input width 8.\n\nTop datapath child consumer uses module consumer_core.\n\nTop datapath link drive_data -> consumer.result_data.\n\nModule consumer_core signal input_data is input width 8.\n\nModule consumer_core signal result_data is output width 8.\n\nModule consumer_core block route: result_data = input_data.\n",
        )?;
        let (top_port_support_ids, consumer_child_support_ids, link_support_ids) = {
            let explicit_top = intent_ir
                .explicit_tops
                .iter()
                .find(|top| top.top_name == "datapath")
            ...
            let drive_port = explicit_top
                ...
                .iter()
                .find(|port| port.port_name == "drive_data")
                .expect("declared drive_data top port should be present");
            let consumer_child = explicit_top
                .children
                .iter()
                .find(|child| child.instance_name == "consumer")
            ...
            let role_mismatch_link = explicit_top
                ...
                .expect("top drive_data to consumer result_data link should be present");
            (
                drive_port.supporting_statement_ids.clone(),
                consumer_child.supporting_statement_ids.clone(),
                super::explicit_top_link_supporting_ids(role_mismatch_link),
        ...
        assert!(!result_data.graph_direction_hint_conflicted);
        assert!(
            result_data
                .mention_categories
                .iter()
        ...
        assert!(
            consumer
                ...
                .iter()
                .any(|reason| reason
                    .contains("conflicting canonical and graph-backed direction evidence"))
        );
        assert!(
            consumer
                .renderability
        ...
        let drive_port = top_candidate
            ...
            .iter()
            .find(|port| port.port_name == "drive_data")
            .expect("declared drive_data top port should remain visible");
        assert_eq!(
            drive_port.direction_hint,
            Some(InterfaceSignalDirection::Input)
        );
        ...
        assert!(
            top_candidate
                ...
                .iter()
                .any(|reason| reason.contains(
                    "Top link target `consumer.result_data` does not resolve to an emitted child port"
                ))
        );
        assert!(
            top_candidate
    ...
    #[test]
    fn top_composition_blocks_link_to_unemitted_child_target_guidance() -> Result<()> {
        let tempdir = tempdir()?;
        let intent_ir = build_intent_ir_from_markdown(
            tempdir.path(),
            "link_to_unemitted_child_target.md",
            "# Link To Unemitted Child Target\nTop datapath.\n\nTop datapath port result_data is output width 8.\n\nTop datapath child producer uses module producer_core.\n\nTop datapath child consumer uses module consumer_core.\n\nTop datapath link producer.output_data -> consumer.missing_input.\n\nModule producer_core signal output_data is output width 8.\n\nModule producer_core block produce: output_data = 8'3.\n\nModule consumer_core signal input_data is input width 8.\n\nModule consumer_core signal result_data is output width 8.\n\nModule consumer_core block route: result_data = input_data.\n",
        )?;
        let (top_port_support_ids, consumer_child_support_ids, link_support_ids) = {
            let explicit_top = intent_ir
                .explicit_tops
            ...
            let consumer_child = explicit_top
                ...
                .find(|child| child.instance_name == "consumer")
                .expect("consumer child should be present");
            let missing_target_link = explicit_top
                .links
                .iter()
                .find(|link| {
                    link.source.instance_name.as_deref() == Some("producer")
                        && link.source.signal_name == "output_data"
                        && link.target.instance_name.as_deref() == Some("consumer")
                        && link.target.signal_name == "missing_input"
                })
                .expect("producer to missing consumer input link should be present");
            (
                result_port.supporting_statement_ids.clone(),
                consumer_child.supporting_statement_ids.clone(),
                super::explicit_top_link_supporting_ids(missing_target_link),
            )
        };
        assert!(
            !top_port_support_ids.is_empty()
        ...
        assert!(
            top_candidate
                ...
                .iter()
                .any(|reason| reason.contains(
                    "Top link target `consumer.missing_input` does not resolve to an emitted child port"
                ))
        );
        assert!(
            top_candidate
        ...
        let composition_residual = adapter
            ...
            .iter()
            .find(|packet| packet.packet_id == "fsm_adapter_composition_topology")
            .expect("blocked unemitted child target should surface a composition residual");
        assert_eq!(
            composition_residual.automation_confidence,
            AutomationConfidence::Low
        );
