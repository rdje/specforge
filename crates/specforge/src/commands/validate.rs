...
pub fn run(args: ValidateArgs) -> Result<()> {
    ...
    match probe.stage {
        ...
        IrStage::FsmAdapter => {
            ...
            let report = validate_fsm_adapter(&artifact, fsm_adapter_fingerprint(&artifact)?);
...
fn persist_fsm_adapter_validation(
...
fn validate_fsm_adapter(
    artifact: &AdapterArtifact,
    artifact_fingerprint: String,
) -> ValidationReportRecord {
    println!("command: validate");
    println!("stage: fsm_adapter");
    println!("document_key: {}", artifact.document_identity.document_key);
    println!();

    let fsm = artifact.fsm.as_ref();
    let fsm_absent = fsm.is_none();

    let schema_ok = artifact.schema_version == 1;
    let is_renderable = fsm.is_some_and(|f| f.renderability.is_renderable);
    let blocking_reasons: &[String] = fsm
        .map(|f| f.renderability.blocking_reasons.as_slice())
        .unwrap_or(&[]);
    let state_count = fsm.map(|f| f.state_candidates.len()).unwrap_or(0);
    let transition_count = fsm.map(|f| f.transition_candidates.len()).unwrap_or(0);
    let signal_count = fsm.map(|f| f.signal_inventory.len()).unwrap_or(0);
    let initial_state_count = fsm
        .map(|f| f.state_candidates.iter().filter(|s| s.is_initial).count())
        .unwrap_or(0);
    let module_candidate_count = fsm.map(|f| f.module_candidates.len()).unwrap_or(0);
    let top_candidate_count = fsm.map(|f| f.top_candidates.len()).unwrap_or(0);
    let decision_tree_count = fsm.map(|f| f.decision_tree_candidates.len()).unwrap_or(0);
    let system_contract_present = fsm.is_some_and(|f| f.system_contract.is_some());
    let init_assignment_count = fsm.map(|f| f.init_assignments.len()).unwrap_or(0);
    ...
    println!(
        "  root_name: {}",
        fsm.map(|f| &*f.root_name).unwrap_or("n/a")
    );
    println!(
        "  selected_root_kind: {}",
        fsm.map(|f| f.root_kind_decision.selected_root_kind.as_str())
            .unwrap_or("n/a")
    );
    ...
    println!("  signal_inventory: {signal_count}");
    println!(
        "  system_contract: {}",
        if system_contract_present {
            "present"
        } else {
            "absent"
        }
    );
    println!("  init_assignments: {init_assignment_count}");
    println!("  state_candidates: {state_count}");
    println!("  initial_state_count: {initial_state_count}");
    println!("  transition_candidates: {transition_count}");
    println!("  module_candidates: {module_candidate_count}");
    println!("  top_candidates: {top_candidate_count}");
    println!("  decision_tree_candidates: {decision_tree_count}");
    ...
    if fsm_absent {
        findings.push(finding(
            "fsm_adapter_artifact_missing_fsm_payload",
            ValidationFindingSeverity::Error,
            "structural",
            "adapter artifact targets FSM but carries no FSM payload".to_string(),
            Vec::new(),
        ));
    }
    if !schema_ok {
    ...
    if !is_renderable && !fsm_absent {
        findings.push(finding(
            "fsm_adapter_not_renderable",
            ValidationFindingSeverity::Warning,
            "renderability",
            format!(
                "FSM adapter is not renderable ({} blocking reason(s))",
                blocking_reasons.len()
            ),
            blocking_reasons.iter().take(8).cloned().collect(),
        ));
    }
    if fsm.is_some() && state_count == 0 {
        findings.push(finding(
            "fsm_adapter_no_state_candidates",
            ValidationFindingSeverity::Info,
            "state_machine",
            "no state candidates in FSM adapter; state graph is empty".to_string(),
            Vec::new(),
        ));
    }
    if state_count > 0 && initial_state_count != 1 {
        findings.push(finding(
            "fsm_adapter_initial_state_cardinality",
            ValidationFindingSeverity::Warning,
            "state_machine",
            format!(
                "FSM state machine has {initial_state_count} initial state(s) across {state_count} state candidate(s); expected exactly one"
            ),
            Vec::new(),
        ));
    }
    if state_count > 0 && transition_count == 0 {
        findings.push(finding(
            "fsm_adapter_no_transitions",
            ValidationFindingSeverity::Info,
            "state_machine",
            format!("{state_count} state candidate(s) present but no transition candidates"),
            Vec::new(),
        ));
    }
    if !is_renderable && !fsm_absent && signal_count == 0 {
        findings.push(finding(
            "fsm_adapter_empty_signal_inventory",
            ValidationFindingSeverity::Info,
            "interface_coverage",
            "signal inventory is empty; no signals carried forward from IntentIR".to_string(),
            Vec::new(),
        ));
    }
    if !system_contract_present && !fsm_absent {
        findings.push(finding(
            "fsm_adapter_no_system_contract",
            ValidationFindingSeverity::Info,
            "system_contract",
            "no system contract (clock/reset) carried forward from IntentIR".to_string(),
            Vec::new(),
        ));
    }
    if residual_decision_count > 0 {
        findings.push(finding(
            "fsm_adapter_residual_decisions_present",
            ValidationFindingSeverity::Warning,
            "residual_decisions",
            format!("FSM adapter carries {residual_decision_count} residual decision packet(s)"),
            artifact
                .residual_decisions
                .iter()
                .map(|packet| packet.packet_id.clone())
    ...
    let report = ValidationReportRecord {
        report_id: format!("validation_fsm_adapter_{artifact_fingerprint}"),
        validated_stage: IrStage::FsmAdapter,
        artifact_fingerprint,
        summary: format!(
            "FSM adapter validation for {} with {} finding(s)",
        ...
        metrics: vec![
            ...
            metric("signal_inventory", signal_count.to_string()),
            metric(
                "system_contract",
                if system_contract_present {
                    "present"
            ...
            metric("init_assignments", init_assignment_count.to_string()),
            metric("state_candidates", state_count.to_string()),
            metric("initial_state_count", initial_state_count.to_string()),
            metric("transition_candidates", transition_count.to_string()),
...
mod tests {
    ...
    #[test]
    fn validate_fsm_adapter_reports_structural_and_coverage_findings() -> Result<()> {
        let tempdir = tempdir()?;
        let source = tempdir.path().join("fsm_validate_spec.md");
        let source_artifact_base = tempdir.path().join("generated").join("source_ir");
        let evidence_artifact_base = tempdir.path().join("generated").join("evidence_ir");
        let semantic_artifact_base = tempdir.path().join("generated").join("semantic_ir");
        let intent_artifact_base = tempdir.path().join("generated").join("intent_ir");
        ...
        let fingerprint = fsm_adapter_fingerprint(&adapter)?;
        let report = validate_fsm_adapter(&adapter, fingerprint);

        // Structural: schema version is 1
        assert_eq!(metric_value(&report, "schema_version"), Some("1"));
        // Coverage: signal inventory should have entries
        let signal_count: usize = metric_value(&report, "signal_inventory")
            .and_then(|v| v.parse().ok())
            .unwrap_or(0);
        assert!(signal_count > 0);
        // Coverage: state and transition candidates should exist
        assert!(metric_value(&report, "state_candidates").is_some());
        assert!(metric_value(&report, "transition_candidates").is_some());
        // Should NOT report missing FSM payload
        assert!(!has_finding(
            &report,
            "fsm_adapter_artifact_missing_fsm_payload"
        ));
