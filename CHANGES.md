...
- Tightened `builds_renderable_computed_selector_dt_fsm_adapter_artifact` so the renderable computed-selector DT now proves its decision-tree candidate remains visible.
- The test locks the candidate's medium automation confidence and selected signal inventory for the computed selector inputs plus action outputs while preserving emitted `.fsm` computed-selector text.
- This captures the honest lower-confidence shape of computed selector lowering without inventing canonical control-fragment support where the fixture does not have it.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge builds_renderable_computed_selector_dt_fsm_adapter_artifact` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge builds_renderable_selector_based_dt_fsm_adapter_artifact` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed (`148` fixtures, `0` failures)
...
### Added: selector/test-node provenance coverage
- Tightened `builds_renderable_selector_based_dt_fsm_adapter_artifact` so the renderable selector-based DT now proves its decision-tree candidate remains visible.
- The test locks control-fragment support IDs, the single selector branch block, high automation confidence, and selector/output signal inventory while preserving emitted `.fsm` selector/test-node text.
- This pairs the renderable selector path with the blocked selector-branch coverage so both outcomes keep their control evidence auditable.
...
### Added: selector-blocker provenance coverage
- Tightened `keeps_selector_based_dt_blocked_when_branch_predicate_is_not_relative_to_selector` so a blocked selector-based DT now proves its decision-tree candidate remains visible.
- The test locks control-fragment support IDs, referenced guard/action signals, and high automation confidence while the selector branch predicate remains unsupported for `.fsm` emission.
- The selected signal inventory also keeps the selector head plus guard/action signals visible, preserving reviewability without rendering an unsafe branch token.
...
- Tightened `keeps_structured_fsm_blocked_when_transition_target_is_undeclared` so a blocked structured-FSM root now proves the undeclared-target transition remains visible in `fsm.transition_candidates`.
- The test locks source/target state names, transition support IDs, and high automation confidence while the missing target-state blocker remains active.
- This keeps the unsafe transition auditable without fabricating the target state or emitting `.fsm` output.
...
- Tightened `top_composition_blocks_conflicting_child_link_topology_directions` so a blocked child output now proves both contradictory topology-link support-ID sets survive in the selected module inventory.
- The test also locks preservation of the child output signal declaration support after direction hints are cleared and topology evidence creates the conflict.
- This keeps both link directions and the local signal declaration auditable while graph-backed direction conflict blocks `.fsm` emission.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge top_composition_blocks_conflicting_child_link_topology_directions` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed (`148` fixtures, `0` failures)
...
### Fixed: signal-level support preservation in adapter inventories
- Updated adapter signal-inventory registration so interface signal records carry both the enclosing interface id and the signal record's own supporting statement IDs.
- Tightened `top_composition_blocks_conflicting_child_topology_widths` so a blocked child output now proves both the explicit child signal declaration support and conflicting topology-link support survive in the selected module inventory.
- The slice keeps child-local width evidence and top-link width evidence auditable when they disagree and block `.fsm` emission.
...
- Tightened `top_composition_blocks_conflicting_sibling_child_link_widths` so the blocked consumer input now proves both conflicting sibling topology-link support-ID sets survive in the selected signal inventory.
- This is coverage-only over the existing adapter behavior: incompatible sibling child-link widths still block `.fsm` emission instead of selecting one width.
- The slice keeps both producer-to-consumer link evidence trails auditable while the consumer input remains width-conflicted.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge top_composition_blocks_conflicting_sibling_child_link_widths` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed (`148` fixtures, `0` failures)
...
- Tightened `renderable_top_document_deduplicates_reused_fsm_child_roots` so reused FSM child declarations now prove their distinct support IDs survive in the selected top candidate.
- This is coverage-only over the existing adapter behavior: two FSM child instances can still share one emitted FSM module root while each top child remains separately auditable.
- The slice extends reused-child provenance coverage from direct-DT child roots to reused FSM child roots.
...
- Tightened `top_composition_recovers_child_width_from_sibling_child_link_topology` so renderable top-composition `.fsm` lowering now proves target-side child width recovery from a sibling child link preserves topology-link support IDs and high automation confidence.
- This is coverage-only over the existing adapter behavior: a consumer child input width recovered from a producer sibling link remains renderable.
- The slice keeps the producer-to-consumer link evidence auditable in the consumer module inventory.
...
### Added: child top-link width recovery provenance coverage
- Tightened `top_composition_recovers_child_width_from_top_link_topology` so renderable top-composition `.fsm` lowering now proves child width recovery from a top link preserves topology-link support IDs and high automation confidence.
- This is coverage-only over the existing adapter behavior: a child port width recovered from a public top-link width remains renderable when the composition is otherwise complete.
- The slice keeps the producer child width recovery auditable from the explicit top-link evidence.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge top_composition_recovers_child_width_from_top_link_topology` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed (`148` fixtures, `0` failures)
...
- Tightened `top_composition_keeps_duplicate_top_port_direction_conflict_unresolved` so blocked top-composition `.fsm` lowering now proves both duplicate top-port declarations keep support IDs and high automation confidence on the recovered top ports and selected signal inventory.
- This is coverage-only over the existing adapter behavior: duplicate public top-port declarations with conflicting directions still block `.fsm` emission.
- The slice keeps both duplicate declaration records auditable while their direction remains unresolved.
...
- Tightened `top_composition_blocks_conflicting_top_actor_port_width` so blocked top-composition `.fsm` lowering now proves explicit top-port support IDs, graph actor-port support ID, width-conflict inventory state, and high automation confidence survive.
- This is coverage-only over the existing adapter behavior: conflicting top-boundary width evidence still blocks `.fsm` emission.
- The slice keeps flat top-port width and graph actor-port width evidence auditable together when they disagree.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge top_composition_blocks_conflicting_top_actor_port_width` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed (`148` fixtures, `0` failures)
...
### Added: top actor-port direction conflict provenance coverage
- Tightened `top_composition_blocks_conflicting_top_actor_port_direction` so blocked top-composition `.fsm` lowering now proves explicit top-port support IDs, graph actor-port support ID, and high automation confidence survive in both the recovered top port and selected signal inventory.
- This is coverage-only over the existing adapter behavior: conflicting top-boundary direction evidence still blocks `.fsm` emission.
- The slice keeps flat top-port and graph actor-port evidence auditable together when they disagree.
...
### Added: recovered top-port blocked-path provenance coverage
- Tightened `top_composition_preserves_recovered_top_port_direction_when_still_blocked` so top-link-recovered top-port direction now proves support IDs and high automation confidence survive on the blocked top candidate and selected signal inventory.
- This is coverage-only over the existing adapter behavior: the top remains blocked when its child module is missing, but recovered top-boundary direction evidence stays auditable.
- The slice keeps top-link direction recovery visible even when another composition gate still blocks `.fsm` emission.
...
- Tightened `top_composition_blocks_parametric_top_port_width_for_fsm_public_io` so blocked top-composition `.fsm` lowering now proves explicit top-port support IDs and high automation confidence survive in both the top candidate and selected signal inventory.
- This is coverage-only over the existing adapter behavior: a public top port with symbolic width still blocks `.fsm` emission until numeric width evidence exists.
- The slice keeps explicit top-boundary evidence auditable while the renderability blocker explains the unresolved parametric width.
...
- Tightened `structured_fsm_blocks_graph_backed_undriven_output_inventory` so blocked structured-FSM `.fsm` lowering now proves graph-backed undriven output support ID and high automation confidence survive in the FSM signal inventory.
- This is coverage-only over the existing adapter behavior: graph-backed output inventory entries with no typed FSM-state drive action keep `.fsm` emission blocked.
- The slice keeps the selected output actor evidence auditable while the renderability blocker explains the missing typed action.
...
- Tightened `standalone_sequential_dt_blocks_conflicting_system_contract_signal_direction` so blocked standalone sequential `.fsm` lowering now proves system-contract support IDs and high automation confidence survive when clock direction evidence conflicts.
- This is coverage-only over the existing adapter behavior: contradictory canonical clock direction evidence keeps `.fsm` emission blocked instead of self-healing to an input.
- The slice keeps system-contract direction conflict evidence auditable in `fsm.signal_inventory` for review/rescan.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge standalone_sequential_dt_blocks_conflicting_system_contract_signal_direction` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed (`601` Rust tests, warning-deny Clippy/rustdoc, mdBook validation)
...
- Tightened `standalone_dt_blocks_conflicting_flat_direction_even_with_actor_graph` so blocked direct-root canonical direction conflicts now prove interface and actor-port categories, graph support ID, and high automation confidence remain visible.
- This is coverage-only over the existing adapter behavior: contradictory flat direction evidence blocks `.fsm` emission even when graph-backed actor evidence is unambiguous.
- The slice keeps canonical conflict provenance and graph evidence inspectable together in `fsm.signal_inventory`.
...
- Tightened `standalone_dt_ignores_unrelated_actor_ports_for_graph_context` so renderable direct-root graph recovery proves selected controller output support IDs and high automation confidence survive even when an unrelated actor exposes a side-band port.
- This is coverage-only over the existing adapter behavior: unrelated actor-port signals stay out of `fsm.signal_inventory`.
- The slice keeps direct actor selection auditable while proving unrelated side-band graph evidence does not contaminate selected output provenance.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge standalone_dt_ignores_unrelated_actor_ports_for_graph_context` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed (`601` Rust tests, warning-deny Clippy/rustdoc, mdBook validation)
...
- Tightened `standalone_dt_blocks_conflicting_control_input_actor_port_widths` so blocked direct-root actor-port width conflicts now prove both graph-backed support IDs and high automation confidence survive in `fsm.signal_inventory`.
...
- The slice keeps contradictory direct-root graph width evidence auditable for review/rescan instead of reducing the blocked artifact to a generic width-conflict flag.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge standalone_dt_blocks_conflicting_control_input_actor_port_widths` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed (`601` Rust tests, warning-deny Clippy/rustdoc, mdBook validation)
...
- Tightened `standalone_explicit_module_recovers_control_input_width_from_actor_port_graph` so recovered module-local control-input widths now prove graph-backed actor-port support IDs and high automation confidence survive in module `signal_inventory`.
- This is coverage-only over the existing adapter behavior: actor-port numeric width evidence can recover missing explicit-module control-input width without importing the external actor's direction.
- The slice keeps recovered graph width evidence auditable on the renderable explicit-module `.fsm` path.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge standalone_explicit_module_recovers_control_input_width_from_actor_port_graph` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed (`601` Rust tests, warning-deny Clippy/rustdoc, mdBook validation)
...
- Tightened `structured_fsm_derives_guard_inputs_from_control_reads_after_output_actor_selection` so recovered structured-FSM control-read inputs now prove canonical control/transition support IDs and high automation confidence survive in `fsm.signal_inventory`.
- This is coverage-only over the existing adapter behavior: FSM state-body reads and transition guards can recover target-actor input directions without importing external actor-port direction.
- The slice keeps structured-FSM guard/action evidence auditable beside actor-port graph evidence on the renderable `.fsm` path.
...
- Tightened `standalone_dt_blocks_parametric_signal_width_with_diagnostic` so blocked canonical parametric signal widths now prove interface provenance category, nonempty supporting IDs, and high automation confidence remain visible in `fsm.signal_inventory`.
- This is coverage-only over the existing adapter behavior: symbolic canonical width evidence stays inspectable while numeric `.fsm` signal emission remains blocked.
- The slice complements the actor-port parametric width provenance lock by keeping canonical and graph-backed symbolic width sources independently auditable.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge standalone_dt_blocks_parametric_signal_width_with_diagnostic` -> passed (`1` test)
- `cargo test --manifest-path Cargo.toml -p specforge ir::adapters::tests` -> passed (`80` tests)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed (`601` Rust tests, warning-deny Clippy/rustdoc, mdBook validation)
...
- Extended the malformed local-provider regression so NLP enrichment command hints reject repeated `--vlm-provider` flags.
- This keeps source enrichment and NLP enrichment provider parsing covered for duplicate-provider fail-closed behavior.
...
- Added execution-parser hardening so structured rescan command hints reject current-directory source or artifact path tokens.
- Added regression coverage for current-directory ingest, enrich, and validate replay paths.
- Updated the mdBook rescan-plan reference to describe the current-directory replay path restriction.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_current_directory_replay_artifact_paths` -> passed (`1` test, `592` filtered out)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed (`593` Rust tests, warnings denied for Clippy/rustdoc, mdBook validation)
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed (`130` fixtures, `0` failures)
...
- Added parser coverage proving source-enrichment replay hints accept `--vlm-provider skip`.
- This completes the local-provider matrix coverage for source-enrichment replay hints without changing parser behavior.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_parses_skip_source_enrich_provider_hint` -> passed (`1` test, `587` filtered out)
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed (`588` Rust tests, warnings denied for Clippy/rustdoc, mdBook validation)
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed (`130` fixtures, `0` failures)
- `git diff --check` -> passed
- checkout-specific absolute path scan across tracked markdown -> passed
...
- Added execution-status coverage proving grade removal marks the recommendation `executed_validated_changed`.
- This keeps qualitative grade availability drift visible in execution counters even when arbitration classifies the delta as neutral review.
...
- Added `run_plan` dry-run coverage proving report metadata preserves the plan path, document-key filter, scoped pending count, selected count, and zero execution counters.
- This protects non-executing `rescan-plan` summary behavior when operators scope and limit a replay queue.
...
- Added run-report coverage proving `arbitration_verdict_count(...)` counts exact verdict matches without folding neighboring review verdicts together.
- This keeps `rescan-plan` summary counters precise for validated no-change, regression review, possible-improvement review, and absent neutral-review verdicts.
...
- Added render coverage proving multiple `recommended_commands` for one rescan recommendation print in stored plan order.
- This keeps review-facing replay lanes faithful to the generated schema-v2 plan when a recommendation carries rebuild and validate hints together.
...
- Added helper coverage proving `rescan-plan` derives `validation_report.json` beside the artifact being validated.
- This keeps execution summaries tied to the deterministic stage-local validation sidecar path.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_validation_report_path_uses_artifact_directory_sidecar` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
### Changed: rescan plans reject unsupported schema versions
- Added loader coverage proving a schema-v1 rescan plan is rejected before queue selection or execution.
- This keeps the executor pinned to the current schema-v2 replay contract.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_unsupported_schema_version` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
- Added render coverage proving dry-run output includes selected recommendations and excludes unselected queue entries.
- This keeps document-scoped or limited previews faithful to the actual selected execution set.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_dry_run_render_honors_selected_indices` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
- Added negative coverage for cargo-shaped hints that replace `run` with another cargo subcommand.
- Added negative coverage for hints that drop the required `--` separator before SpecForge args.
- This keeps the executor pinned to the exact repository-local `cargo run --manifest-path Cargo.toml -- ...` prefix.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_malformed_cargo_prefix_tokens` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
### Changed: non-enrichment replay commands reject missing paths
- Added negative coverage for missing `ingest`, `evidence`, `semantic`, and `validate` command-hint path arguments.
- This locks required artifact/path presence for exact stage rebuild and validation replay hints.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_missing_stage_command_paths` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
- Added negative coverage for source enrichment command hints where `--vlm-provider` is followed by another option token.
- Added the same coverage for NLP enrichment command hints.
- This locks the existing local-provider parser boundary so truncated provider hints cannot be mistaken for valid replay policy.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_flag_shaped_provider_values` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
- Added negative coverage for `--vlm-model` with no value in source enrichment command hints.
- Added the same missing-value coverage for NLP enrichment command hints.
- This keeps local replay model overrides explicit and prevents truncated command hints from reaching execution.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_missing_model_values` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
- Added negative parser coverage for malformed local provider options in `rescan-plan` command hints.
- The new test proves the executor rejects:
  - repeated `--vlm-provider` flags
  - missing `--vlm-provider` values
  - unsupported provider-side args
- This keeps local replay hints strict without relying only on accepted-provider and OpenAI-rejection coverage.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_malformed_local_provider_hints` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
...
- Added a `rescan-plan` dry-run renderer test for pending recommendations with no replay inputs, no related ids, and no command hints.
- The preview must render those fields as explicit `none` values:
  - `replay_inputs: none`
  - `related_ids: none`
  - `recommended_commands: none`
- This keeps sparse schema-v2 recommendations readable in dry-run mode instead of letting empty lists disappear from the review surface.

### Validation
- `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_dry_run_render_surfaces_empty_replay_fields_as_none` -> passed with `1` test
- `cargo fmt --manifest-path Cargo.toml -- --check` -> passed
- `bash scripts/run_docs_ci.sh` -> passed
- `bash scripts/run_ci.sh` -> passed with `524` Rust tests plus warning-deny Clippy/rustdoc and mdBook validation
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` -> passed with `130` fixtures and `0` failures
