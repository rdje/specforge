...
- New batch slice 166/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the semantic phrase source-kind mismatch guard by proving the visual-caption-only prior remains free of alias-grounded prose hints for the prose-local `XSTAGE` phrase.
...
- Broader `bash scripts/run_ci.sh` passed on slice 140 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
- New batch slice 165/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the semantic phrase source-kind mismatch guard by proving the visual-caption-only prior remains free of table semantic hints for the prose-local `XSTAGE` phrase.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase family-mismatch timing-extraction absence
- New batch slice 164/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the semantic phrase protocol-family mismatch guard by proving the unrelated APB prior remains free of timing-diagram extraction support for the AXI-local `XREQ` phrase.
...
## 2026-05-07 KG fixture for semantic phrase family-mismatch VLM exclusion
- New batch slice 163/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the semantic phrase protocol-family mismatch guard by proving the unrelated APB prior remains free of VLM timing annotations for the AXI-local `XREQ` phrase.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase family-mismatch visual exclusion
- New batch slice 162/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the semantic phrase protocol-family mismatch guard by proving the unrelated APB prior remains free of visual-caption hints for the AXI-local `XREQ` phrase.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase family-mismatch alias exclusion
- New batch slice 161/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the semantic phrase protocol-family mismatch guard by proving the unrelated APB prior remains free of alias-grounded prose hints for the AXI-local `XREQ` phrase.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase family-mismatch table exclusion
- New batch slice 160/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the semantic phrase protocol-family mismatch guard by proving the unrelated APB prior remains free of table semantic hints for the AXI-local `XREQ` phrase.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior semantic phrase timing-extraction absence
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the no-prior semantic phrase mirror by proving `XACK can receive the transfer` remains free of timing-diagram extraction support when learned memory is absent.
...
## 2026-05-07 KG fixture for no-prior semantic phrase VLM exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the no-prior semantic phrase mirror by proving `XACK can receive the transfer` remains free of VLM timing annotations when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior semantic phrase visual exclusion
- New batch slice 157/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the no-prior semantic phrase mirror by proving `XACK can receive the transfer` remains free of visual-caption hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior semantic phrase alias exclusion
- New batch slice 156/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the no-prior semantic phrase mirror by proving `XACK can receive the transfer` remains free of alias-grounded prose hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 155/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the no-prior semantic phrase mirror by proving `XACK can receive the transfer` remains free of table semantic hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 154/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the no-prior ready-sink semantic phrase mirror by proving `XACK can sink the transfer` remains free of timing-diagram extraction support when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` passed on slice 140 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
## 2026-05-07 KG fixture for no-prior ready-sink phrase VLM exclusion
- New batch slice 153/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the no-prior ready-sink semantic phrase mirror by proving `XACK can sink the transfer` remains free of VLM timing annotations when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior ready-sink phrase visual exclusion
- New batch slice 152/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the no-prior ready-sink semantic phrase mirror by proving `XACK can sink the transfer` remains free of visual-caption hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior ready-sink phrase alias exclusion
- New batch slice 151/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the no-prior ready-sink semantic phrase mirror by proving `XACK can sink the transfer` remains free of alias-grounded prose hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior ready-sink phrase table exclusion
- New batch slice 150/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the no-prior ready-sink semantic phrase mirror by proving `XACK can sink the transfer` remains free of table semantic hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior valid-like phrase timing-extraction absence
- New batch slice 149/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the no-prior valid-like semantic phrase mirror by proving `XREQ can publish the beat` remains free of timing-diagram extraction support when learned memory is absent.
...
## 2026-05-07 KG fixture for no-prior valid-like phrase VLM exclusion
- New batch slice 148/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the no-prior valid-like semantic phrase mirror by proving `XREQ can publish the beat` remains free of VLM timing annotations when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior valid-like phrase visual exclusion
- New batch slice 147/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the no-prior valid-like semantic phrase mirror by proving `XREQ can publish the beat` remains free of visual-caption hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior valid-like phrase alias exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the no-prior valid-like semantic phrase mirror by proving `XREQ can publish the beat` remains free of alias-grounded prose hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior valid-like phrase table exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the no-prior valid-like semantic phrase mirror by proving `XREQ can publish the beat` remains free of table semantic hints when learned memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for valid-like phrase prior timing-extraction absence
- New batch slice 144/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the prior-guided valid-like semantic phrase gold by proving the `XREQ can publish the beat` recovery has no timing-diagram extraction support before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` passed on slice 140 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
## 2026-05-07 KG fixture for valid-like phrase prior VLM exclusion
- New batch slice 143/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the prior-guided valid-like semantic phrase gold by proving the `XREQ can publish the beat` recovery does not use VLM timing annotations before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for valid-like phrase prior visual exclusion
- New batch slice 142/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the prior-guided valid-like semantic phrase gold by proving the `XREQ can publish the beat` recovery does not use visual-caption evidence before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for valid-like phrase prior alias exclusion
- New batch slice 141/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the prior-guided valid-like semantic phrase gold by proving the `XREQ can publish the beat` recovery does not use alias-grounded prose before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for valid-like phrase prior table exclusion
- New batch slice 140/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the prior-guided valid-like semantic phrase gold by proving the `XREQ can publish the beat` recovery remains prose-only before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for ready-sink phrase prior timing-extraction absence
- New batch slice 139/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the prior-guided ready-sink semantic phrase gold by proving the `XACK can sink the transfer` recovery has no timing-diagram extraction support before role resolution.
...
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` passed on slice 120 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.

## 2026-05-07 KG fixture for ready-sink phrase prior VLM exclusion
- New batch slice 138/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the prior-guided ready-sink semantic phrase gold by proving the `XACK can sink the transfer` recovery does not use VLM timing annotations before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for ready-sink phrase prior visual exclusion
- New batch slice 137/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the prior-guided ready-sink semantic phrase gold by proving the `XACK can sink the transfer` recovery does not use visual-caption evidence before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for ready-sink phrase prior alias exclusion
- New batch slice 136/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the prior-guided ready-sink semantic phrase gold by proving the `XACK can sink the transfer` recovery does not use alias-grounded prose before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for ready-sink phrase prior table exclusion
- New batch slice 135/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the prior-guided ready-sink semantic phrase gold by proving the `XACK can sink the transfer` recovery remains prose-only before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase prior timing-extraction absence
- New batch slice 134/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the semantic phrase prior gold by proving the `XACK can receive the transfer` recovery has no timing-diagram extraction support before role resolution.
...
## 2026-05-07 KG fixture for semantic phrase prior VLM exclusion
- New batch slice 133/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the semantic phrase prior gold by proving the `XACK can receive the transfer` recovery does not use VLM timing annotations before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase prior visual exclusion
- New batch slice 132/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the semantic phrase prior gold by proving the `XACK can receive the transfer` recovery does not use visual-caption evidence before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase prior alias exclusion
- New batch slice 131/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the semantic phrase prior gold by proving the `XACK can receive the transfer` recovery does not use alias-grounded prose before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic phrase prior table exclusion
- New batch slice 130/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside total and prose semantic-hint metrics.
- This strengthens the semantic phrase prior gold by proving the `XACK can receive the transfer` recovery remains prose-only before role resolution.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for AMBA-generic modality fallback timing-extraction absence
- New batch slice 129/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the AMBA-generic semantic-modality fallback by proving no timing diagram extraction participates before generic AMBA memory breaks the tie.
...
## 2026-05-07 KG fixture for AMBA-generic modality fallback VLM exclusion
- New batch slice 128/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the AMBA-generic semantic-modality fallback by proving VLM timing annotations do not participate before generic AMBA memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for AMBA-generic modality fallback visual exclusion
- New batch slice 127/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the AMBA-generic semantic-modality fallback by proving visual-caption evidence does not participate before generic AMBA memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for AMBA-generic modality fallback alias exclusion
- New batch slice 126/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the AMBA-generic semantic-modality fallback by proving alias-grounded prose does not participate before generic AMBA memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 125/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 1` alongside total and table semantic-hint metrics.
- This strengthens the AMBA-generic semantic-modality fallback by proving the local prose observation remains counted before generic AMBA memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 1` alongside total semantic hints.
- This strengthens the AMBA-generic semantic-modality fallback by proving the structured-table observation remains counted before generic AMBA memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for AMBA-generic modality fallback hint totals
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` at `EvidenceIR`.
- This strengthens the AMBA-generic semantic-modality fallback by proving the AXI-family document still supplies exactly the local table/prose observations before generic AMBA memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 122/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the prior-guided semantic-modality gold by proving no timing diagram extraction participates before modality memory breaks the tie.
...
- New batch slice 121/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the prior-guided semantic-modality gold by proving VLM timing annotations do not participate before modality memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for modality-prior gold visual exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the prior-guided semantic-modality gold by proving visual-caption evidence does not participate before modality memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_ci.sh` passed on slice 120 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the prior-guided semantic-modality gold by proving alias-grounded prose does not participate before modality memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` passed on slice 100 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 1` alongside total and table semantic-hint metrics.
- This strengthens the prior-guided semantic-modality gold by proving the local prose observation remains counted before modality memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for modality-prior gold table split
- New batch slice 117/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 1` alongside total semantic hints.
- This strengthens the prior-guided semantic-modality gold by proving the structured-table observation remains counted before modality memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for modality-prior gold hint totals
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` at `EvidenceIR`.
- This strengthens the prior-guided semantic-modality gold by proving the current document still supplies exactly the local table/prose observations before modality memory breaks the tie.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior modality timing-extraction absence
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the no-prior semantic-modality mirror by proving no timing diagram extraction participates while modality memory is absent.
...
## 2026-05-07 KG fixture for no-prior modality VLM exclusion
- New batch slice 114/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the no-prior semantic-modality mirror by proving VLM timing annotations do not participate while modality memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior modality visual exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose semantic-hint metrics.
- This strengthens the no-prior semantic-modality mirror by proving visual-caption evidence does not participate while modality memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior modality alias exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the no-prior semantic-modality mirror by proving alias-grounded prose does not participate while modality memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior modality prose split
- New batch slice 111/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 1` alongside total and table semantic-hint metrics.
- This strengthens the no-prior semantic-modality mirror by proving the local prose observation remains counted while modality memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior modality table split
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 1` alongside total semantic hints.
- This strengthens the no-prior semantic-modality mirror by proving the local structured-table observation remains counted while modality memory is absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior modality hint totals
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` at `EvidenceIR`.
- This strengthens the no-prior semantic-modality mirror by proving absent modality memory does not alter the local table/prose evidence count.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for weak modality-prior timing-extraction absence
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the weak semantic-modality prior guard by proving no timing diagram extraction participates while underpowered memory stays non-decisive.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the weak semantic-modality prior guard by proving VLM timing annotations do not participate while underpowered memory stays non-decisive.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the weak semantic-modality prior guard by proving visual-caption evidence does not participate while underpowered memory stays non-decisive.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for weak modality-prior alias exclusion
- New batch slice 105/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the weak semantic-modality prior guard by proving alias-grounded prose does not participate while underpowered memory stays non-decisive.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for weak modality-prior prose split
- New batch slice 104/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 1` alongside total and table semantic-hint metrics.
- This strengthens the weak semantic-modality prior guard by proving the local prose observation remains counted while underpowered memory stays non-decisive.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for weak modality-prior table split
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 1` alongside total semantic hints.
- This strengthens the weak semantic-modality prior guard by proving the local structured-table observation remains counted while underpowered memory stays non-decisive.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for weak modality-prior hint totals
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` at `EvidenceIR`.
- This strengthens the weak semantic-modality prior guard by proving underpowered memory does not alter the local table/prose evidence count.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality source-kind mismatch timing-extraction absence
...
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the semantic-modality source-kind mismatch guard by proving no timing diagram extraction participates while the visual-only prior stays scoped out of table/prose arbitration.
...
## 2026-05-07 KG fixture for semantic-modality source-kind mismatch VLM exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the semantic-modality source-kind mismatch guard by proving VLM timing annotations do not participate while the visual-only prior stays scoped out of table/prose arbitration.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_ci.sh` passed at the batch midpoint with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the semantic-modality source-kind mismatch guard by proving visual-caption evidence does not participate while the visual-only prior stays scoped out of table/prose arbitration.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose semantic-hint metrics.
- This strengthens the semantic-modality source-kind mismatch guard by proving alias-grounded prose does not participate while the visual-only prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality source-kind mismatch prose split
- New batch slice 97/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 1` alongside total and table semantic-hint metrics.
- This strengthens the semantic-modality source-kind mismatch guard by proving the local prose observation remains counted while the visual-only prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality source-kind mismatch table split
- New batch slice 96/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 1` alongside total semantic hints.
- This strengthens the semantic-modality source-kind mismatch guard by proving the local structured-table observation remains counted while the visual-only prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality source-kind mismatch hint totals
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` at `EvidenceIR`.
- This strengthens the semantic-modality source-kind mismatch guard by proving the visual-caption-only prior does not alter the local table/prose evidence count.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality family-mismatch timing-extraction absence
...
- The fixture now asserts `timing_diagram_extractions = 0` alongside the full semantic-hint source split.
- This strengthens the semantic-modality protocol-family mismatch guard by proving no timing diagram extraction participates while the unrelated APB modality prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` last passed on slice 50 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
## 2026-05-07 KG fixture for semantic-modality family-mismatch VLM exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table, prose, alias-grounded prose, and visual-caption metrics.
- This strengthens the semantic-modality protocol-family mismatch guard by proving VLM timing annotations do not participate while the unrelated APB modality prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality family-mismatch visual exclusion
- New batch slice 92/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 0` alongside total, table, prose, and alias-grounded prose metrics.
- This strengthens the semantic-modality protocol-family mismatch guard by proving visual captions do not participate while the unrelated APB modality prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality family-mismatch alias exclusion
- New batch slice 91/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table, and prose source-split metrics.
- This strengthens the semantic-modality protocol-family mismatch guard by proving alias-grounded prose does not participate while the unrelated APB modality prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality family-mismatch prose split
- New batch slice 90/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 1` alongside total and table source-split metrics.
- This strengthens the semantic-modality protocol-family mismatch guard by proving the local prose-side observation remains counted while the unrelated APB modality prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 89/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 1` alongside total `signal_semantic_hints = 2`.
- This strengthens the semantic-modality protocol-family mismatch guard by proving the local table-side observation remains counted while the unrelated APB modality prior stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` at `EvidenceIR`.
- This strengthens the semantic-modality protocol-family mismatch guard by proving the unrelated APB modality prior stays scoped out while both AXI-local table/prose observations remain counted.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_guided_semantic_conflict_caution_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside its total and other source-split semantic-hint metrics.
- This strengthens the caution gold by proving the negative-knowledge-guided `XCTRL` conflict is not alias-prose-supported.
...
- New batch slice 86/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_guided_semantic_conflict_caution_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 0` alongside its total and visual source-split semantic-hint metrics.
- This strengthens the caution gold by proving the negative-knowledge-guided `XCTRL` conflict is not ordinary-prose-supported.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_guided_semantic_conflict_caution_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_guided_semantic_conflict_caution_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside its total, visual-caption, and VLM timing semantic-hint metrics.
- This strengthens the caution gold by proving the negative-knowledge-guided `XCTRL` conflict is visual-only rather than table-supported.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_guided_semantic_conflict_caution_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `timing_diagram_extractions = 1` alongside its source-split semantic-hint metrics.
- This strengthens the protocol-family mismatch guard by proving the VLM timing semantic hint is extraction-backed while unrelated APB caution memory stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` last passed on slice 50 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside the total and other source splits.
- This strengthens the protocol-family mismatch guard by proving the local AXI conflict is not alias-prose-supported while unrelated APB caution memory stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for negative-knowledge family-mismatch prose exclusion
- New batch slice 82/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 0` alongside the total and other source splits.
- This strengthens the protocol-family mismatch guard by proving the local AXI conflict is not ordinary-prose-supported while unrelated APB caution memory stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for negative-knowledge family-mismatch table exclusion
- New batch slice 81/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 0` alongside the total, visual-caption, and VLM timing source splits.
- This strengthens the protocol-family mismatch guard by proving the local AXI conflict is not table-supported while unrelated APB caution memory stays scoped out.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 1` alongside the total and visual-caption source split.
- This strengthens the protocol-family mismatch guard by proving unrelated APB caution memory stays scoped out while the AXI-local VLM timing observation remains counted.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 1` alongside the total semantic-hint count.
- This strengthens the protocol-family mismatch guard by proving unrelated APB caution memory stays scoped out while the AXI-local visual-caption observation remains counted.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for cross-modality grounding VLM exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_grounding_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside the total and other source splits.
- This strengthens the gold fixture by proving the positive `XREQ` cross-modality path does not use VLM timing-note evidence.
...
## 2026-05-07 KG fixture for cross-modality grounding alias exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_grounding_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside the total and other source splits.
- This strengthens the gold fixture by proving the positive `XREQ` cross-modality path does not use alias-grounded prose evidence.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_grounding_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for cross-modality grounding prose exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_grounding_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 0` alongside the total, table, and visual-caption source splits.
- This strengthens the gold fixture by proving the positive `XREQ` cross-modality path is not silently supported by ordinary prose.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_grounding_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_grounding_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_visual_captions = 1` alongside the total and table source split.
- This strengthens the gold fixture by proving the positive `XREQ` cross-modality path includes exactly one visual-caption semantic-role hint.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_grounding_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_grounding_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_tables = 1` alongside total `signal_semantic_hints = 2`.
- This strengthens the gold fixture by proving the positive `XREQ` cross-modality path includes exactly one table-sourced semantic-role hint.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_grounding_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_grounding_gold/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` for the positive table plus visual-caption cross-modality semantic-grounding path.
- This strengthens the gold fixture by proving decisive `XREQ` grounding consumes exactly the two semantic-role observations that create cross-modality support.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_grounding_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for cross-modality semantic-conflict VLM split
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_conflict_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside the existing total, table, prose, alias-grounded prose, and visual-caption splits.
- This strengthens the cross-modality semantic-conflict negative by proving the two-observation conflict does not hide a VLM timing-note semantic hint.
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_conflict_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_alias_grounded_prose = 0` alongside the existing total, table, prose, and visual-caption splits.
- This strengthens the cross-modality semantic-conflict negative by proving alias-grounded prose does not participate in the two-observation conflict.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_conflict_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 70/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_conflict_negative/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_prose = 0` alongside the existing total, table, and visual-caption splits.
- This strengthens the cross-modality semantic-conflict negative by proving the two conflicting observations are not contaminated by ordinary prose semantic hints.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_conflict_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/cross_modality_semantic_conflict_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 2` alongside the existing table and visual-caption source splits.
- This strengthens the cross-modality semantic-conflict negative by proving the aggregate evidence metric matches the two conflicting semantic-role observations.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench cross_modality_semantic_conflict_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- The fixture now asserts total `signal_semantic_hints = 0` in addition to zero VLM timing-annotation semantic hints.
- This strengthens the waveform-motion VLM timing negative by proving states such as `rising` and `LOW_TO_HIGH` create no semantic-role hints from any evidence source.
...
- New batch slice 67/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/vlm_timing_motion_annotation_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 0` in addition to zero VLM timing-annotation semantic hints.
- This strengthens the motion-only VLM timing annotation negative by proving prose such as `XREQ rises` creates no semantic-role hints from any evidence source.
...
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures after a loader-stuck first attempt was terminated and rerun cleanly.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` last passed on slice 50 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 66/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/vlm_timing_cycle_qualified_signal_value_annotation_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 0` in addition to zero VLM timing-annotation semantic hints.
- This strengthens the cycle-qualified VLM timing value-label negative by proving labels such as `XREQ HIGH at T1` create no semantic-role hints from any evidence source.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench vlm_timing_cycle_qualified_signal_value_annotation_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` last passed on slice 50 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
- New batch slice 65/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/vlm_timing_indexed_signal_value_annotation_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 0` in addition to zero VLM timing-annotation semantic hints.
- This strengthens the indexed VLM timing value-label negative by proving labels such as `XREQ[0] HIGH` create no semantic-role hints from any evidence source.
...
- New batch slice 64/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/vlm_timing_spurious_annotation_negative/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 0` in addition to zero VLM timing-annotation semantic hints.
- This strengthens the spurious VLM timing annotation negative by proving low-value labels and value-like annotations create no semantic-role hints from any evidence source.
...
## 2026-05-07 KG fixture for active-low VLM deassertion total hint exclusion
...
- Tightened `crates/specforge/test_data/kg_quality/vlm_timing_active_low_deassertion_equivalence_gold/fixture.json`.
- The fixture now asserts total `signal_semantic_hints = 0` in addition to zero VLM timing-annotation semantic hints.
- This strengthens the active-low reset-release VLM fixture by proving the evidence stage emits no semantic-role hints from any source for reset-release timing text.
...
- The fixture now asserts total `signal_semantic_hints = 0` in addition to zero VLM timing-annotation semantic hints.
- This strengthens the active-low reset assertion VLM fixture by proving the evidence stage emits no semantic-role hints from any source for reset assertion timing text.
...
- New batch slice 61/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/vlm_timing_active_low_deassertion_equivalence_gold/fixture.json`.
- The fixture now asserts `signal_semantic_hints_from_vlm_timing_annotations = 0` while retaining `timing_diagram_extractions = 1`.
- This strengthens the active-low reset-release VLM fixture by proving polarity-aware temporal evidence does not leak into handshake semantic-role hints.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench vlm_timing_active_low_deassertion_equivalence_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- The fixture now asserts `timing_diagram_extractions = 1` alongside the existing VLM timing-annotation semantic hint.
- This strengthens the VLM timing semantic-grounding gold from semantic recovery alone to evidence-stage extraction plus downstream ready-like role recovery.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench vlm_timing_semantic_grounding_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` last passed on slice 50 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
- Tightened `crates/specforge/test_data/kg_quality/vlm_state_machine_duplicate_initial_gold/fixture.json`.
- The fixture now excludes `semantic_state_machine_initial_cardinality` and `intent_state_machine_initial_cardinality` while retaining `initial_regular_states = 1`.
- This strengthens the duplicate-initial positive from exact merge shape to explicit absence of the warning path used by the missing/multiple-initial negatives.
...
- New batch slice 57/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/vlm_state_machine_label_noise_negative/fixture.json`.
- The fixture now asserts `IDLE` remains the only canonical initial state and `initial_regular_states` stays `1` at `SemanticIR` and `IntentIR` while noisy prose/OCR labels remain excluded.
- This strengthens the VLM state-machine label-noise negative from state/transition filtering to exact initial-state preservation as well.
...
- New batch slice 56/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/vlm_state_machine_undeclared_transition_negative/fixture.json`.
- The fixture now asserts `IDLE` remains the only canonical initial state and `initial_regular_states` stays `1` at `SemanticIR` and `IntentIR` while undeclared `DONE`/`RESET` transition endpoints remain excluded.
- This strengthens the VLM state-machine endpoint-grounding negative from state/transition filtering to exact initial-state preservation as well.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench vlm_state_machine_undeclared_transition_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` last passed on slice 50 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
...
- The fixture now asserts exact validation payloads for `semantic_state_machine_initial_cardinality` and `intent_state_machine_initial_cardinality`: severity `warning`, category `state_machine`, summary text requiring exactly one initial state, and related ids `IDLE`/`BUSY`.
- This strengthens the missing-initial VLM state-machine negative from finding-id presence to stage-specific warning details while preserving the inspectable state graph.
...
- New batch slice 53/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/visual_motif_prior_guided_diagram_classification_gold/fixture.json`.
- The fixture now asserts the exact validation payload for `evidence_visual_motif_corroboration_guidance`: severity `info`, category `rescan_guidance`, summary text proving prior memory did not rewrite SourceIR or create semantic facts, and related id `visual_0001`.
- This strengthens the visual-motif prior-guided gold path from finding-id presence to actionable VLM/multimodal corroboration details.
...
- New batch slice 52/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts exact validation payloads for non-decisive semantic-role arbitration, role-arbitration rescan guidance, and unresolved semantic-conflict rescan guidance at `SemanticIR` and `IntentIR` when a modality-reliability prior has insufficient support.
- This strengthens the weak-prior guard from finding-id presence to actionable `XCTRL`/`semantic_conflict_0001` validation details while continuing to exclude prior-guided arbitration and consensus findings.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts exact validation payloads for non-decisive semantic-role arbitration, role-arbitration rescan guidance, and unresolved semantic-conflict rescan guidance at `SemanticIR` and `IntentIR` when an APB-scoped modality-reliability prior is staged against an AXI-local document.
- This strengthens the protocol-family mismatch guard from finding-id presence to actionable `XCTRL`/`semantic_conflict_0001` validation details while continuing to exclude prior-guided arbitration and consensus findings.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- Broader `bash scripts/run_ci.sh` passed for slice 50 with formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.

## 2026-05-07 KG fixture for no-prior modality finding payloads
- New batch slice 49/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts exact validation payloads for non-decisive semantic-role arbitration, role-arbitration rescan guidance, and unresolved semantic-conflict rescan guidance at `SemanticIR` and `IntentIR` when no modality-reliability prior is staged.
- This strengthens the no-prior semantic-modality mirror from finding-id presence to actionable `XCTRL`/`semantic_conflict_0001` validation details while continuing to exclude prior-guided arbitration and consensus findings.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 48/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts exact validation payloads for prior-guided semantic arbitration, consensus, and rescan-guidance findings at `SemanticIR` and `IntentIR` when AMBA-generic modality-reliability memory guides an AXI-family document.
- This strengthens the AMBA-generic semantic-modality fallback from finding-id presence to signal-related validation detail parity with exact-family memory.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 47/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts `XCTRL` still enters semantic arbitration and remains explicitly non-decisive at `SemanticIR` and `IntentIR` while the unrelated APB-scoped negative-knowledge prior emits no prior-match guidance.
- This strengthens the protocol-family mismatch guard from conflict shape and validation metrics to exact arbitration state as well.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 46/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_guided_semantic_conflict_caution_gold/fixture.json`.
- The fixture now asserts the exact `XCTRL` semantic-conflict observations at `SemanticIR` and `IntentIR`: visual-caption `handshake_valid_like` evidence remains in conflict with VLM timing-diagram annotation `handshake_ready_like` evidence.
- This strengthens the prior-guided negative-knowledge semantic-conflict caution path from conflict count and validation guidance to exact local conflict shape preservation.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_guided_semantic_conflict_caution_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 45/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/actor_taxonomy_prior_guided_section_direction_gold/fixture.json`.
- The fixture now asserts per-signal graph-backed direction coverage for `XADDR`, `XCMD`, and `XRESP` at `SemanticIR` and `IntentIR` when learned `Issuer` and `Acceptor` actor-taxonomy priors are staged.
- This strengthens the prior-guided actor-taxonomy positive path from aggregate direction metrics and actor relations to exact graph-direction coverage.
...
- New batch slice 44/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/actor_taxonomy_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts per-signal graph-backed direction absence for `XADDR` at `SemanticIR` and `IntentIR` when an APB-scoped `issuer` actor-taxonomy prior is staged against an AXI-local section heading.
- This strengthens the actor-taxonomy protocol-family mismatch guard from aggregate direction metrics to exact graph-direction absence.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench actor_taxonomy_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 43/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/actor_taxonomy_prior_guided_section_direction_without_prior_negative/fixture.json`.
- The fixture now asserts per-signal graph-backed direction absence for `XADDR`, `XCMD`, and `XRESP` at `SemanticIR` and `IntentIR` while preserving the local signal inventory.
- This strengthens the no-prior actor-taxonomy section-heading mirror from aggregate direction metrics to exact graph-direction absence.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench actor_taxonomy_prior_guided_section_direction_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 42/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/table_shape_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts zero table-signal declaration provenance at `EvidenceIR` when an APB-scoped table-shape prior is staged against an AXI-local unknown `Name | Direction | Width` table.
- This strengthens the protocol-family mismatch guard from canonical inventory absence to earliest-stage provenance absence as well.
...
- The fixture now asserts zero table-signal declaration provenance at `EvidenceIR` and excludes `XREQ`/`XACK` from canonical `SemanticIR` and `IntentIR` signal inventory when the local table shape is still `unknown` and no learned table-shape prior is staged.
- This strengthens the no-prior table-shape signal-table path from metric-only absence to exact provenance and inventory absence.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench table_shape_prior_guided_signal_table_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- The fixture now asserts the table-derived `XREQ` output direction at `SemanticIR` and `IntentIR` while the absent visual-motif prior leaves the local `XREQ cycle trace` diagram ambiguous and non-semantic.
- This strengthens the no-prior visual-motif mirror from visual-classification absence to local signal-shape preservation.
...
- New batch slice 37/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/visual_semantic_prior_guided_caption_gold/fixture.json`.
- The fixture now asserts the local visual-channel `XACK` input direction at `SemanticIR` and `IntentIR` alongside the visual-caption prior-guided `handshake_ready_like` role and `single_source` grounding.
- This strengthens the positive visual-caption semantic phrase path from role/grounding exactness to local signal-shape preservation as well.
...
- New batch slice 36/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/visual_semantic_prior_guided_caption_without_prior_negative/fixture.json`.
- The fixture now asserts the local visual-channel `XACK` input direction at `SemanticIR` and `IntentIR` while the absent visual-caption prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens the no-prior visual-caption mirror from semantic absence to local signal-shape preservation.
...
## 2026-05-07 KG fixture for prior-guided receive signal shape
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts the local prose `XACK` input direction at `SemanticIR` and `IntentIR` alongside the prior-guided `handshake_ready_like` role and `single_source` grounding.
- This strengthens the positive `can receive the transfer` semantic phrase path from role/grounding exactness to local signal-shape preservation as well.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for prior-guided ready-sink signal shape
...
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts the local prose `XACK` input direction at `SemanticIR` and `IntentIR` alongside the prior-guided `handshake_ready_like` role and `single_source` grounding.
- This strengthens the positive ready-sink semantic phrase path from role/grounding exactness to local signal-shape preservation as well.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 33/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts the local prose `XREQ` output direction at `SemanticIR` and `IntentIR` alongside the prior-guided `handshake_valid_like` role and `single_source` grounding.
- This strengthens the positive valid-like semantic phrase path from role/grounding exactness to local signal-shape preservation as well.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 32/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_valid_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts the local prose `XREQ` output direction at `SemanticIR` and `IntentIR` while the absent prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens the no-prior mirror for `can publish the beat` from semantic absence to local signal-shape preservation.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_valid_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for no-prior ready-sink signal shape
- New batch slice 31/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_ready_sink_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts the local prose `XACK` input direction at `SemanticIR` and `IntentIR` while the absent prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens the no-prior mirror for `can sink the transfer` from semantic absence to local signal-shape preservation.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_ready_sink_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 30/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_without_prior_negative/fixture.json`.
- The fixture now asserts the local prose `XACK` input direction at `SemanticIR` and `IntentIR` while the absent prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens the no-prior mirror for `can receive the transfer` from semantic absence to local signal-shape preservation.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 29/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_conflicting_roles_negative/fixture.json`.
- The fixture now asserts the local prose `XCTRL` input direction at `SemanticIR` and `IntentIR` while the conflicting valid-like and ready-like phrase priors still leave the semantic role, consensus, and candidate surfaces empty.
- This strengthens conflicting-prior fail-closed coverage from semantic absence to local signal-shape preservation.
...
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff after rerunning a hung first attempt.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.

## 2026-05-07 KG fixture for broad semantic phrase signal shape
- New batch slice 28/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_broad_phrase_negative/fixture.json`.
- The fixture now asserts the local prose `XFLOW` output direction at `SemanticIR` and `IntentIR` while the one-token broad phrase prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens broad-prior rejection from semantic absence to local signal-shape preservation.
...
- New batch slice 27/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_phrase_mismatch_negative/fixture.json`.
- The fixture now asserts the local prose `XFLOW` output direction at `SemanticIR` and `IntentIR` while the unmatched learned phrase prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens exact-phrase mismatch coverage from semantic absence to local signal-shape preservation.
...
- New batch slice 26/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts the local prose `XSTAGE` output direction at `SemanticIR` and `IntentIR` while the visual-caption-only semantic phrase prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens the source-kind mismatch guard from semantic absence to exact local signal-shape preservation.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 25/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts the local AXI `XREQ` output direction at `SemanticIR` and `IntentIR` while the unrelated APB semantic phrase prior still leaves the semantic role, consensus, and candidate surfaces empty.
- This strengthens the protocol-family mismatch guard from semantic absence to exact local signal-shape preservation.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_ci.sh` passed as a slice-25 broader gate: formatting, Clippy warning-deny, `614` Rust tests, rustdoc warning-deny, and mdBook all passed.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.

## 2026-05-07 KG fixture for semantic phrase resolved shape
- New batch slice 24/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_prior_guided_phrase_gold/fixture.json`.
- The fixture now asserts exact `XACK` semantic role and grounding at `SemanticIR` and `IntentIR`: the prior-guided `can receive the transfer` phrase resolves `handshake_ready_like` with `single_source` grounding.
- This aligns the older semantic phrase prior gold with the newer exact role/grounding expectation surface already used by the ready-sink phrase fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_prior_guided_phrase_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 23/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_amba_generic_fallback_gold/fixture.json`.
- The fixture now asserts exact `XCTRL` resolution at `SemanticIR` and `IntentIR`: AMBA-generic prior-guided arbitration resolves `handshake_valid_like`, canonical grounding strength remains `single_source`, and the table/prose semantic-conflict observations stay explicit.
- This locks the generic-fallback positive path without overstating current-document grounding.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_amba_generic_fallback_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 22/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_gold/fixture.json`.
- The fixture now asserts exact `XCTRL` resolution at `SemanticIR` and `IntentIR`: prior-guided arbitration resolves `handshake_valid_like`, canonical grounding strength remains `single_source`, and the table/prose semantic-conflict observations stay explicit.
- This confirms semantic-modality memory guides arbitration without converting current-document grounding into cross-modality support.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_gold` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 21/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_prior_guided_conflict_without_prior_negative/fixture.json`.
- The fixture now asserts exact `XCTRL` semantic-conflict observations at `SemanticIR` and `IntentIR`: signal-description-table valid-like evidence stays contested with prose ready-like evidence.
- With no modality-reliability prior staged, arbitration remains non-decisive and no prior-guided consensus appears.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_prior_guided_conflict_without_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 20/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_weak_prior_negative/fixture.json`.
- The fixture now asserts exact `XCTRL` semantic-conflict observations at `SemanticIR` and `IntentIR`: signal-description-table valid-like evidence stays contested with prose ready-like evidence.
- The weak semantic-modality reliability prior still does not create prior-guided arbitration, consensus, or a resolved role.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_weak_prior_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality source-kind mismatch shape
- New batch slice 19/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_source_kind_mismatch_negative/fixture.json`.
- The fixture now asserts exact `XCTRL` semantic-conflict observations at `SemanticIR` and `IntentIR`: signal-description-table valid-like evidence stays contested with prose ready-like evidence.
- The visual-only semantic-modality reliability prior still does not create prior-guided arbitration, consensus, or a resolved role for table/prose evidence.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_source_kind_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for semantic-modality mismatch conflict shape
- New batch slice 18/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/semantic_modality_reliability_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts exact `XCTRL` semantic-conflict observations at `SemanticIR` and `IntentIR`: signal-description-table valid-like evidence stays contested with prose ready-like evidence.
- The unrelated APB semantic-modality reliability prior still does not create prior-guided arbitration, consensus, or a resolved role.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench semantic_modality_reliability_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
## 2026-05-07 KG fixture for negative-knowledge mismatch conflict shape
- New batch slice 17/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/negative_knowledge_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts exact `XCTRL` semantic-conflict observations at `SemanticIR` and `IntentIR`: visual-caption valid-like evidence stays contested with VLM timing-annotation ready-like evidence.
- The unrelated APB negative-knowledge prior still produces no prior matches, rescan recommendations, or corroboration requirements.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench negative_knowledge_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- New batch slice 16/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/visual_motif_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts that the AXI-local `XREQ` signal remains table-backed with output direction while an unrelated APB visual-motif prior stays silent.
- It also excludes semantic candidates and semantic consensus for `XREQ`, so the family-mismatch guard covers both visual classification silence and downstream semantic silence.
...
- Tightened `crates/specforge/test_data/kg_quality/table_shape_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts that the AXI-local unknown table rows `XREQ` and `XACK` stay out of canonical signal inventory and graph-backed direction coverage when only an unrelated APB table-shape prior is staged.
- This upgrades the family-mismatch guard from direction-count checks to explicit no-inventory/no-graph false-positive control.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench table_shape_prior_protocol_family_mismatch_negative` passed with the requested fixture.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
...
- Tightened `crates/specforge/test_data/kg_quality/temporal_prior_protocol_family_mismatch_negative/fixture.json`.
- The fixture now asserts exact AXI-local `XREADY` temporal rule shape at `SemanticIR` and `IntentIR` while proving the unrelated APB temporal phrase prior does not supply a cycle window.
- Clock-grounding replay findings are excluded, so the family-mismatch guard preserves only cycle-window replay guidance for the missing bound.
...
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `bash scripts/run_ci.sh` also passed on the clean post-slice-13 baseline (`ab854acee98ffebc1fb928d1e321b4068cb2a578`) before this slice started.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- Tightened `crates/specforge/test_data/kg_quality/temporal_prior_guided_cycle_window_without_prior_negative/fixture.json`.
- The fixture now excludes clock-grounding replay findings while preserving cycle-window replay guidance for the locally clocked no-prior `one beat later` rule.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- Tightened `crates/specforge/test_data/kg_quality/temporal_prior_guided_cycle_window_gold/fixture.json`.
- The fixture now excludes cycle-window and clock-grounding replay findings after the prior recovers the one-cycle window and local clock grounding for `one beat later`.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 11/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/temporal_prior_guided_cycle_window_without_prior_negative/fixture.json`.
- The fixture now asserts the exact no-prior temporal rule shape at `SemanticIR` and `IntentIR`: local `clk` rising-edge grounding, asserted-value predicate, and supporting statement id while the cycle window remains absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 10/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/temporal_prior_guided_cycle_window_gold/fixture.json`.
- The fixture now asserts exact prior-guided temporal rule shape at `SemanticIR` and `IntentIR`: local `clk` rising-edge grounding, one-cycle window, asserted-value predicate, and supporting statement id.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 9/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/temporal_cycle_window_grounded_gold/fixture.json`.
- The fixture now excludes all three temporal replay lanes for a fully grounded bounded rule: cycle-window, actor-grounding, and clock-grounding guidance stay absent at `SemanticIR` and `IntentIR`.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 8/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/temporal_cycle_window_surface_negative/fixture.json`.
- The fixture now asserts the exact unbounded temporal rule shape at `SemanticIR` and `IntentIR`: `PCLK` rising-edge grounding, `Completer` actor-drive predicate, asserted-value predicate, and supporting statement id while the cycle window remains absent.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 7/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/temporal_actor_grounding_surface_negative/fixture.json`.
- The fixture now asserts the exact actorless temporal rule shape at `SemanticIR` and `IntentIR`: `clk` rising-edge grounding, `within 2 cycles` window, asserted-value predicate, and supporting statement id.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- Tightened `crates/specforge/test_data/kg_quality/temporal_clock_grounding_surface_negative/fixture.json`.
- The fixture now asserts the exact clockless temporal rule shape at `SemanticIR` and `IntentIR`: unknown edge, `within 2 cycles` window, `Subordinate` actor-stability predicate, signal-stability predicate, and supporting statement id.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- Tightened `crates/specforge/test_data/kg_quality/temporal_clock_grounding_surface_negative/`.
- The fixture now proves a bounded, actor-grounded temporal rule emits clock-grounding replay guidance while excluding cycle-window and actor-grounding replay findings because those surfaces are already present.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 4/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/temporal_actor_grounding_surface_negative/fixture.json`.
- The fixture now proves a bounded, clock-grounded temporal rule emits actor-grounding replay guidance while excluding cycle-window and clock-grounding replay findings because those surfaces are already present.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `cargo run --manifest-path Cargo.toml -p specforge -- corpus-kb --kg-fixtures-root crates/specforge/test_data/kg_quality` refreshed tracked corpus-KB projections for `150` fixtures with `0` failures and no tracked projection diff.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- New batch slice 3/200 uses active `BWFSC=200`; push remains deferred until all 200 slices complete unless a documented blocker stops the batch.
- Tightened `crates/specforge/test_data/kg_quality/temporal_cycle_window_surface_negative/fixture.json`.
- The fixture now proves the same unbounded rule emits cycle-window replay guidance while excluding actor-grounding and clock-grounding replay findings because actor and clock grounding are already present.
- `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `150/150` tracked fixtures.
- `bash scripts/run_docs_ci.sh` passed after the live-book sync.
- `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is still active.
...
- This locks conflicting system-contract width evidence against stale DT-root output while clock/reset provenance and system-surface repair guidance remain visible.
- Focused system-contract width coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.

## 2026-05-06 `.fsm` system-contract direction renderable block
- New batch slice 192/200 updates `crates/specforge/src/ir/adapters.rs`.
- `standalone_sequential_dt_blocks_conflicting_system_contract_signal_direction` now asserts standalone sequential clock/reset direction conflicts leave no renderable module and no aggregate `.fsm` source document.
- This locks conflicting system-contract direction evidence against stale DT-root output while clock/reset provenance and system-surface repair guidance remain visible.
- Focused system-contract direction coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks conflicting canonical direction evidence against stale DT-root output while graph-backed actor provenance and signal-inventory repair guidance remain visible.
- Focused direct flat-conflict coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks structured-FSM flat/graph direction disagreements against stale `?fsm` output before canonical and graph-backed direction evidence agree.
- Focused structured-FSM flat/graph coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks explicit-module width conflicts against stale `(+size ...)` output before conflicting actor-port width evidence is resolved.
- Focused explicit-module width-conflict coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.

## 2026-05-06 `.fsm` undeclared target renderable block
- New batch slice 181/200 updates `crates/specforge/src/ir/adapters.rs`.
- `keeps_structured_fsm_blocked_when_transition_target_is_undeclared` now asserts structured FSMs with a transition targeting an undeclared state leave the aggregate `.fsm` artifact without a renderable source document.
- This locks undeclared-target state-graph blockers against stale `?fsm` output before every transition target is declared.
- Focused undeclared-target coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks child actor-port direction conflicts against stale child module output before actor-relative graph direction evidence is resolved.
- Focused child actor-port direction coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks conflicting top-boundary actor-port direction evidence against stale `?top` output before the top port direction conflict is resolved.
- Focused top actor-port direction coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.

## 2026-05-06 `.fsm` dedicated child role renderable block
- New batch slice 174/200 updates `crates/specforge/src/ir/adapters.rs`.
- The dedicated `top_composition_blocks_child_source_direction_role_guidance` and `top_composition_blocks_child_target_direction_role_guidance` tests now assert child endpoint direction-role mismatches leave the selected top without a renderable top root and the aggregate `.fsm` artifact without a renderable source document.
- This explicitly anchors child-side source/target role blocker coverage in the child endpoint tests; the earlier top-boundary role assertion slices remain covered by the top-port role tests they changed.
- Focused child-source and child-target role coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.

## 2026-05-06 `.fsm` child target role renderable block
- New batch slice 173/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_child_target_direction_role_guidance` now asserts child target endpoint direction-role mismatches leave the selected top without a renderable top root and the aggregate `.fsm` artifact without a renderable source document.
- This locks the child-target direction-role blocker against stale `?top` output before endpoint direction evidence is made consistent with top-link target semantics.
- Focused child-target role coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.

## 2026-05-06 `.fsm` child source role renderable block
- New batch slice 172/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_child_source_direction_role_guidance` now asserts child source endpoint direction-role mismatches leave the selected top without a renderable top root and the aggregate `.fsm` artifact without a renderable source document.
- This locks the child-source direction-role blocker against stale `?top` output before endpoint direction evidence is made consistent with top-link source semantics.
- Focused child-source role coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks the missing top-link source endpoint blocker against stale `?top` output before the source top port is declared and emitted.
- Focused undeclared top-source coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.

## 2026-05-06 `.fsm` undeclared top target renderable block
- New batch slice 168/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_link_to_undeclared_top_target_guidance` now asserts top links targeting an undeclared top-boundary port leave the selected top without a renderable top root and the aggregate `.fsm` artifact without a renderable source document.
- This locks the missing top-link target endpoint blocker against stale `?top` output before the target top port is declared and emitted.
- Focused undeclared top-target coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks the missing-top-port composition blocker against stale `?top` output before top-port records are supplied.
- Focused top-without-port coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.
...
- This locks the no-link composition blocker against stale `?top` output before topology links are supplied.
- Focused multi-child no-link coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred because `target/release/tool_matrix` is active.

## 2026-05-06 `.fsm` widthless top-port renderable block
- New batch slice 163/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_widthless_top_port_without_width_recovery` now asserts a public top port with no numeric width recovery has no renderable top root and no aggregate renderable source document.
- This locks the widthless top-boundary blocker against stale `?top` output before width evidence is recovered.
- Focused widthless top-port coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred until no target-tree process is active.
...
- This locks the direct child/top width-conflict path against stale child `(+size ...)` output after renderability is blocked.
- Focused child topology-width conflict coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred until no target-tree process is active.
...
- This locks sibling child-link width recovery through child target emission, not just module inventory: `input_data` lowers as an input width-8 size entry while its width originates from the producer-to-consumer topology link.
- Focused sibling child-link width size-entry coverage, the adapter suite, fmt, docs CI, KG bench (`148/148` fixtures), and full CI passed; user-requested `cargo sweep --time 1` remains deferred until no target-tree process is active.

## 2026-05-06 `.fsm` top-link child-width size-entry lock
- New batch slice 154/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_width_from_top_link_topology` now asserts the producer child renderable `(+size ...)` entry for `output_data` consumes width recovered from top-link topology.
- This locks child top-link width recovery through child target emission, not just module inventory: `output_data` lowers as an output width-8 size entry while its width originates from the top boundary link.
- Focused top-link child-width size-entry coverage, the adapter suite, fmt, docs CI, KG bench (`148/148` fixtures), and full CI passed; user-requested `cargo sweep --time 1` remains deferred until no target-tree process is active.

## 2026-05-06 `.fsm` top child topology direction size-entry lock
- New batch slice 153/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_directions_from_link_topology` now asserts child renderable `(+size ...)` entries consume graph-backed link-topology directions for `producer_core.output_data`, `consumer_core.input_data`, and `consumer_core.result_data`.
- This locks top-composition child link-topology direction recovery through child target emission, not just module inventory: recovered child ports lower with output/input/output roles and width 8 while flat child-module direction hints remain absent.
- Focused top-composition child link-topology direction size-entry coverage, the adapter suite, fmt, docs CI, KG bench (`148/148` fixtures), and full CI passed; user-requested `cargo sweep --time 1` remains deferred until no target-tree process is active.
...
- This locks direct-DT control-input width recovery through target emission, not just signal inventory: `DATA_IN` remains flat-direction-free but still lowers as an input width-8 size entry.
- Focused direct-DT control-input width size-entry coverage, the adapter suite, fmt, docs CI, KG bench (`148/148` fixtures), and full CI passed; user-requested `cargo sweep --time 1` remains deferred until no target-tree process is active.
...
- This locks `DATA_IN`, `GO`, and `DONE` as renderable input size entries backed by structured FSM control-read recovery rather than flat compatibility hints.
- Focused structured-FSM graph-backed size-entry coverage, the adapter suite, fmt, docs CI, KG bench, and full CI passed; user-requested `cargo sweep --time 1` remains deferred until no target-tree process is active.
...
- This keeps source-endpoint enrichment guidance, top-port, child, link provenance, and renderable-child residual context tied together.
- Focused unemitted child source guidance residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` child actor-port direction residual diagnostics
- New batch slice 143/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_conflicting_actor_port_directions` now asserts child actor-port direction conflicts keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps actor-relative graph-direction repair guidance, explicit child signal and graph actor-port support IDs, and renderable-child residual context tied together.
- Focused child actor-port direction residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` child topology direction residual diagnostics
- New batch slice 142/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_conflicting_child_link_topology_directions` now asserts child link-topology direction conflicts keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps actor-relative graph-direction repair guidance, contradictory topology-link support IDs, and renderable-child residual context tied together.
- Focused child topology direction residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` top-link width mismatch residual diagnostics
- New batch slice 141/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_width_mismatched_top_link_guidance` now asserts top-link endpoint width mismatches keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps width-compatible top-link repair guidance, mismatched topology-link support IDs, and explicit-top-port residual context tied together.
- Focused top-link width mismatch residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` top child-link width residual diagnostics
- New batch slice 140/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_conflicting_top_port_widths_from_child_links` now asserts child links that conflict on top-boundary width keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps top-boundary width-conflict repair guidance, both conflicting topology-link support-ID sets, and explicit-top-port residual context tied together.
- Focused top child-link width residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps width-conflict repair guidance, signal-declaration and topology-link support IDs, and renderable-child residual context tied together.
- Focused child topology width residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` undeclared top-source residual diagnostics
- New batch slice 137/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_link_from_undeclared_top_source_guidance` now asserts top links originating from undeclared top-boundary ports keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps source-endpoint enrichment guidance, top-port, child, link provenance, and explicit-top-port residual context tied together.
- Focused undeclared top-source residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps child-instance deduplication guidance, both child declaration support-ID sets, and child-module reference residual context tied together.
- Focused duplicate child-instance residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` no-top-port residual diagnostics
- New batch slice 134/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_top_without_port_guidance` now asserts top roots without explicit top-port records keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps top-port enrichment guidance, child declaration provenance, resolved child root kind, and explicit-top-port residual guidance tied together.
- Focused no-top-port residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` no-child top residual diagnostics
- New batch slice 133/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_top_without_child_guidance` now asserts top roots without child-module references keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps child-module enrichment guidance, declared top-port provenance, and child-module reference residual guidance tied together.
- Focused no-child top residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps duplicate top-port declaration blocking, top-boundary direction-conflict guidance, duplicate support-ID sets, and explicit-top-port residual guidance tied together.
- Focused duplicate top-direction residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps child-module flat/graph direction-role guidance, top and aggregate endpoint-role guidance, top-port/child/link provenance, and renderable-child residual guidance tied together.
- Focused target-side child role residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` child source role residual diagnostics
- New batch slice 127/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_child_source_direction_role_guidance` now asserts source-side child endpoint direction-role blockers keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps child-module graph-direction role guidance, top and aggregate endpoint-role guidance, top-port/child/link provenance, and renderable-child residual guidance tied together.
- Focused source-side child role residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` top-target role residual diagnostics
- New batch slice 126/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_top_target_direction_role_guidance` now asserts public top targets declared with the wrong boundary direction keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps top-boundary direction-conflict guidance, endpoint-role guidance, selected top signal-inventory evidence, and renderable-child residual guidance tied together.
- Focused top-target role residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` top-link direction residual diagnostics
- New batch slice 125/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_keeps_conflicting_top_port_direction_unresolved` now asserts top-link-derived top-boundary direction conflicts keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps boundary direction-conflict guidance, endpoint-role guidance, top-port/topology-link provenance, and structured residual guidance tied together.
- Focused top-link direction residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps top-boundary direction-conflict guidance, top-port and graph actor-port provenance, and explicit-top-port residual guidance tied together.
- Focused top actor-port direction residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` recovered top-port direction residual diagnostics
- New batch slice 122/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_preserves_recovered_top_port_direction_when_still_blocked` now asserts blocked recovered top-port direction cases keep low-confidence `fsm_adapter_composition_topology` diagnostics with explicit topology-detail blocker text.
- This keeps topology-recovered direction provenance, child-source declaration guidance, and child-module residual guidance tied together.
- Focused recovered top-port direction residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps source-endpoint repair guidance, preserved top-link support/confidence, and renderable-child residual guidance tied together.
- Focused source-side unemitted child residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps missing-width blocker text, top-boundary width recovery guidance, preserved top-port support/confidence, and explicit-top-ports repair guidance tied together.
- Focused widthless top-port residual-diagnostics regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps DT control-fragment support IDs, referenced signal inventory, high DT candidate confidence, missing-contract blocker text, and system-surface repair guidance tied together.
- Focused missing-system-contract residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` system-contract flat/graph residual guidance
- New batch slice 113/200 updates `crates/specforge/src/ir/adapters.rs`.
- `standalone_sequential_dt_blocks_system_contract_flat_graph_direction_disagreement` now asserts clock flat/graph direction disagreements keep the `fsm_adapter_system_contract` residual packet with low confidence and the upstream system-surface enrichment candidate.
- This keeps conflicting system/actor-port direction evidence, graph support IDs, high signal confidence, exact flat/graph conflict text, and system-surface repair guidance tied together.
- Focused system-contract flat/graph residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps system-contract support IDs, high signal confidence, exact clock-direction conflict text, and system-surface repair guidance tied together.
- Focused system-contract direction residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` explicit-module width residual guidance
- New batch slice 110/200 updates `crates/specforge/src/ir/adapters.rs`.
- `standalone_explicit_module_blocks_conflicting_control_input_actor_port_widths` now asserts explicit-module actor-port width conflicts keep the `fsm_adapter_signal_inventory` residual packet with low confidence and the upstream interface-inventory enrichment candidate.
- This keeps both graph-backed width support IDs, high automation confidence, blocked module/root renderability, and signal-inventory repair guidance tied together.
- Focused explicit-module width-conflict residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` explicit-module control-read residual guidance
- New batch slice 109/200 updates `crates/specforge/src/ir/adapters.rs`.
- `standalone_explicit_module_blocks_conflicting_module_control_read_direction` now asserts explicit-module control-read direction conflicts keep the `fsm_adapter_signal_inventory` residual packet with low confidence and the upstream interface-inventory enrichment candidate.
- This keeps actor-port/control-read categories, unresolved conflicting graph direction, blocked module/root renderability, and signal-inventory repair guidance tied together.
- Focused explicit-module control-read residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` explicit-module flat/graph residual guidance
- New batch slice 108/200 updates `crates/specforge/src/ir/adapters.rs`.
- `standalone_explicit_module_blocks_flat_graph_direction_disagreement` now asserts explicit-module flat/graph direction disagreements keep the `fsm_adapter_signal_inventory` residual packet with low confidence and the upstream interface-inventory enrichment candidate.
- This keeps module interface direction, graph-backed actor direction, graph support provenance, high inventory confidence, blocked renderability, and signal-inventory repair guidance tied together.
- Focused explicit-module flat/graph residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` structured flat/graph residual guidance
- New batch slice 107/200 updates `crates/specforge/src/ir/adapters.rs`.
- `structured_fsm_blocks_flat_graph_direction_disagreement` now asserts structured-FSM flat/graph direction disagreements keep the `fsm_adapter_signal_inventory` residual packet with low confidence and the upstream interface-inventory enrichment candidate.
- This keeps explicit interface direction, graph-backed actor direction, graph support provenance, high inventory confidence, blocked renderability, and signal-inventory repair guidance tied together.
- Focused structured flat/graph residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.

## 2026-05-06 `.fsm` undeclared-transition residual guidance
- New batch slice 106/200 updates `crates/specforge/src/ir/adapters.rs`.
- `keeps_structured_fsm_blocked_when_transition_target_is_undeclared` now asserts undeclared transition-target blockers keep the `fsm_adapter_state_graph` residual packet with low confidence and the upstream state-graph enrichment candidate.
- This keeps declared state support IDs, rejected transition support IDs, high candidate confidence, exact undeclared-target blocker text, and state-graph repair guidance tied together.
- Focused undeclared-transition residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps graph-backed structured-FSM output provenance, high inventory confidence, exact FSM-state undriven-output blocker text, and low-confidence signal-inventory repair guidance tied together.
- Focused structured undriven output residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps direct-root flat direction conflicts and same-actor width conflicts tied to their signal-inventory repair guidance without mixing the regression names.
- Focused flat direction conflict residual-guidance regression, focused width conflict residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps conflicting actor-port support IDs, high inventory confidence, the blocked renderability reason, and low-confidence signal-inventory repair guidance tied together.
- Focused actor direction conflict residual-guidance regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps direct-control-input provenance, actor-port-width provenance, graph support IDs, high automation confidence, emitted width text, and signal-inventory residual cleanliness paired once the direct root is renderable.
- Focused direct actor-port width residual-clean regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps selected output actor support IDs, excluded external actor support, high automation confidence, emitted DT assignments, and signal-inventory residual cleanliness paired once the direct root is renderable.
- Focused output actor selection residual-clean regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps producer and consumer child signal declaration support, topology-link width support, selected child inventories, emitted size text, and aggregate residual cleanliness paired once the top is renderable.
- Focused transitive child-width recovery residual-clean regression, adapter suite, fmt, docs, KG bench, and full CI all passed; user-requested `cargo sweep --time 1` is deferred until no target-tree process is active.
...
- This keeps child signal declaration support, topology-link width support, selected consumer inventory, emitted size text, and aggregate residual cleanliness paired once the top is renderable.
- Focused sibling-link child-input width recovery residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` top-link child-width residual cleanliness
- New batch slice 89/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_width_from_top_link_topology` now asserts no stale composition topology residual remains after top-link topology evidence recovers a child output width and the top emits.
- This keeps child signal declaration support, topology-link width support, selected child inventory, emitted size text, and aggregate residual cleanliness paired once the top is renderable.
- Focused top-link child-width recovery residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` child link-direction residual cleanliness
- New batch slice 88/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_directions_from_link_topology` now asserts no stale composition topology residual remains after topology-link evidence recovers child directions and the top emits.
- This keeps child signal declaration support, topology-link support, selected child inventories, emitted topology text, and aggregate residual cleanliness paired once the top is renderable.
- Focused child link-topology direction recovery residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` child actor direction residual cleanliness
- New batch slice 87/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_directions_from_actor_ports` now asserts no stale composition topology residual remains after graph actor-port evidence recovers child directions and the top emits.
- This keeps child signal declaration support, graph actor-port support, selected child inventories, emitted topology text, and aggregate residual cleanliness paired once the top is renderable.
- Focused child actor-port direction recovery residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This keeps top-port declaration support, topology-link width support, selected inventory provenance, emitted topology text, and aggregate residual cleanliness paired once the top is renderable.
- Focused child-link top-width recovery residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` actor-port width residual cleanliness
- New batch slice 84/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_top_port_width_from_actor_ports` now asserts no stale composition topology residual remains after graph actor-port evidence recovers a numeric public top-port width and the top emits.
- This keeps top-port declaration support, graph actor-port width support, selected inventory provenance, and emitted `.fsm` text paired with a clean aggregate residual surface once the top is renderable.
- Focused actor-port width recovery residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` actor-port direction residual cleanliness
- New batch slice 83/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_top_port_direction_from_actor_ports` now asserts no stale composition topology residual remains after graph actor-port evidence recovers a public top-port direction and the top emits.
- This keeps top-port declaration support, graph actor-port support, selected inventory provenance, and emitted `.fsm` text paired with a clean aggregate residual surface once the top is renderable.
- Focused actor-port direction recovery residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This keeps per-output top-port, topology-link, and per-instance `?fsmc` provenance paired with a clean aggregate residual surface once the composition is renderable.
- Focused reused-FSM-child residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This keeps child, top-port, topology-link, and top-first emission provenance paired with a clean aggregate residual surface once the composition is renderable.
- Focused top-before-child residual-clean regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- The test also asserts no stale composition topology residual remains once the linked topology is renderable.
- Focused top-link confidence regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This pairs graph-backed top-port support-ID confidence with structured residual diagnostics for incomplete topology repair.
- Focused recovered top-root confidence regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This pairs child-module graph-direction conflict guidance with structured residual diagnostics while preserving child signal and graph actor-port support IDs.
- Focused child actor-port direction regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This pairs child-module canonical width-conflict guidance with structured residual diagnostics while preserving child signal and topology-link support IDs.
- Focused child topology width regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` sibling child-link width residual lock
- New batch slice 65/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_conflicting_sibling_child_link_widths` now asserts that sibling child-link width conflicts emit the composition topology residual.
- This pairs child-module canonical width-conflict guidance with structured residual diagnostics while preserving both conflicting topology-link support-ID sets and high automation confidence.
- Focused sibling child-link width regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This aligns the top-link direction conflict lane with structured residual diagnostics while preserving topology-link support IDs, graph-direction inventory state, role guidance, and confidence.
- Focused top-link direction regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` top actor-width residual lock
- New batch slice 62/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_conflicting_top_actor_port_width` now asserts `fsm_adapter_composition_topology` and checks that its interpretation keeps explicit top-port repair visible.
- This pairs top-boundary width-conflict guidance with structured residual diagnostics while preserving explicit top-port and graph actor-port width support IDs.
- Focused top actor-port width regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` top actor-direction residual lock
- New batch slice 61/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_conflicting_top_actor_port_direction` now asserts `fsm_adapter_composition_topology` and checks that its interpretation keeps explicit top-port repair visible.
- This pairs top-boundary direction-conflict guidance with structured residual diagnostics while preserving explicit top-port and graph actor-port support IDs.
- Focused top actor-port direction regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.

## 2026-05-06 `.fsm` duplicate top-width residual lock
- New batch slice 60/200 updates `crates/specforge/src/ir/adapters.rs`.
- `top_composition_keeps_duplicate_top_port_width_conflict_unresolved` now asserts `fsm_adapter_composition_topology` and checks that its interpretation keeps explicit top-port repair visible.
- This pairs duplicate top-port deduplication and width-conflict guidance with structured residual diagnostics while preserving duplicate support IDs and confidence.
- Focused duplicate top-width regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This pairs duplicate child-instance deduplication guidance with structured residual diagnostics while preserving both child declaration support-ID sets.
- Focused duplicate child-instance regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This pairs the no-child top enrichment guidance with structured residual diagnostics while preserving top-port provenance.
- Focused no-child regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- This mirrors the target-side residual lock while keeping source-endpoint enrichment guidance, support IDs, and high-confidence topology-link provenance visible.
- Focused source-side regression, adapter suite, fmt, docs, KG bench, full CI, and the user-requested `cargo sweep --time 1` all passed.
...
- The residual interpretation mirrors the source-side lock by keeping upstream repair tied to renderable child modules while endpoint-role enrichment remains visible on child, top, and aggregate `.fsm` renderability.
- Full adapter coverage remains `92` adapter tests, full CI remains `613` Rust tests, and KG bench remains `148/148`.
...
- The residual interpretation keeps upstream repair centered on carrying explicit top ports, child-module references, renderable child modules, and width-compatible top links in canonical form.
- Full adapter coverage remains `92` adapter tests, full CI remains `613` Rust tests, and KG bench remains `148/148`.
...
- The regression keeps the conflicting top-port declaration, topology-link support IDs, selected top inventory evidence, blocked top candidate, and aggregate `.fsm` renderability visible for repair.
- Full adapter coverage is now `91` adapter tests, full CI is now `612` Rust tests, and KG bench remains `148/148`.
...
- `top_composition_blocks_child_target_direction_role_guidance` proves a child target declared as an output but used as a top-link target carries the role guidance through the blocked child module, blocked top candidate, aggregate `.fsm` renderability, and surviving topology provenance.
- Full adapter coverage is now `90` adapter tests, full CI is now `611` Rust tests, and KG bench remains `148/148`.
...
- The regression also proves the declared `result_data` top port, consumer child, and blocked topology link keep support IDs while the child source resolves as a renderable DT root.
- Full adapter coverage is now `87` adapter tests, full CI is now `608` Rust tests, and KG bench remains `148/148`.

## 2026-05-06 `.fsm` undeclared top-source guidance lock
- New batch slice 38/200 adds coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_link_from_undeclared_top_source_guidance` asserts top links that originate from an undeclared top-boundary port keep `declare and emit every top-link source endpoint before lowering ?top:name` guidance on the blocked top candidate and aggregate `.fsm` renderability.
- The regression also proves the declared `result_data` top port, consumer child, and blocked topology link keep support IDs while the child source resolves as a renderable DT root.
- Full adapter coverage is now `86` adapter tests, full CI is now `607` Rust tests, and KG bench remains `148/148`.
...
- The regression also proves child declaration support IDs survive, the child resolves as a renderable DT root, and no top port is fabricated for emission.
- Full adapter coverage is now `84` adapter tests, full CI is now `605` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove recovered `result_data` top-boundary direction evidence, topology-link support IDs, signal-inventory graph provenance, high automation confidence, and the missing-module blocker survive together.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove explicit top-port support IDs, high automation confidence, and selected top signal-inventory missing-width state remain distinct from symbolic and conflict states.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove explicit top-port support IDs, `graph_wrapper_ext_data`, high automation confidence, and selected top signal-inventory direction disagreement survive together.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove both conflicting child-link support-ID sets, high automation confidence, and selected top signal-inventory width-conflict state survive together.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove child signal declaration support IDs, topology-link support IDs, high automation confidence, and width-conflict inventory state survive together.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove actor-port support IDs, high automation confidence, selected child signal-inventory graph provenance, and emitted topology text.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-06 `.fsm` child topology-direction provenance lock
- New batch slice 15/200 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_directions_from_link_topology` now asserts recovered `producer_core.output_data`, `consumer_core.input_data`, and `consumer_core.result_data` child directions keep original child signal declaration support IDs in selected child module inventories.
- Existing checks still prove topology-link support IDs, high automation confidence, selected child signal-inventory topology provenance, and emitted topology text.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-06 `.fsm` transitive child-width provenance lock
- New batch slice 14/200 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_width_through_transitive_topology` now asserts recovered `producer_core.output_data` and `consumer_core.input_data` child widths keep original child signal declaration support IDs in selected child module inventories.
- Existing checks still prove topology-link support IDs, high automation confidence, selected child signal-inventory topology provenance, and emitted `(output_data 8)` / `(input_data 8)` text.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-06 `.fsm` source child-width provenance lock
- New batch slice 13/200 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_source_child_width_from_sibling_child_link_topology` now asserts the recovered `producer_core.output_data` child width keeps the original child signal declaration support IDs in the selected child module inventory.
- Existing checks still prove topology-link support IDs, high automation confidence, selected child signal-inventory topology provenance, and emitted `(output_data 8)` text.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove topology-link support IDs, high automation confidence, selected signal-inventory topology provenance, and `result_data>8` emission.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- Existing checks still prove graph actor-port support IDs, high automation confidence, selected signal-inventory actor-port provenance, and `ext_data>8` emission.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- The test also locks producer-to-controller data-link and controller-to-top `ACC` topology-link support IDs while preserving mixed DT/FSM child support IDs, root kind order, and top-first direct-root emission.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-06 `.fsm` reused-FSM-child top provenance lock
- New batch slice 4/200 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `renderable_top_document_deduplicates_reused_fsm_child_roots` now asserts `ACC_A` and `ACC_B` top-port support IDs survive in both selected and renderable top records.
- The test also locks first/second child `ACC` topology-link support IDs while preserving per-instance child support IDs and one shared emitted `controller_core` FSM direct root.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the blocked child actor-port direction conflict path.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the blocked sibling-link width conflict path where two child links infer incompatible consumer input widths.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the single-FSM-child top-composition document path.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the reused-FSM-child top-composition document path.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` reused-child top support lock
- New batch slice 73/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `renderable_top_document_deduplicates_reused_child_module_roots` now asserts reused top child declarations keep distinct support IDs in the selected top candidate while their shared `stage_core` direct root remains emitted once.
- This is a coverage-only lock for the reused-child top-composition document path.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the top-root selection path where child declarations, rather than recovered top-port evidence, supply the high-confidence root-kind signal.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the renderable top-composition path where a producer output width is recovered from a consumer sibling link.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` child sibling-link target width provenance lock
- New batch slice 67/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_width_from_sibling_child_link_topology` now asserts target-side child width recovery from a sibling child link preserves explicit top-link support IDs and high automation confidence in the consumer module inventory.
- This is a coverage-only lock for the renderable top-composition path where a consumer input width is recovered from a producer sibling link.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` child top-link width provenance lock
- New batch slice 66/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_recovers_child_width_from_top_link_topology` now asserts child width recovery from public top-link topology preserves explicit top-link support IDs and high automation confidence in the producer module inventory.
- This is a coverage-only lock for the renderable top-composition path where a child module output width is recovered from the linked top port.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the renderable top-composition path where child module directions are recovered from actor-port graph evidence.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` duplicate top-port width provenance lock
- New batch slice 63/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_keeps_duplicate_top_port_width_conflict_unresolved` now asserts duplicate top-port width declarations preserve both support-ID sets, width-conflict inventory state, and high automation confidence on duplicate recovered top ports and selected signal inventory entries.
- This is a coverage-only lock for the blocked top-composition path where duplicate public top-port declarations disagree on width.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the blocked top-composition path where the flat top-port declaration and graph actor-port width disagree.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` top actor-direction conflict provenance lock
- New batch slice 59/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `top_composition_blocks_conflicting_top_actor_port_direction` now asserts conflicting top-boundary direction evidence preserves explicit top-port support IDs, `graph_wrapper_ext_data`, and high automation confidence on the recovered top port and selected signal inventory.
- This is a coverage-only lock for the blocked top-composition path where the flat top-port declaration and graph actor-port direction disagree.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the blocked top-composition path where an explicit top port has symbolic width evidence but no numeric width evidence.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the blocked structured-FSM path where graph evidence marks an output but no typed FSM-state action drives it.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` structured-FSM flat/graph provenance lock
- New batch slice 52/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `structured_fsm_blocks_flat_graph_direction_disagreement` now asserts blocked FSM-root flat-vs-graph disagreement keeps interface and actor-port categories, `graph_controller_ACC` support ID, and high automation confidence.
- This is a coverage-only lock for the blocked structured-FSM path where canonical FSM-root direction and actor-port graph direction disagree.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the blocked standalone sequential DT path where local clock width evidence conflicts with canonical system-contract width requirements.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` system direction-conflict provenance lock
- New batch slice 49/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `standalone_sequential_dt_blocks_conflicting_system_contract_signal_direction` now asserts blocked clock direction conflicts keep system-contract support IDs and high automation confidence.
- This is a coverage-only lock for the blocked standalone sequential DT path where local clock direction evidence conflicts with canonical system-contract direction requirements.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` system flat/graph disagreement provenance lock
- New batch slice 48/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `standalone_sequential_dt_blocks_system_contract_flat_graph_direction_disagreement` now asserts blocked clock flat-vs-graph disagreement keeps the contradictory `graph_controller_clk` support ID and high automation confidence.
- This is a coverage-only lock for the blocked standalone sequential DT system-contract path where canonical system-contract direction and actor-port graph direction disagree.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the blocked path where flat canonical direction evidence conflicts even though actor graph evidence is unambiguous.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` direct flat/graph disagreement provenance lock
- New batch slice 44/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `standalone_dt_blocks_flat_graph_direction_disagreement` now asserts blocked direct-root flat/graph disagreement keeps interface and actor-port categories, graph support ID, and high automation confidence.
- This is a coverage-only lock for the blocked path where canonical flat direction evidence disagrees with graph-backed actor-port direction evidence.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for renderable direct-root graph recovery in the presence of unrelated actor-port context.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for blocked direct-root actor-port width disagreement.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.

## 2026-05-05 `.fsm` explicit-module width-conflict provenance lock
- New batch slice 37/100 tightens coverage in `crates/specforge/src/ir/adapters.rs`.
- `standalone_explicit_module_blocks_conflicting_control_input_actor_port_widths` now asserts blocked module-local actor-port width conflicts keep both graph-backed support IDs and high automation confidence.
- This is a coverage-only lock for blocked explicit-module actor-port width disagreement.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for renderable module-local state-body and transition-guard input recovery.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for renderable direct-root control-read input recovery.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the renderable precedence path where canonical numeric width remains authoritative for `.fsm` emission.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- This is a coverage-only lock for the adapter inventory surface and keeps canonical symbolic widths distinct from graph-backed actor-port symbolic widths.
- Full adapter coverage remains `80` adapter tests, full CI remains `601` Rust tests, and KG bench remains `148/148`.
...
- The new regression covers `[address bus](#address-bus)` and `[write enable control](signals.md#write-enable)`, preserving the visible prose alias while discarding the target syntax.
- Full CI now runs `600` Rust tests, and KG bench remains `148/148`.
...
- `docs/book/src/commands/pipeline.md` and `docs/book/src/reference/generated-artifacts.md` now tell users to read renderability reasons together with signal inventory provenance when adapter lowering is blocked.
- Validation remains at `597` Rust tests and `148/148` KG fixtures.
...
- The blocked artifact keeps both sides visible (`direction_hint: input`, `graph_direction_hint: output`) and routes the decision through the `fsm_adapter_system_contract` residual packet.
- Full adapter coverage now includes `80` adapter tests, and full CI now runs `597` Rust tests.
...
- Semantic phrase lookup now uses the same exact-family plus AMBA-generic helper as semantic modality, temporal phrase, table-shape, and visual-motif prior lookup.
- Positive AMBA-generic semantic-prior fixture source files were renamed to AMBA-scoped filenames so they still cover intended AMBA-generic fallback after unknown-family fallback was removed from this consumer.
- The corpus-KB benchmark, semantic/truthfulness, and prior-candidate readiness projections now count the semantic-prior family-mismatch guard.
...
- This keeps provider-option hardening symmetrical between source enrichment and NLP enrichment replay hints.

## 2026-05-05 rescan-plan extra intent argument coverage
- New batch slice 38/40 extends the extra-stage-argument regression to the intent rebuild lane.
- The parser already rejected extra intent args through the exact match arm; the assertion locks that behavior directly.
- This keeps every simple rebuild/validate replay lane covered for unsupported trailing arguments.
...
- This keeps accepted local-provider spellings regression-locked.
