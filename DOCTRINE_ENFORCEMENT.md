...
| `CORPUS-FRONTIER` | derive-and-diff | gate | `scripts/check_corpus_frontier.sh` | the corpus refresh frontier is derived from persisted SourceIR provenance and bundle retention, and agrees exactly with its declaration and with the root task file's stated counts — a remaining-work total may never be a number each slice decrements |
