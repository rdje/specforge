...
For the two measured SpecForge cases this rule came from — a gate-tier census that compared none of
its eighteen counts, and a checker that published its own case total — see
[`docs/DOCTRINE_INSTANCE.md`](docs/DOCTRINE_INSTANCE.md) §3.
...
The reference deployment — the complete doctrine registry with its archetype, tier, check and what each
one proves, plus the aggregate and suite rules it settled — is
[`docs/DOCTRINE_INSTANCE.md`](docs/DOCTRINE_INSTANCE.md) §10. It is one instance of this standard: it
grows with every doctrine this repository adds, and the standard above does not. Measured over 58
revisions, §10 and §3's cases carried 270 of this file's 274 added lines while the portable prose added
four, which is why they are two documents rather than one.
