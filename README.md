...
- the canonical polarity surface is now live in the refreshed AMBA projection too: AXI, APB, and AXI-Stream each currently report `with_resolved_polarity: 1`, while AHB still reports `0`, so the next polarity win is now targeted extraction coverage for AHB and broader non-reset control polarity recovery rather than more schema plumbing
