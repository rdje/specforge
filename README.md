...
- the canonical polarity surface is now live across the refreshed AMBA projection too: AXI, APB, AHB, and AXI-Stream each currently report `with_resolved_polarity: 1`, so the next polarity win is broader non-reset control polarity recovery rather than more schema plumbing
...
- `.fsm` explicit-module/top-composition lowering now has its first graph-first direction consumer too: matching `IntentIR.actor_ports` can recover child-module port directions when the flat module-local `direction_hint` surface lags, while ambiguous or conflicting graph directions still block rather than inventing `.fsm` port roles
