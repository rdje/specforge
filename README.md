...
- the tracked KG fixture set now also includes a prior-guided actor-taxonomy gold/negative pair: width-only `Issuer signals` / `Acceptor signals` sections stay directionless without prior memory and gain canonical signal directions only when matching actor-taxonomy priors are staged into the fixture
