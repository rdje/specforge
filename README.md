...
- that change came from the first unseen-protocol stress run too: AXI-Stream now converges in `2` pipeline iterations with full Ollama VLM + NLP Level 3 enabled, validates at `88/100 GOOD`, and is strong enough to contribute real semantic priors now that graph-derived direction coverage is complete; the remaining structural gaps are the missing widths on `TDESTCHK` / `TIDCHK` / `TSTRBCHK` / `TUSERCHK` / `TWAKEUPCHK` plus unresolved producer attribution for shared infrastructure signals like `ACLK` / `ARESETN`
