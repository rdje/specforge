...
- `specforge doctor [--strict]` now inspects both Docling ingest readiness and the default Ollama loopback path, reporting the selected Python candidate, Ollama endpoint/model status, and all probe results so the default local-first pipeline can be preflighted before a long converge run
