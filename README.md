...
- `SemanticIR` and `IntentIR` now also resolve a canonical per-signal semantic role plus `semantic_grounding_strength`, so handshake-role consumers can prefer provenance-backed role consensus and validation can distinguish single-source grounding from multi-source grounding
