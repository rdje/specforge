...
- the tracked KG fixture set now also includes a prior-guided table-shape gold/negative pair: a locally `unknown` `Name | Direction | Width` table stays inert without prior memory and gains signal-description recovery only when a matching learned table-shape prior is staged into the fixture
