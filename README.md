...
- that temporal surface now also recovers bounded `cycle_window` latency from prose/timing text like `within 2 cycles`, `within 2 ticks`, `after 3 falling edges`, `two cycles later`, `next cycle`, `next tick`, `next posedge`, `next negedge`, `tick T3`, and `on the third rising edge`, and it now preserves the explicit rising/falling edge semantics for shorthand forms instead of silently flattening them to the default clock edge
