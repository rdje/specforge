...
- that change came from the first unseen-protocol stress run too: AXI-Stream now converges in `2` pipeline iterations with full Ollama VLM + NLP Level 3 enabled, validates at `84/100 GOOD`, and is strong enough to contribute real semantic priors even though the remaining structural gaps still include the four `*CHK` signals and unresolved producer attribution for shared infrastructure signals like `ACLK` / `ARESETN`
