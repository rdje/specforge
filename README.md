...
- that change came from the first unseen-protocol stress run too: AXI-Stream now converges in `2` pipeline iterations with full Ollama VLM + NLP Level 3 enabled, validates at `90/100 EXCELLENT`, and now carries complete declared-signal direction and width coverage (`22/22`) after parity-check width recovery; the remaining honest gaps are unresolved producer attribution for shared infrastructure signals like `ACLK` / `ARESETN`, missing explicit cycle-window bounds on typed temporal rules, and the two carried temporal conflicts
