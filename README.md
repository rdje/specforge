...
- that temporal surface now also recovers bounded `cycle_window` latency from prose/timing text like `within 2 cycles`, `within 2 ticks`, `within 2 clock edges`, `within 2 HCLK edges`, `after 3 falling edges`, `two cycles later`, `next cycle`, `next tick`, `next clock edge`, `next posedge`, `next negedge`, `tick T3`, and `on the third rising edge`, it preserves the explicit rising/falling edge semantics for shorthand forms instead of silently flattening them to the default clock edge, explicit local clock names like `HCLK` now override the document default clock whether the sentence says `rising edge of HCLK`, `HCLK rising edge`, `next HCLK edge`, or `the third edge of HCLK`, named cycle/tick phrasing like `same ACLK cycle` now becomes fully grounded temporal structure instead of stopping at only a recovered clock name, and named one-cycle or named bounded-edge forms like `next ACLK cycle`, `next HCLK rising edge`, `next HCLK edge`, and `within 2 HCLK edges` now recover their bounded `cycle_window` too
