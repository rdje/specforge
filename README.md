...
- a pinned `subs/fsmgen` git submodule exists as the local read-only reference for the downstream `.isf` consumer, currently refreshed to upstream `88a7af9c` (bumped from `9bfb9a20` under `FSMGEN-REFRESH-INTEGRATE`); it carries FSMGen's ISF public-interface contract, downstream-integration spec, feature-support matrix, capability-manifest/check-JSON/diagnostic-code/normalized-semantic-JSON surfaces, and its own live mdBook for adapter planning
...
- `subs/fsmgen/`
  ...
  - currently refreshed to upstream `88a7af9c`, including FSMGEN's own live mdBook at `subs/fsmgen/docs/book/`
