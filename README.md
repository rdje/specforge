...
- that change came from the first unseen-protocol stress run too: AXI-Stream now converges in `2` pipeline iterations with full Ollama VLM + NLP Level 3 enabled, validates at `90/100 EXCELLENT`, and now carries complete declared-signal direction and width coverage (`22/22`) after parity-check width recovery; shared infrastructure signals like `ACLK` / `ARESETN` now surface through `infrastructure_signal_connectivity` and a dedicated system-contract note rather than an ordinary missing-producer warning, six same-cycle temporal rules now carry explicit `0`-cycle windows, and the remaining honest gaps are the single carried temporal conflict plus the residual interface-grouping decision
