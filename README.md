...
- that temporal surface now also recovers bounded `cycle_window` latency from prose/timing text like `within 2 cycles`, `two cycles later`, `next cycle`, `next tick`, and `on the third rising edge` instead of leaving every temporal rule unbounded
