...
- `specforge corpus-kb [validation-report]... [--kg-fixtures-root <fixture-root>]` now refreshes tracked `R15g` corpus knowledge-base pages under `corpus_kb/` from reviewable evidence such as validation reports, KG fixture outcomes, KG fixture-family summaries, and dedicated table/visual/timing/infra/protocol family pages, using managed blocks so human synthesis is preserved and canonical IR truth is never mutated
...
- `specforge corpus-kb [validation-report]... [--kg-fixtures-root <fixture-root>]` refreshes tracked `corpus_kb/` synthesis pages from reviewable validation reports, KG fixture outcomes, fixture-family summaries, and dedicated family pages for table, visual, timing, infrastructure, and protocol patterns; it only replaces managed blocks and preserves human-authored notes, so the corpus KB remains guidance and synthesis rather than canonical truth promotion
