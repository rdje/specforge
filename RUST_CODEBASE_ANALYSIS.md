...
- `specforge validate` now reports `table_signal_declaration_provenance` for `EvidenceIR` plus `with_table_support` for `SemanticIR` and `IntentIR`, while `specforge kg-bench` can assert exact EvidenceIR table-signal provenance records and exact canonical table support, making both evidence-stage table-declaration provenance and canonical table-backed signal coverage visible without replacing exact per-signal provenance checks
...
- the workspace currently validates through `bash scripts/run_ci.sh`, which runs Rust formatting, Clippy with `-D warnings`, Rust tests with `RUSTFLAGS="-D warnings"`, rustdoc with `RUSTDOCFLAGS="-D warnings"`, and the mdBook docs build; after the EvidenceIR table-signal provenance validation slice the full local CI path reports clean formatting, clean Clippy, `321` passing Rust tests, clean Rust API docs, and a successful mdBook build
