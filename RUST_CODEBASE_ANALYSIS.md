...
- the latest unseen protocol stress run is now AXI-Stream: it converges in `2` full pipeline iterations, validates at `84/100 GOOD`, and the latest truthfulness fixes remove width-only `_WIDTH` pseudo-signals from declared interface coverage, add graph-backed `ACLK` / `ARESETN` input ports for the relation-grounded actors, and raise graph-direction coverage to `12/16` while leaving the four `*CHK` signals and unresolved producers for `ACLK` / `ARESETN` as the dominant remaining structural gap
