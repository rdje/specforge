...
## Session update (2026-05-13 `.fsm` top composition root_kind_decision batch 2)
- Added Top-root root_kind_decision to 5 more top composition tests (3 renderable + 2 blocked). 10 of ~37 top composition tests now covered.
- Fixed 2 previously-failed blocked test edits using unique `.expect()` message anchors.
...
- Remaining: ~27 top composition tests (some may already have blocks from earlier work).
...
- Top confidence is `High` (positive-evidence-based), consistent with FSM's `High` from explicit regular-state facts; DT is the only root kind at `Medium` (absence-based inference).
- The `root_kind_decision` completeness audit is now fully closed across DT (7 tests), FSM (6 tests), and Top (15 tests).
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing root-kind confidence test now covers clean diagnostics in addition to link-evidence-based confidence classification.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing top-before-child ordering regression now covers clean topology and aggregate no-direct-module coverage in addition to the existing ordering and identity assertions.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing reused-FSM-child top regression now covers clean topology and aggregate no-direct-module coverage in addition to per-instance child support deduplication.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing blocked explicit-module flat/graph regression now covers aggregate stale-renderable prevention in addition to module-candidate diagnostics.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing blocked explicit-module width-conflict regression now covers aggregate stale-renderable prevention in addition to module-candidate diagnostics.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- The selected renderable module now also proves output-side size entries preserve output direction and width for the same signals.
- Production adapter behavior did not change; the existing standalone explicit-module regression now covers both input-side recovery and output-side provenance.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
## Session update (2026-05-11 `.fsm` standalone explicit-module root-kind provenance)
- Extended `standalone_explicit_module_recovers_inputs_from_module_control_reads` in `crates/specforge/src/ir/adapters.rs`.
- This is a regression-only `R6`/`R15` hardening slice: standalone explicit-module candidates now prove root identity, declaration order, selected `?fsm:name` root kind, deferred top root, root-kind confidence, and explicit-state rationale before renderable projection.
- Production adapter behavior did not change; the existing standalone explicit-module regression now covers root-kind decision provenance in addition to module-candidate system/init and renderable projection provenance.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
## Session update (2026-05-11 `.fsm` standalone explicit-module candidate surface provenance)
- Extended `standalone_explicit_module_recovers_inputs_from_module_control_reads` in `crates/specforge/src/ir/adapters.rs`.
- This is a regression-only `R6`/`R15` hardening slice: standalone explicit-module candidates now prove source explicit-module system-contract equality plus init-assignment target, value, support IDs, and automation confidence before renderable projection.
- Production adapter behavior did not change; the existing standalone explicit-module regression now covers the module-candidate surface in addition to selected renderable module and emitted source-document direct-root provenance.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
## Session update (2026-05-11 `.fsm` standalone explicit-module init-assignment provenance)
- Extended `standalone_explicit_module_recovers_inputs_from_module_control_reads` in `crates/specforge/src/ir/adapters.rs`.
- This is a regression-only `R6`/`R15` hardening slice: standalone explicit-module `?fsm:name` selected renderable modules and emitted source-document direct roots now prove source explicit-module init-assignment targets, values, support IDs, and automation confidence survive projection.
- Production adapter behavior did not change; the existing standalone explicit-module regression now covers init-assignment provenance in addition to system-contract provenance, control-block provenance, state-graph provenance, module-control graph input recovery, emitted `(+size ...)` behavior, and residual-clean renderability.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing standalone explicit-module regression now covers control-block provenance in addition to state-graph provenance, module-control graph input recovery, emitted `(+size ...)` behavior, and residual-clean renderability.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing structured-FSM regression now covers renderable module/source-document state graph provenance in addition to candidate provenance, signal inventory, control/action graph, emitted text, and reset-block provenance behavior.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
- Production adapter behavior did not change; the existing reset-block structured-FSM regression now covers reset-synchronous, reset-asynchronous, and state-body control-block provenance in addition to state graph, system, signal-inventory, and emitted-text behavior.
- The Rust test listing remains `666` tests, and the adapter-filtered suite reports `143/143` passing tests.
...
## Session update (2026-05-09 adapter undeclared top-target residual live-status alignment)
...
## Session update (2026-05-09 adapter duplicate child reference residual live-status alignment)
...
## Session update (2026-05-09 adapter parametric top-port residual live-status alignment)
...
## Session update (2026-05-09 adapter widthless top-port residual live-status alignment)
...
## Session update (2026-05-09 adapter duplicate top-width residual live-status alignment)
- Synced live docs and mdBook with existing `top_composition_keeps_duplicate_top_port_width_conflict_unresolved` coverage in `crates/specforge/src/ir/adapters.rs`.
- The duplicate top-port width regression already locks the composition residual to include explicit top-port diagnostics alongside renderable child-module and child-module reference diagnostics, low confidence, deduplication/width repair guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a documentation alignment slice for existing `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter duplicate top-direction residual live-status alignment)
...
## Session update (2026-05-09 adapter top-link direction residual live-status alignment)
...
## Session update (2026-05-09 adapter top actor-direction residual live-status alignment)
- Synced live docs and mdBook with existing `top_composition_blocks_conflicting_top_actor_port_direction` coverage in `crates/specforge/src/ir/adapters.rs`.
- The top actor-port direction conflict regression already locks the composition residual to include explicit top-port diagnostics alongside renderable child-module and child-module reference diagnostics, low confidence, direction-conflict repair guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a documentation alignment slice for existing `R6` `.fsm` adapter diagnostics, not a production architecture change.
...
## Session update (2026-05-09 adapter no-child residual live-status alignment)
- Synced live docs and mdBook with existing `top_composition_blocks_top_without_child_guidance` coverage in `crates/specforge/src/ir/adapters.rs`.
- The no-child top regression already locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, child-reference enrichment guidance, retained top-port provenance, and blocked `.fsm` output.
- This is a documentation alignment slice for existing `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter recovered direction residual live-status alignment)
- Synced live docs and mdBook with existing `top_composition_preserves_recovered_top_port_direction_when_still_blocked` coverage in `crates/specforge/src/ir/adapters.rs`.
- The blocked recovered top-port direction regression already locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, recovered top-port direction evidence, retained selected inventory provenance, and blocked `.fsm` output.
- This is a documentation alignment slice for existing `R6` `.fsm` adapter diagnostics, not a production architecture change.
...
## Session update (2026-05-09 adapter child actor residual live-status alignment)
...
## Session update (2026-05-09 adapter missing-child residual renderable-child diagnostic guard)
...
## Session update (2026-05-09 adapter recovered direction residual renderable-child diagnostic guard)
...
- The blocked recovered top-port direction regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, recovered top-port direction evidence, retained selected inventory provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter recovered top-root residual renderable-child diagnostic guard)
- Extended `top_root_kind_confidence_follows_recovered_top_port_evidence` in `crates/specforge/src/ir/adapters.rs`.
- The recovered top-root confidence regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, recovered top-port evidence, retained selected inventory provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter multi-child no-link residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_multi_child_without_links_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The multi-child no-link regression now locks the composition residual to include renderable child-module diagnostics alongside width-compatible top-link, explicit top-port, and child-module reference diagnostics, low confidence, top-link enrichment guidance, retained child/top provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.
...
## Session update (2026-05-09 adapter parametric top-port residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_parametric_top_port_width_for_fsm_public_io` in `crates/specforge/src/ir/adapters.rs`.
- The parametric public top-port regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, width-resolution guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter widthless top-port residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_widthless_top_port_without_width_recovery` in `crates/specforge/src/ir/adapters.rs`.
- The widthless public top-port regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, width-recovery guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter duplicate top-width residual renderable-child diagnostic guard)
...
## Session update (2026-05-09 adapter duplicate top-direction residual renderable-child diagnostic guard)
- Extended `top_composition_keeps_duplicate_top_port_direction_conflict_unresolved` in `crates/specforge/src/ir/adapters.rs`.
- The duplicate top-port direction regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, deduplication/direction repair guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter top actor-port width residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_conflicting_top_actor_port_width` in `crates/specforge/src/ir/adapters.rs`.
- The top actor-port width conflict regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, width-conflict repair guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter top actor-port direction residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_conflicting_top_actor_port_direction` in `crates/specforge/src/ir/adapters.rs`.
- The top actor-port direction conflict regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, direction-conflict repair guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter top child-link width residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_conflicting_top_port_widths_from_child_links` in `crates/specforge/src/ir/adapters.rs`.
- The top child-link width conflict regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, width-conflict repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter undeclared top-source residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_link_from_undeclared_top_source_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The undeclared top-source regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, source-endpoint repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter undeclared top-target residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_link_to_undeclared_top_target_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The undeclared top-target regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, target-endpoint repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter no-top-port residual renderable-child diagnostic guard)
- Extended `top_composition_blocks_top_without_port_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The no-top-port regression now locks the composition residual to include renderable child-module diagnostics alongside explicit top-port and child-module reference diagnostics, low confidence, top-port repair guidance, retained child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter duplicate child residual renderable-child diagnostic guard)
...
## Session update (2026-05-09 adapter multi-child no-link residual child-reference diagnostic guard)
- Extended `top_composition_blocks_multi_child_without_links_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The multi-child top-without-links regression now locks the composition residual to include child-module reference diagnostics alongside width-compatible top-link and explicit top-port diagnostics, low confidence, top-link repair guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter top-link width residual child-reference diagnostic guard)
- Extended `top_composition_blocks_width_mismatched_top_link_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The width-mismatched top-link regression now locks the composition residual to include child-module reference diagnostics alongside width-compatible top-link and explicit top-port diagnostics, low confidence, width-compatible link repair guidance, retained top-port/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter unemitted child target guidance residual child-reference diagnostic guard)
- Extended `top_composition_blocks_link_to_unemitted_child_target_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The target-side unemitted child endpoint regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, target-endpoint repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter child target role residual child-reference diagnostic guard)
- Extended `top_composition_blocks_child_target_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The target-side child direction-role regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, endpoint-role repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter child source role residual child-reference diagnostic guard)
- Extended `top_composition_blocks_child_source_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The source-side child direction-role regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, endpoint-role repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter unemitted child source guidance residual child-reference diagnostic guard)
- Extended `top_composition_blocks_link_from_unemitted_child_source_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The provenance-rich source-side unemitted child endpoint regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, source-endpoint repair guidance, retained selected-top inventory provenance, child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter unemitted child source residual child-reference diagnostic guard)
- Extended `top_composition_blocks_link_from_unemitted_child_port` in `crates/specforge/src/ir/adapters.rs`.
- The source-side unemitted child endpoint regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, source-endpoint repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.
...
- This is a test-surface correction for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter widthless top-port residual child-reference diagnostic guard)
- Extended `top_composition_blocks_widthless_top_port_without_width_recovery` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter duplicate top-port width residual child-reference diagnostic guard)
- Extended `top_composition_keeps_duplicate_top_port_width_conflict_unresolved` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter duplicate top-port direction residual child-reference diagnostic guard)
- Extended `top_composition_keeps_duplicate_top_port_direction_conflict_unresolved` in `crates/specforge/src/ir/adapters.rs`.
- The duplicate top-port direction conflict regression now locks the composition residual to include child-module reference diagnostics alongside explicit top-port diagnostics, low confidence, deduplication repair guidance, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter top-target role residual child-reference diagnostic guard)
- Extended `top_composition_blocks_top_target_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The top-target boundary role regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, endpoint-role repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter top-link direction residual child-reference diagnostic guard)
- Extended `top_composition_keeps_conflicting_top_port_direction_unresolved` in `crates/specforge/src/ir/adapters.rs`.
- The top-link-driven top-boundary direction conflict regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, direction-conflict repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.
...
## Session update (2026-05-09 adapter child actor-port direction residual child-reference diagnostic guard)
- Extended `top_composition_blocks_conflicting_actor_port_directions` in `crates/specforge/src/ir/adapters.rs`.
- The child actor-port direction conflict regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, direction-conflict repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter child topology direction residual child-reference diagnostic guard)
- Extended `top_composition_blocks_conflicting_child_link_topology_directions` in `crates/specforge/src/ir/adapters.rs`.
- The child topology direction conflict regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, direction-conflict repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter sibling child-link width residual child-reference diagnostic guard)
- Extended `top_composition_blocks_conflicting_sibling_child_link_widths` in `crates/specforge/src/ir/adapters.rs`.
- The sibling child-link width conflict regression now locks the composition residual to include child-module reference diagnostics alongside renderable child-module and explicit top-port diagnostics, low confidence, width-conflict repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter child topology width residual child-reference diagnostic guard)
...
## Session update (2026-05-09 adapter top child-link width residual child-reference diagnostic guard)
- Extended `top_composition_blocks_conflicting_top_port_widths_from_child_links` in `crates/specforge/src/ir/adapters.rs`.
- The top child-link width conflict regression now locks the composition residual to include child-module reference diagnostics alongside explicit top-port diagnostics, low confidence, width-conflict repair guidance, retained top-port/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter missing child residual explicit-port diagnostic guard)
- Extended `keeps_top_composition_blocked_when_child_module_is_missing` in `crates/specforge/src/ir/adapters.rs`.
- The primary missing child-module top regression now locks the composition residual to include explicit top-port diagnostics alongside child-module reference diagnostics, low confidence, retained top-port/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.
...
## Session update (2026-05-09 adapter target child-role guidance explicit-port diagnostic guard)
- Extended `top_composition_blocks_child_target_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The target-side child direction-role guidance regression now locks the composition residual to include explicit top-port diagnostics alongside renderable child-module diagnostics, low confidence, endpoint role repair guidance, retained top-port/child/link provenance, and blocked `.fsm` output.
...
## Session update (2026-05-09 adapter source unemitted guidance explicit-port diagnostic guard)
- Extended `top_composition_blocks_link_from_unemitted_child_source_guidance` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter undeclared top-source residual child-reference diagnostic guard)
- Extended `top_composition_blocks_link_from_undeclared_top_source_guidance` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter no-top-port residual child-reference diagnostic guard)
- Extended `top_composition_blocks_top_without_port_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The no-top-port regression now locks the composition residual to include child-module reference diagnostics alongside explicit top-port diagnostics, low confidence, top-port repair guidance, retained child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter no-child top residual explicit-port diagnostic guard)
- Extended `top_composition_blocks_top_without_child_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The no-child top regression now locks the composition residual to include explicit top-port diagnostics alongside child-module reference diagnostics, low confidence, child-reference repair guidance, retained top-port provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter multi-child no-link residual explicit-port diagnostic guard)
- Extended `top_composition_blocks_multi_child_without_links_guidance` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter top-link width residual explicit-port diagnostic guard)
- Extended `top_composition_blocks_width_mismatched_top_link_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The top-link width mismatch regression now locks the composition residual to include explicit top-port diagnostics alongside width-compatible top-link diagnostics, low confidence, width repair guidance, retained top-port/link provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter child topology-direction residual explicit-port diagnostic guard)
- Extended `top_composition_blocks_conflicting_child_link_topology_directions` in `crates/specforge/src/ir/adapters.rs`.
- The child topology direction conflict regression now locks the composition residual to include explicit top-port diagnostics alongside renderable child-module diagnostics, low confidence, actor-relative graph-direction repair guidance, retained top-port/link/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter target role residual explicit-port diagnostic guard)
- Extended `top_composition_blocks_child_target_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The target-side child direction-role blocker regression now locks the composition residual to include explicit top-port diagnostics alongside renderable child-module diagnostics, low confidence, endpoint role repair guidance, retained top-port/link/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.

## Session update (2026-05-09 adapter source role residual explicit-port diagnostic guard)
- Extended `top_composition_blocks_child_source_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The source-side child direction-role blocker regression now locks the composition residual to include explicit top-port diagnostics alongside renderable child-module diagnostics, low confidence, endpoint role repair guidance, retained top-port/link/child provenance, and blocked `.fsm` output.
- This is a test-surface hardening slice for `R6` `.fsm` adapter diagnostics, not a production architecture change.
...
## Session update (2026-05-09 adapter child topology-direction conflict inventory provenance guard)
- Extended `top_composition_blocks_conflicting_child_link_topology_directions` in `crates/specforge/src/ir/adapters.rs`.
- The blocked child topology direction conflict regression now locks conflicted `output_data` child inventory to retained `interface` provenance alongside contradictory topology-link provenance, child signal support IDs, topology support ID sets, conflicted graph direction, width, and high confidence.
- This is a test-surface hardening slice for `R6` `.fsm` adapter provenance, not a production architecture change.

## Session update (2026-05-09 adapter child topology-width conflict inventory provenance guard)
- Extended `top_composition_blocks_conflicting_child_topology_widths` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter source-side sibling child-width inventory provenance guard)
- Extended `top_composition_recovers_source_child_width_from_sibling_child_link_topology` in `crates/specforge/src/ir/adapters.rs`.
- The source-side sibling child-link-backed width recovery regression now locks selected `output_data` child inventory to retained `interface` provenance alongside topology-link provenance, child signal support IDs, topology support IDs, recovered width, and high confidence.
- This is a test-surface hardening slice for `R6` `.fsm` adapter provenance, not a production architecture change.

## Session update (2026-05-09 adapter sibling child-width inventory provenance guard)
...
## Session update (2026-05-09 adapter actor-port direction inventory provenance guard)
...
## Session update (2026-05-09 adapter child-link top-width inventory provenance guard)
...
## Session update (2026-05-09 adapter recovered top-root inventory provenance guard)
- Extended `top_root_kind_confidence_follows_recovered_top_port_evidence` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter source-side child direction-role inventory provenance guard)
- Extended `top_composition_blocks_child_source_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The source-side child direction-role blocker regression now locks retained selected `result_data` top inventory to `top_port` provenance alongside direction, width, support IDs, and high confidence.
- This is a test-surface hardening slice for `R6` `.fsm` adapter provenance, not a production architecture change.
...
- This keeps the undeclared-source coverage tied to the fixture that owns the unresolved top-link source blocker.
...
- This keeps undeclared target blockers tied to explainable public top inventory while unresolved top-link targets remain the renderability blocker.

## Session update (2026-05-09 adapter missing-link inventory provenance guard)
- Extended `top_composition_blocks_multi_child_without_links_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The multi-child-without-links blocker regression now locks retained selected `result_data` top inventory to `top_port` provenance alongside direction, width, support IDs, and high confidence.
- This keeps missing-link multi-child blockers tied to explainable public top inventory while absent topology links remain the renderability blocker.

## Session update (2026-05-09 adapter duplicate child inventory provenance guard)
- Extended `top_composition_blocks_duplicate_child_instance_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The duplicate child-instance blocker regression now locks retained selected `result_data` top inventory to `top_port` provenance alongside direction, width, support IDs, and high confidence.
- This keeps duplicate child-instance blockers tied to explainable public top inventory while duplicate child records remain the renderability blocker.

## Session update (2026-05-09 adapter child actor-port direction inventory provenance guard)
- Extended `top_composition_blocks_conflicting_actor_port_directions` in `crates/specforge/src/ir/adapters.rs`.
- The child actor-port direction conflict regression now locks conflicted child `output_data` inventory to numeric width and actor-port provenance while graph direction remains conflicted.
- The same guard locks retained selected `result_data` top inventory to `top_port` provenance alongside direction, width, support IDs, and high confidence.

## Session update (2026-05-09 adapter child topology direction inventory provenance guard)
- Extended `top_composition_blocks_conflicting_child_link_topology_directions` in `crates/specforge/src/ir/adapters.rs`.
...
## Session update (2026-05-09 adapter child topology width inventory provenance guard)
- Extended `top_composition_blocks_conflicting_child_topology_widths` in `crates/specforge/src/ir/adapters.rs`.
- The direct child topology width conflict regression now locks conflicted child `output_data` inventory to output direction and topology-link provenance while width remains conflicted.
- The same guard locks retained selected `result_data` top inventory to `top_port` provenance alongside direction, width, support IDs, and high confidence.

## Session update (2026-05-09 adapter sibling child-link inventory provenance guard)
- Extended `top_composition_blocks_conflicting_sibling_child_link_widths` in `crates/specforge/src/ir/adapters.rs`.
- The sibling child-link width conflict regression now locks the conflicted child `input_data` inventory to input direction and topology-link provenance while width remains conflicted.
- The same guard locks retained selected `result_data` top inventory to `top_port` provenance alongside direction, width, support IDs, and high confidence.
...
- This keeps duplicate direction blockers explainable at the selected inventory surface alongside both duplicate declaration support-ID sets and high confidence.
...
- This keeps conflicting top-boundary actor width evidence explainable at the selected inventory surface alongside support IDs and high confidence.

## Session update (2026-05-09 adapter top actor-direction inventory provenance guard)
- Extended `top_composition_blocks_conflicting_top_actor_port_direction` in `crates/specforge/src/ir/adapters.rs`.
- The top actor-port direction conflict regression now locks selected `ext_data` inventory to numeric width, `top_port` provenance, actor-port provenance, declared input direction, graph output direction, support IDs, and high confidence.
- This keeps conflicting top-boundary actor direction evidence explainable at the selected inventory surface.
...
## Session update (2026-05-09 adapter unemitted child-source inventory provenance guard)
- Extended `top_composition_blocks_link_from_unemitted_child_port` in `crates/specforge/src/ir/adapters.rs`.
- The legacy unemitted child-source blocker regression now locks selected `result_data` inventory to `top_port` provenance alongside output direction, width, support IDs, and high confidence.
- This keeps source-endpoint composition blockers tied to explainable public top-port inventory while the blocked child source remains unresolved.
...
- The selected `status` signal inventory now also locks its output direction, width, `top_port` provenance, support IDs, and low automation confidence.
...
- This keeps actor-port-backed top-boundary direction recovery tied to the explicit child provenance needed for renderable top composition.

## Session update (2026-05-09 adapter top-link direction topology confidence guard)
- Extended `top_composition_recovers_top_port_direction_from_link_topology` in `crates/specforge/src/ir/adapters.rs`.
...
- This keeps emitted reset-block FSM artifacts tied to explainable system/control/data inventory provenance instead of only checking reset roles and emitted text.
...
- This keeps emitted sequential DT artifacts tied to explainable system/data inventory provenance instead of only checking that the signals exist and render.
...
- This keeps graph-context ambiguity recoverable without letting blocked `.fsm` emission downgrade direct signal inventory.

## Session update (2026-05-09 adapter sibling child-link inventory confidence guard)
- Extended `top_composition_blocks_conflicting_sibling_child_link_widths` in `crates/specforge/src/ir/adapters.rs`.
- The sibling child-link width blocker now locks selected signal-inventory support IDs and high automation confidence for the retained declared `result_data` top port.
- This keeps child input width-conflict diagnostics tied to retained top inventory while top composition emission stays blocked.

## Session update (2026-05-09 adapter legacy-unemitted inventory confidence guard)
- Extended `top_composition_blocks_link_from_unemitted_child_port` in `crates/specforge/src/ir/adapters.rs`.
- The legacy source-side unemitted child-port blocker now locks selected signal-inventory support IDs and high automation confidence for the retained declared `result_data` top port.
- This keeps legacy unemitted child-source diagnostics tied to retained top inventory while top composition emission stays blocked.

## Session update (2026-05-09 adapter missing-child inventory confidence guard)
- Extended `keeps_top_composition_blocked_when_child_module_is_missing` in `crates/specforge/src/ir/adapters.rs`.
...
- This keeps missing child-module diagnostics tied to retained top inventory while top composition emission stays blocked.
...
- This keeps child actor-port direction diagnostics tied to retained top inventory while top composition emission stays blocked.

## Session update (2026-05-09 adapter child-topology-direction inventory confidence guard)
- Extended `top_composition_blocks_conflicting_child_link_topology_directions` in `crates/specforge/src/ir/adapters.rs`.
- The child topology direction-conflict blocker now locks selected signal-inventory support IDs and high automation confidence for the retained declared `drive_data` top port.
- This keeps child topology direction diagnostics tied to retained top inventory while top composition emission stays blocked.
...
- This keeps child target role-mismatch diagnostics tied to retained top inventory while top composition emission stays blocked.

## Session update (2026-05-09 adapter source-role inventory confidence guard)
- Extended `top_composition_blocks_child_source_direction_role_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The source-side child direction-role blocker now locks selected signal-inventory support IDs and high automation confidence for the retained declared `result_data` top port.
- This keeps child source role-mismatch diagnostics tied to retained top inventory while top composition emission stays blocked.

## Session update (2026-05-09 adapter unemitted target inventory confidence guard)
- Extended `top_composition_blocks_link_to_unemitted_child_target_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The target-side unemitted child endpoint blocker now locks selected signal-inventory support IDs and high automation confidence for the retained declared `result_data` top port.
- This keeps missing emitted-child-target diagnostics tied to retained top inventory while top composition emission stays blocked.
...
- This keeps missing source-endpoint diagnostics tied to retained top inventory while top composition emission stays blocked.
...
- This keeps retained topology records diagnostic-rich while top composition emission stays blocked on the missing child module.

## Session update (2026-05-09 adapter parametric top-port confidence guard)
- Extended `top_composition_blocks_parametric_top_port_width_for_fsm_public_io` in `crates/specforge/src/ir/adapters.rs`.
- The parametric top-port blocker now locks high automation confidence on the producer child declaration alongside retained top-port evidence.
- This keeps retained topology records diagnostic-rich while top composition emission stays blocked on unresolved parametric top-boundary width evidence.

## Session update (2026-05-08 adapter widthless top-port confidence guard)
- Extended `top_composition_blocks_widthless_top_port_without_width_recovery` in `crates/specforge/src/ir/adapters.rs`.
- The widthless top-port blocker now locks high automation confidence on the producer child declaration alongside retained top-port evidence.
- This keeps retained topology records diagnostic-rich while top composition emission stays blocked on missing top-boundary numeric width evidence.
...
- This keeps retained topology records diagnostic-rich while top composition emission stays blocked on unresolved duplicate top-port direction evidence.
...
- This keeps retained topology records diagnostic-rich while top composition emission stays blocked on unresolved top actor-port direction evidence.
...
- This keeps retained topology records diagnostic-rich while top composition emission stays blocked on unresolved top-boundary direction evidence.

## Session update (2026-05-08 adapter sibling child-link width confidence guard)
- Extended `top_composition_blocks_conflicting_sibling_child_link_widths` in `crates/specforge/src/ir/adapters.rs`.
- The sibling child-link width conflict blocker now locks high automation confidence on the visible top port, all child declarations, and both conflicting links into `consumer.input_data`.
...
## Session update (2026-05-08 adapter child topology direction confidence guard)
- Extended `top_composition_blocks_conflicting_child_link_topology_directions` in `crates/specforge/src/ir/adapters.rs`.
- The child-link topology direction conflict blocker now locks high automation confidence on the drive top port, affected producer child, and both conflicting topology links alongside the conflicting child signal.
- This keeps retained topology records diagnostic-rich while child renderability blocks top composition emission.
...
- This mirrors the source-side role guard so target-role mismatch diagnostics retain their topology records while emission stays blocked.
...
- This completes target-side unemitted child retained-record diagnostics across top port, child declaration, and top link while emission stays blocked.

## Session update (2026-05-08 adapter unemitted source confidence guard)
- Extended `top_composition_blocks_link_from_unemitted_child_source_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The test now locks high automation confidence on the declared top port and producer child while the existing blocked-link confidence check remains in place.
- This completes source-side unemitted child retained-record diagnostics across top port, child declaration, and top link while emission stays blocked.
...
- This complements existing support-ID coverage so blocked missing-target diagnostics keep the retained topology records intact.

## Session update (2026-05-08 adapter multi-child confidence guard)
- Extended `top_composition_blocks_multi_child_without_links_guidance` in `crates/specforge/src/ir/adapters.rs`.
- The test now locks high automation confidence on each visible child declaration while the top candidate remains blocked on missing explicit links.
- This complements child support-ID and root-kind coverage so incomplete multi-child composition diagnostics keep full declaration metadata.
...
- This preserves endpoint direction, width, provenance, support IDs, and confidence for diagnostics instead of trading metadata away at the composition blocker.

## Session update (2026-05-08 adapter binary enum symbol width-cast guard)
- Added `binary_enum_symbol_width_cast_renderability_keeps_graph_signal_references_signal_free` in `crates/specforge/src/ir/adapters.rs`.
- The test locks binary enum-member validation so graph-backed width-cast operands stay signal-free, while the active `.fsm` symbol slice still reports width-cast and per-signal symbol-value blockers.
- This keeps recursive binary enum symbol-expression recovery out of renderable metadata until symbol lowering is explicitly widened.
...
- This keeps enum symbol-expression recovery out of renderable metadata until symbol lowering is explicitly widened.
...
- This keeps recursive unary symbol-expression recovery out of renderable metadata until symbol lowering is explicitly widened.
...
- This keeps graph-first compound-update amount recovery independent from unsupported width-cast lowering.

## Session update (2026-05-08 adapter registered dual-output value width-cast guard)
- Added `registered_dual_output_assignment_value_width_cast_blocks_after_graph_signals` in `crates/specforge/src/ir/adapters.rs`.
- The test locks registered-signal dual-output assignment validation so graph-backed output targets and width-cast value references still produce size entries, while the active `.fsm` suffix-lowering gate blocks width casts.
- This keeps graph-first registered dual-output value recovery independent from unsupported width-cast lowering.
...
- This keeps graph-first assignment value recovery independent from unsupported width-cast lowering.
...
- This keeps graph-first unary operand recovery independent from unsupported width-cast lowering.

## Session update (2026-05-08 adapter branch-predicate width-cast guard)
- Added `branch_predicate_width_cast_blocks_after_graph_signal` in `crates/specforge/src/ir/adapters.rs`.
- The test locks non-selector branch-predicate width-cast validation so graph-backed predicate base signals still produce size entries, while the active `.fsm` suffix-lowering gate blocks width casts.
- This keeps graph-first predicate recovery independent from unsupported width-cast lowering.
...
- This covers the registered-signal dual-output value branch separately from next-signal dual-output values.
...
- This keeps delayed-pulse target recovery graph-first without accidentally widening value-expression lowering.
...
- This covers binary enum member values separately from direct and unary enum member values.
...
- This covers binary symbol values separately from direct scalar and unary symbol values.
...
- This covers the assignment action's value-expression path separately from the output-target guard and direct expression tests.
...
- This covers the unary branch of `validate_control_expression_renderability` separately from plain references.
...
- This covers the combined `validate_control_block_branches` selector path while keeping transition actions valid and out of scope for the slice.
...
- This covers expression values, selectors, and nested expression consumers that flow through the shared graph-first signal registration path.

## Session update (2026-05-08 adapter guard graph direction guard)
- Added `guard_renderability_uses_graph_direction_without_stale_flat_override` in `crates/specforge/src/ir/adapters.rs`.
- The test locks guard renderability so graph-backed guard signals can create size entries through the shared graph-first signal registration path, while stale flat-vs-graph disagreement leaves no stale entry.
- This anchors truthy/equality `.fsm` guard lowering behind the same graph-first/conflict-sticky direction contract as assignments and system contracts.
...
- This completes the current count-style validation metric hardening pass by proving multi-producer `PREADY` ambiguity is covered as both typed conflict shape and aggregate validation metrics.

## Session update (2026-05-08 KG fixture for interface-signal conflict metric)
- Tightened `interface_signal_conflict_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts `interface_signal_conflicts = 2` at both `SemanticIR` and `IntentIR` validation surfaces.
- This locks `DATA` direction and width disagreement as both typed conflict shape and aggregate validation metrics.
...
- This completes signal-semantic conflict validation metric hardening by proving contested `XVALID` role evidence remains counted while typed `HandshakeComplete` fallback stays blocked.

## Session update (2026-05-08 KG fixture for alias-dependent handshake-caveat metric)
- Tightened `alias_dependent_handshake_completion_caveat` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts `temporal_rules_with_alias_dependent_handshake_completion = 1` at both `SemanticIR` and `IntentIR` validation surfaces.
- This completes alias-dependent handshake-completion validation metric hardening by proving the caveated `XREQ` / `XACK` completion is counted by the specific alias-dependent metric.
...
- The fixture now asserts `temporal_rules = 1` at both `SemanticIR` and `IntentIR` validation surfaces.
- This continues aggregate temporal-rule metric hardening by proving the single `PSEL` / `PREADY` guarded payload-stability rule remains visible as a counted temporal rule while handshake completion and multi-predicate antecedent metrics stay locked.
...
## Session update (2026-05-08 KG fixture for AXI width-only temporal metrics)
- Tightened `axi_width_only_prose_direction_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts `temporal_rules_with_multi_predicate_antecedents = 1` at both `SemanticIR` and `IntentIR` validation surfaces.
- This completes the current AXI temporal multi-predicate metric hardening pass by proving the `AWVALID` / `AWREADY` handshake guard stays counted as a multi-predicate antecedent while width-only table role hints and prose actor-direction recovery remain intact.

## Session update (2026-05-08 KG fixture for AXI write-response temporal metrics)
- Tightened `axi_write_response_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts `temporal_rules_with_multi_predicate_antecedents = 1` at both `SemanticIR` and `IntentIR` validation surfaces.
- This continues R15b/R15e temporal metric hardening by proving the `BVALID` / `BREADY` handshake guard stays counted as a multi-predicate antecedent while next-cycle timing and actor-grounded stability remain recovered.

## Session update (2026-05-08 KG fixture for AXI write-data temporal metrics)
- Tightened `axi_write_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts `temporal_rules_with_multi_predicate_antecedents = 1` at both `SemanticIR` and `IntentIR` validation surfaces.
- This continues R15b/R15e temporal metric hardening by proving the `WVALID` / `WREADY` handshake guard stays counted as a multi-predicate antecedent while next-cycle timing and actor-grounded stability remain recovered.

## Session update (2026-05-08 KG fixture for AXI read-data temporal metrics)
- Tightened `axi_read_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts `temporal_rules_with_multi_predicate_antecedents = 1` at both `SemanticIR` and `IntentIR` validation surfaces.
- This continues R15b/R15e temporal metric hardening by proving the `RVALID` / `RREADY` handshake guard stays counted as a multi-predicate antecedent while next-cycle timing and actor-grounded stability remain recovered.

## Session update (2026-05-08 KG fixture for AXI read-address temporal metrics)
- Tightened `axi_read_address_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts `temporal_rules_with_multi_predicate_antecedents = 1` at both `SemanticIR` and `IntentIR` validation surfaces.
- This continues R15b/R15e temporal metric hardening by proving the `ARVALID` / `ARREADY` handshake guard stays counted as a multi-predicate antecedent while next-cycle timing and actor-grounded stability remain recovered.
...
- This completes AXI write-response ID source-split hardening by proving `BVALID` / `BREADY` request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI write-response ID visual exclusion)
- Tightened `axi_write_response_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
...
- This continues AXI write-response ID source-split hardening by proving `BVALID` / `BREADY` request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI write-response ID alias exclusion)
- Tightened `axi_write_response_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
...
- This continues AXI write-response ID source-split hardening by proving `BVALID` / `BREADY` request/accept hints do not rely on phrase-alias prose recovery.

## Session update (2026-05-08 KG fixture for AXI write-response ID prose exclusion)
- Tightened `axi_write_response_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
...
- This starts AXI write-response ID source-split hardening by proving `BVALID` / `BREADY` request/accept hints stay table-sourced while subordinate-owned `BID` handshake stability remains recovered.

## Session update (2026-05-08 KG fixture for AXI write-address sideband VLM exclusion)
- Tightened `axi_write_address_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This completes AXI write-address sideband source-split hardening by proving `AWVALID` / `AWREADY` request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI write-address sideband visual exclusion)
- Tightened `axi_write_address_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI write-address sideband source-split hardening by proving `AWVALID` / `AWREADY` request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI write-address sideband alias exclusion)
- Tightened `axi_write_address_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This continues AXI write-address sideband source-split hardening by proving `AWVALID` / `AWREADY` request/accept hints do not rely on phrase-alias prose recovery.
...
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This completes AXI read-address ID source-split hardening by proving `ARVALID` / `ARREADY` request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI read-address ID visual exclusion)
- Tightened `axi_read_address_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI read-address ID source-split hardening by proving `ARVALID` / `ARREADY` request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI read-address ID alias exclusion)
- Tightened `axi_read_address_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This continues AXI read-address ID source-split hardening by proving `ARVALID` / `ARREADY` request/accept hints do not rely on phrase-alias prose recovery.

## Session update (2026-05-08 KG fixture for AXI read-address ID prose exclusion)
- Tightened `axi_read_address_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate and table-sourced semantic hints.
- This starts AXI read-address ID source-split hardening by proving `ARVALID` / `ARREADY` request/accept hints stay table-sourced while manager-owned `ARID` handshake stability remains recovered.

## Session update (2026-05-08 KG fixture for AXI address/response USER VLM exclusion)
- Tightened `axi_address_response_user_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This completes AXI address/response USER sideband source-split hardening by proving address-channel and write-response request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI address/response USER visual exclusion)
- Tightened `axi_address_response_user_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI address/response USER sideband source-split hardening by proving address-channel and write-response request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI address/response USER alias exclusion)
- Tightened `axi_address_response_user_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This continues AXI address/response USER sideband source-split hardening by proving address-channel and write-response request/accept hints do not rely on phrase-alias prose recovery.

## Session update (2026-05-08 KG fixture for AXI address/response USER prose exclusion)
- Tightened `axi_address_response_user_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate and table-sourced semantic hints.
- This starts AXI address/response USER sideband source-split hardening by proving address-channel and write-response request/accept hints stay table-sourced while `AWUSER` / `ARUSER` / `BUSER` sideband stability remains recovered.
...
- Tightened `axi_data_user_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI data USER sideband source-split hardening by proving `WVALID` / `WREADY` and `RVALID` / `RREADY` request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI data USER alias exclusion)
- Tightened `axi_data_user_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This continues AXI data USER sideband source-split hardening by proving `WVALID` / `WREADY` and `RVALID` / `RREADY` request/accept hints do not rely on phrase-alias prose recovery.

## Session update (2026-05-08 KG fixture for AXI data USER prose exclusion)
- Tightened `axi_data_user_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate and table-sourced semantic hints.
- This starts AXI data USER sideband source-split hardening by proving `WVALID` / `WREADY` and `RVALID` / `RREADY` request/accept hints stay table-sourced while `WUSER` / `RUSER` sideband stability remains recovered.

## Session update (2026-05-08 KG fixture for AXI read-data response VLM exclusion)
- Tightened `axi_read_data_response_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This completes AXI read-data response stability source-split hardening by proving `RVALID` / `RREADY` request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI read-data response visual exclusion)
- Tightened `axi_read_data_response_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI read-data response stability source-split hardening by proving `RVALID` / `RREADY` request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI read-data response alias exclusion)
- Tightened `axi_read_data_response_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This continues AXI read-data response stability source-split hardening by proving `RVALID` / `RREADY` request/accept hints do not rely on phrase-alias prose recovery.

## Session update (2026-05-08 KG fixture for AXI read-data response prose exclusion)
- Tightened `axi_read_data_response_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate and table-sourced semantic hints.
- This starts AXI read-data response stability source-split hardening by proving `RVALID` / `RREADY` request/accept hints stay table-sourced while subordinate-owned `RRESP` handshake stability remains recovered.

## Session update (2026-05-08 KG fixture for AXI write-data VLM exclusion)
- Tightened `axi_write_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This completes AXI write-data timing source-split hardening by proving `WVALID` / `WREADY` request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI write-data visual exclusion)
- Tightened `axi_write_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI write-data timing source-split hardening by proving `WVALID` / `WREADY` request/accept hints do not rely on visual captions.
...
- Tightened `axi_write_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate and table-sourced semantic hints.
- This starts AXI write-data timing source-split hardening by proving `WVALID` / `WREADY` request/accept hints stay table-sourced while write-data temporal obligations remain recovered.

## Session update (2026-05-08 KG fixture for AXI write-response VLM exclusion)
- Tightened `axi_write_response_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This completes AXI write-response timing source-split hardening by proving `BVALID` / `BREADY` request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI write-response visual exclusion)
- Tightened `axi_write_response_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI write-response timing source-split hardening by proving `BVALID` / `BREADY` request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI write-response alias exclusion)
- Tightened `axi_write_response_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This continues AXI write-response timing source-split hardening by proving `BVALID` / `BREADY` request/accept hints do not rely on phrase-alias prose recovery.
...
- Tightened `axi_width_only_prose_direction_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This completes AXI width-only source-split hardening by proving `AWVALID` / `AWREADY` request/accept hints do not rely on VLM timing annotations.

## Session update (2026-05-08 KG fixture for AXI width-only visual exclusion)
- Tightened `axi_width_only_prose_direction_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This continues AXI width-only source-split hardening by proving `AWVALID` / `AWREADY` request/accept hints do not rely on visual captions.

## Session update (2026-05-08 KG fixture for AXI width-only alias exclusion)
- Tightened `axi_width_only_prose_direction_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This continues AXI width-only source-split hardening by proving `AWVALID` / `AWREADY` request/accept hints do not rely on phrase-alias prose recovery.

## Session update (2026-05-08 KG fixture for AXI width-only prose exclusion)
- Tightened `axi_width_only_prose_direction_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate and table-sourced semantic hints.
- This starts AXI width-only source-split hardening by proving `AWVALID` / `AWREADY` request/accept hints stay table-sourced while prose actor relations recover direction.
...
- Tightened `source_column_bogus_actor_attribution_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table-sourced, prose, and alias-grounded prose semantic hints.
- This keeps negative source-column source-split hardening table-grounded by proving bogus actor-attribution coverage does not rely on visual captions.

## Session update (2026-05-08 KG fixture for source-column bogus actor alias exclusion)
- Tightened `source_column_bogus_actor_attribution_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table-sourced, and prose semantic hints.
- This keeps negative source-column source-split hardening table-grounded by proving bogus actor-attribution coverage does not rely on phrase-alias prose.

## Session update (2026-05-08 KG fixture for source-column bogus actor prose exclusion)
- Tightened `source_column_bogus_actor_attribution_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate and table-sourced semantic hints.
- This begins negative source-column source-split hardening by proving bogus actor-attribution coverage does not rely on prose-recovered semantic hints.

## Session update (2026-05-08 KG fixture for AMBA source-column VLM exclusion)
- Tightened `amba_source_column_handshake_gold` in `crates/specforge/test_data/kg_quality/`.
...
- This locks AMBA source-column `XREQ` / `XACK` semantic hints as table-only rather than VLM timing-annotation recovered while preserving typed handshake completion.

## Session update (2026-05-08 KG fixture for AMBA source-column visual exclusion)
- Tightened `amba_source_column_handshake_gold` in `crates/specforge/test_data/kg_quality/`.
...
- This locks AMBA source-column `XREQ` / `XACK` semantic hints as table-only rather than visual-caption recovered while preserving typed handshake completion.

## Session update (2026-05-08 KG fixture for AMBA source-column alias exclusion)
- Tightened `amba_source_column_handshake_gold` in `crates/specforge/test_data/kg_quality/`.
...
- This locks AMBA source-column `XREQ` / `XACK` semantic hints as table-only rather than phrase-alias recovered while preserving typed handshake completion.

## Session update (2026-05-08 KG fixture for AMBA source-column prose exclusion)
- Tightened `amba_source_column_handshake_gold` in `crates/specforge/test_data/kg_quality/`.
...
- This locks AMBA source-column `XREQ` / `XACK` semantic hints as table-only rather than prose-derived while preserving typed handshake completion.

## Session update (2026-05-08 KG fixture for APB requester/completer VLM exclusion)
- Tightened `apb_requester_completer_handshake_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks APB requester/completer request/accept semantic hints as table-only rather than VLM timing-annotation recovered while preserving typed `PSEL` / `PREADY` handshake completion.
...
- Tightened `apb_requester_completer_handshake_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks APB requester/completer request/accept semantic hints as table-only rather than phrase-alias recovered while preserving typed `PSEL` / `PREADY` handshake completion.

## Session update (2026-05-08 KG fixture for APB requester/completer prose exclusion)
- Tightened `apb_requester_completer_handshake_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks APB requester/completer request/accept semantic hints as table-only rather than prose-derived while preserving typed `PSEL` / `PREADY` handshake completion.

## Session update (2026-05-08 KG fixture for APB response VLM exclusion)
- Tightened `apb_response_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks APB response request/accept semantic hints as table-only rather than VLM timing-annotation recovered while preserving completer-owned stability for `PRDATA` and `PSLVERR` under completed-access handshake semantics.

## Session update (2026-05-08 KG fixture for APB response visual exclusion)
- Tightened `apb_response_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks APB response request/accept semantic hints as table-only rather than visual-caption recovered while preserving completer-owned stability for `PRDATA` and `PSLVERR` under completed-access handshake semantics.
...
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks APB write-control request/accept semantic hints as table-only rather than VLM timing-annotation recovered while preserving requester-owned stability for `PWRITE`, `PWDATA`, and `PSTRB`.
...
- Tightened `apb_write_control_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks APB write-control request/accept semantic hints as table-only rather than phrase-alias recovered while preserving requester-owned stability for `PWRITE`, `PWDATA`, and `PSTRB`.

## Session update (2026-05-08 KG fixture for APB write-control prose exclusion)
- Tightened `apb_write_control_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks APB write-control request/accept semantic hints as table-only rather than prose-derived while preserving requester-owned stability for `PWRITE`, `PWDATA`, and `PSTRB`.

## Session update (2026-05-08 KG fixture for APB setup/access VLM exclusion)
- Tightened `apb_setup_access_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks APB setup/access request/accept semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for APB setup/access visual exclusion)
- Tightened `apb_setup_access_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks APB setup/access request/accept semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for APB setup/access alias exclusion)
- Tightened `apb_setup_access_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks APB setup/access request/accept semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for APB setup/access prose exclusion)
- Tightened `apb_setup_access_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks APB setup/access request/accept semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI write-address control VLM exclusion)
- Tightened `axi_write_address_control_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks AXI write-address control request/accept semantic hints as table-only rather than VLM timing-annotation recovered.
...
- Tightened `axi_write_address_control_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks AXI write-address control request/accept semantic hints as table-only rather than phrase-alias recovered.
...
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks prior-guided `XACK` ready-like recovery as visual-caption backed rather than VLM timing-annotation backed.

## Session update (2026-05-08 KG fixture for visual semantic prior alias exclusion)
- Tightened `visual_semantic_prior_guided_caption_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, prose, and visual-caption semantic hints.
- This locks prior-guided `XACK` ready-like recovery as visual-caption backed rather than phrase-alias prose backed.

## Session update (2026-05-08 KG fixture for visual semantic prior prose exclusion)
- Tightened `visual_semantic_prior_guided_caption_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total, table-sourced, and visual-caption semantic hints.
- This locks prior-guided `XACK` ready-like recovery as visual-caption backed rather than ordinary-prose backed.

## Session update (2026-05-08 KG fixture for visual semantic prior table exclusion)
- Tightened `visual_semantic_prior_guided_caption_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and visual-caption semantic hints.
- This locks prior-guided `XACK` ready-like recovery as visual-caption backed rather than table-backed.

## Session update (2026-05-08 KG fixture for APB address/protection VLM exclusion)
- Tightened `apb_address_protection_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the APB wait-state request/accept semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for APB address/protection visual exclusion)
- Tightened `apb_address_protection_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the APB wait-state request/accept semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for APB address/protection alias exclusion)
- Tightened `apb_address_protection_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the APB wait-state request/accept semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for APB address/protection prose exclusion)
- Tightened `apb_address_protection_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the APB wait-state request/accept semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI read-data timing VLM exclusion)
- Tightened `axi_read_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the AXI read-data request/accept semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI read-data timing visual exclusion)
- Tightened `axi_read_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the AXI read-data request/accept semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI read-data timing alias exclusion)
- Tightened `axi_read_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the AXI read-data request/accept semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI read-data timing prose exclusion)
- Tightened `axi_read_data_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the AXI read-data request/accept semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI write-address ID VLM exclusion)
- Tightened `axi_write_address_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the AXI write-address ID request/accept semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI write-address ID visual exclusion)
- Tightened `axi_write_address_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the AXI write-address ID request/accept semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI write-address ID alias exclusion)
- Tightened `axi_write_address_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the AXI write-address ID request/accept semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI write-address ID prose exclusion)
- Tightened `axi_write_address_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the AXI write-address ID request/accept semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI write-data last VLM exclusion)
- Tightened `axi_write_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the AXI write-data request/accept semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI write-data last visual exclusion)
- Tightened `axi_write_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the AXI write-data request/accept semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI write-data last alias exclusion)
- Tightened `axi_write_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the AXI write-data request/accept semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI write-data last prose exclusion)
- Tightened `axi_write_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the AXI write-data request/accept semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI QoS/region VLM exclusion)
- Tightened `axi_address_qos_region_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the `AWQOS` / `AWREGION` / `ARQOS` / `ARREGION` semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI QoS/region visual exclusion)
- Tightened `axi_address_qos_region_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the `AWQOS` / `AWREGION` / `ARQOS` / `ARREGION` semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI QoS/region alias exclusion)
- Tightened `axi_address_qos_region_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the `AWQOS` / `AWREGION` / `ARQOS` / `ARREGION` semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI QoS/region prose exclusion)
- Tightened `axi_address_qos_region_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the `AWQOS` / `AWREGION` / `ARQOS` / `ARREGION` semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI read-address timing VLM exclusion)
- Tightened `axi_read_address_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the AXI read-address request/accept semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI read-address timing visual exclusion)
- Tightened `axi_read_address_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the AXI read-address request/accept semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI read-address timing alias exclusion)
- Tightened `axi_read_address_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the AXI read-address request/accept semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI read-address timing prose exclusion)
- Tightened `axi_read_address_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the AXI read-address request/accept semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI read-address control VLM exclusion)
- Tightened `axi_read_address_control_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the `ARPROT` / `ARCACHE` / `ARLOCK` semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI read-address control visual exclusion)
- Tightened `axi_read_address_control_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the `ARPROT` / `ARCACHE` / `ARLOCK` semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI read-address control alias exclusion)
- Tightened `axi_read_address_control_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the `ARPROT` / `ARCACHE` / `ARLOCK` semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI read-address control prose exclusion)
- Tightened `axi_read_address_control_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the `ARPROT` / `ARCACHE` / `ARLOCK` semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI next-cycle VLM exclusion)
- Tightened `axi_next_cycle_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the AXI next-cycle request/accept semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI next-cycle visual exclusion)
- Tightened `axi_next_cycle_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the AXI next-cycle request/accept semantic hints as table-only rather than visual-caption recovered.
...
- Tightened `axi_next_cycle_timing_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the AXI next-cycle request/accept semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI sideband VLM exclusion)
- Tightened `axi_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the AXI sideband semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI sideband visual exclusion)
- Tightened `axi_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the AXI sideband semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI sideband alias exclusion)
- Tightened `axi_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the AXI sideband semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI sideband prose exclusion)
- Tightened `axi_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the AXI sideband semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI read-data last VLM exclusion)
- Tightened `axi_read_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the `RLAST` read-data last semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI read-data last visual exclusion)
- Tightened `axi_read_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the `RLAST` read-data last semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI read-data last alias exclusion)
- Tightened `axi_read_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the `RLAST` read-data last semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI read-data last prose exclusion)
- Tightened `axi_read_data_last_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the `RLAST` read-data last semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI read-data ID VLM exclusion)
- Tightened `axi_read_data_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the `RID` read-data ID semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI read-data ID visual exclusion)
- Tightened `axi_read_data_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the `RID` read-data ID semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI read-data ID alias exclusion)
- Tightened `axi_read_data_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the `RID` read-data ID semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI read-data ID prose exclusion)
- Tightened `axi_read_data_id_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the `RID` read-data ID semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for AXI read-address sideband VLM exclusion)
- Tightened `axi_read_address_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside total, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks the `ARSIZE`/`ARBURST` read-address sideband semantic hints as table-only rather than VLM timing-annotation recovered.

## Session update (2026-05-08 KG fixture for AXI read-address sideband visual exclusion)
- Tightened `axi_read_address_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside total, table-sourced, prose, and alias-grounded prose semantic hints.
- This locks the `ARSIZE`/`ARBURST` read-address sideband semantic hints as table-only rather than visual-caption recovered.

## Session update (2026-05-08 KG fixture for AXI read-address sideband alias exclusion)
- Tightened `axi_read_address_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside total, table-sourced, and prose semantic hints.
- This locks the `ARSIZE`/`ARBURST` read-address sideband semantic hints as table-only rather than phrase-alias recovered.

## Session update (2026-05-08 KG fixture for AXI read-address sideband prose exclusion)
- Tightened `axi_read_address_sideband_stability_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside total and table-sourced semantic hints.
- This locks the `ARSIZE`/`ARBURST` read-address sideband semantic hints as table-only rather than prose-derived.

## Session update (2026-05-08 KG fixture for field-table VLM exclusion)
- Tightened `table_misclassification_field_table_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks misclassified field-table rows as free of VLM timing-annotation semantic-role hints and closes the current zero-total source-split scan.

## Session update (2026-05-08 KG fixture for visual-motif family-mismatch VLM exclusion)
- Tightened `visual_motif_prior_protocol_family_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside zero aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks visual-motif protocol-family mismatch as free of VLM timing-annotation role hints.

## Session update (2026-05-08 KG fixture for visual-motif family-mismatch alias exclusion)
- Tightened `visual_motif_prior_protocol_family_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks visual-motif protocol-family mismatch as free of alias-grounded prose role hints.
...
- This locks visual-motif protocol-family mismatch as free of table-sourced role hints.

## Session update (2026-05-08 KG fixture for prior-guided visual-motif VLM exclusion)
- Tightened `visual_motif_prior_guided_diagram_classification_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside zero aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks prior-guided visual-motif classification as free of VLM timing-annotation role hints.
...
- Tightened `visual_motif_prior_guided_diagram_classification_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside zero aggregate, table-sourced, and visual-caption semantic hints.
- This locks prior-guided visual-motif classification as free of ordinary prose role hints.

## Session update (2026-05-08 KG fixture for prior-guided visual-motif table exclusion)
- Tightened `visual_motif_prior_guided_diagram_classification_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside zero aggregate and visual-caption semantic hints.
- This locks prior-guided visual-motif classification as free of table-sourced role hints.

## Session update (2026-05-08 KG fixture for no-prior visual-motif VLM exclusion)
...
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside zero aggregate, table-sourced, prose, alias-grounded prose, and visual-caption semantic hints.
- This locks no-prior visual-motif ambiguity as free of VLM timing-annotation role hints.

## Session update (2026-05-08 KG fixture for no-prior visual-motif alias exclusion)
- Tightened `visual_motif_prior_guided_diagram_classification_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside zero aggregate, table-sourced, prose, and visual-caption semantic hints.
- This locks no-prior visual-motif ambiguity as free of alias-grounded prose role hints.

## Session update (2026-05-08 KG fixture for no-prior visual-motif prose exclusion)
- Tightened `visual_motif_prior_guided_diagram_classification_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside zero aggregate, table-sourced, and visual-caption semantic hints.
- This locks no-prior visual-motif ambiguity as free of ordinary prose role hints.
...
- Tightened `visual_semantic_prior_guided_caption_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside zero aggregate, table-sourced, prose, and visual-caption semantic hints.
- This locks no-prior visual-caption semantic phrase recovery as free of alias-grounded prose role hints.

## Session update (2026-05-08 KG fixture for no-prior visual-caption prose exclusion)
- Tightened `visual_semantic_prior_guided_caption_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside zero aggregate, table-sourced, and visual-caption semantic hints.
- This locks no-prior visual-caption semantic phrase recovery as free of ordinary prose role hints.

## Session update (2026-05-08 KG fixture for no-prior visual-caption table exclusion)
- Tightened `visual_semantic_prior_guided_caption_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside zero aggregate and visual-caption semantic hints.
- This locks no-prior visual-caption semantic phrase recovery as free of table-sourced role hints.

## Session update (2026-05-08 KG fixture for field-table visual-hint exclusion)
- Tightened `table_misclassification_field_table_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside zero aggregate semantic hints, zero table/prose/alias hints, and zero table-signal declaration provenance.
- This locks misclassified field-table rows as free of visual-caption semantic-role hints.

## Session update (2026-05-08 KG fixture for field-table alias-hint exclusion)
- Tightened `table_misclassification_field_table_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside zero aggregate semantic hints, zero table/prose hints, and zero table-signal declaration provenance.
- This locks misclassified field-table rows as free of alias-grounded prose semantic-role hints.

## Session update (2026-05-08 KG fixture for field-table prose-hint exclusion)
- Tightened `table_misclassification_field_table_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside zero aggregate semantic hints, zero table-sourced hints, and zero table-signal declaration provenance.
- This locks misclassified field-table rows as free of ordinary prose semantic-role hints.
...
- This locks rejected motion-only annotation prose as free of visual-caption semantic-role hints.

## Session update (2026-05-08 KG fixture for motion-only VLM alias exclusion)
- Tightened `vlm_timing_motion_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks rejected motion-only annotation prose as free of alias-grounded prose semantic-role hints.

## Session update (2026-05-08 KG fixture for motion-only VLM prose exclusion)
- Tightened `vlm_timing_motion_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks rejected motion-only annotation prose as free of ordinary prose semantic-role hints.

## Session update (2026-05-08 KG fixture for motion-only VLM table exclusion)
- Tightened `vlm_timing_motion_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks rejected motion-only annotation prose as free of table-sourced semantic-role hints.
...
- Tightened `vlm_timing_waveform_motion_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, prose, VLM timing-annotation, and timing extraction metrics.
- This locks rejected waveform-motion state labels as free of alias-grounded prose semantic-role hints.

## Session update (2026-05-08 KG fixture for waveform-motion VLM prose exclusion)
- Tightened `vlm_timing_waveform_motion_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate, table, VLM timing-annotation, and timing extraction metrics.
- This locks rejected waveform-motion state labels as free of ordinary prose semantic-role hints.

## Session update (2026-05-08 KG fixture for waveform-motion VLM table exclusion)
- Tightened `vlm_timing_waveform_motion_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside aggregate, VLM timing-annotation, and timing extraction metrics.
- This locks rejected waveform-motion state labels as free of table-sourced semantic-role hints.

## Session update (2026-05-08 KG fixture for spurious VLM visual exclusion)
- Tightened `vlm_timing_spurious_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, alias-grounded prose, VLM timing-annotation, and timing extraction metrics.
- This locks spurious timing label/value noise as free of visual-caption semantic-role hints.

## Session update (2026-05-08 KG fixture for spurious VLM alias exclusion)
- Tightened `vlm_timing_spurious_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, prose, VLM timing-annotation, and timing extraction metrics.
- This locks spurious timing label/value noise as free of alias-grounded prose semantic-role hints.
...
- Tightened `vlm_timing_spurious_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside aggregate, VLM timing-annotation, and timing extraction metrics.
- This locks spurious timing label/value noise as free of table-sourced semantic-role hints.

## Session update (2026-05-08 KG fixture for indexed VLM visual exclusion)
- Tightened `vlm_timing_indexed_signal_value_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, alias-grounded prose, VLM timing-annotation, and timing extraction metrics.
- This locks indexed timing value-label noise as free of visual-caption semantic-role hints.
...
- Tightened `vlm_timing_indexed_signal_value_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate, table, VLM timing-annotation, and timing extraction metrics.
- This locks indexed timing value-label noise as free of ordinary prose semantic-role hints.

## Session update (2026-05-07 KG fixture for indexed VLM table exclusion)
- Tightened `vlm_timing_indexed_signal_value_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside aggregate, VLM timing-annotation, and timing extraction metrics.
- This locks indexed timing value-label noise as free of table-sourced semantic-role hints.

## Session update (2026-05-07 KG fixture for cycle-qualified VLM visual exclusion)
- Tightened `vlm_timing_cycle_qualified_signal_value_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, alias-grounded prose, VLM timing-annotation, and timing extraction metrics.
- This locks cycle-qualified timing value-label noise as free of visual-caption semantic-role hints.

## Session update (2026-05-07 KG fixture for cycle-qualified VLM alias exclusion)
- Tightened `vlm_timing_cycle_qualified_signal_value_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, prose, VLM timing-annotation, and timing extraction metrics.
- This locks cycle-qualified timing value-label noise as free of alias-grounded prose semantic-role hints.

## Session update (2026-05-07 KG fixture for cycle-qualified VLM prose exclusion)
- Tightened `vlm_timing_cycle_qualified_signal_value_annotation_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate, table, VLM timing-annotation, and timing extraction metrics.
- This locks cycle-qualified timing value-label noise as free of ordinary prose semantic-role hints.
...
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, alias-grounded prose, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset-release timing evidence as free of visual-caption semantic-role hints.

## Session update (2026-05-07 KG fixture for active-low VLM deassertion alias exclusion)
- Tightened `vlm_timing_active_low_deassertion_equivalence_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, prose, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset-release timing evidence as free of alias-grounded prose semantic-role hints.

## Session update (2026-05-07 KG fixture for active-low VLM deassertion prose exclusion)
- Tightened `vlm_timing_active_low_deassertion_equivalence_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate, table, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset-release timing evidence as free of ordinary prose semantic-role hints.

## Session update (2026-05-07 KG fixture for active-low VLM deassertion table exclusion)
- Tightened `vlm_timing_active_low_deassertion_equivalence_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside aggregate, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset-release timing evidence as free of table-sourced semantic-role hints.

## Session update (2026-05-07 KG fixture for active-low VLM assertion visual exclusion)
- Tightened `vlm_timing_active_low_assertion_equivalence_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, alias-grounded prose, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset assertion timing evidence as free of visual-caption semantic-role hints.

## Session update (2026-05-07 KG fixture for active-low VLM assertion alias exclusion)
- Tightened `vlm_timing_active_low_assertion_equivalence_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, prose, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset assertion timing evidence as free of alias-grounded prose semantic-role hints.

## Session update (2026-05-07 KG fixture for active-low VLM assertion prose exclusion)
- Tightened `vlm_timing_active_low_assertion_equivalence_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 0` alongside aggregate, table, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset assertion timing evidence as free of ordinary prose semantic-role hints.

## Session update (2026-05-07 KG fixture for active-low VLM assertion table exclusion)
- Tightened `vlm_timing_active_low_assertion_equivalence_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside aggregate, VLM timing-annotation, and timing extraction metrics.
- This locks active-low reset assertion timing evidence as free of table-sourced semantic-role hints.

## Session update (2026-05-07 KG fixture for cross-modality grounding timing-extraction absence)
- Tightened `cross_modality_semantic_grounding_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its table-plus-visual-caption semantic-hint source split.
- This locks decisive `XREQ` cross-modality semantic grounding as free of timing-diagram extraction support.

## Session update (2026-05-07 KG fixture for cross-modality semantic-conflict timing-extraction absence)
- Tightened `cross_modality_semantic_conflict_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its table-plus-visual-caption semantic-hint source split.
- This locks the cross-modality semantic-conflict negative as free of timing-diagram extraction support.

## Session update (2026-05-07 KG fixture for conflicting semantic phrase timing-extraction absence)
...
- This locks the equal valid-like and ready-like learned-prior guard as free of timing-diagram extraction support for the local `XCTRL` phrase.

## Session update (2026-05-07 KG fixture for conflicting semantic phrase VLM exclusion)
- Tightened `semantic_prior_conflicting_roles_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks the equal valid-like and ready-like learned-prior guard as free of VLM timing-annotation evidence for the local `XCTRL` phrase.

## Session update (2026-05-07 KG fixture for conflicting semantic phrase visual exclusion)
- Tightened `semantic_prior_conflicting_roles_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks the equal valid-like and ready-like learned-prior guard as free of visual-caption semantic hints for the local `XCTRL` phrase.

## Session update (2026-05-07 KG fixture for conflicting semantic phrase alias exclusion)
- Tightened `semantic_prior_conflicting_roles_negative` in `crates/specforge/test_data/kg_quality/`.
...
- This locks the equal valid-like and ready-like learned-prior guard as free of alias-grounded prose semantic hints for the local `XCTRL` phrase.

## Session update (2026-05-07 KG fixture for conflicting semantic phrase table exclusion)
- Tightened `semantic_prior_conflicting_roles_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and prose hint metrics.
- This locks the equal valid-like and ready-like learned-prior guard as free of table semantic hints for the local `XCTRL` phrase.

## Session update (2026-05-07 KG fixture for broad semantic phrase timing-extraction absence)
- Tightened `semantic_prior_broad_phrase_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the one-token broad learned-prior guard as free of timing-diagram extraction support for the local `XFLOW` phrase.

## Session update (2026-05-07 KG fixture for broad semantic phrase VLM exclusion)
- Tightened `semantic_prior_broad_phrase_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the one-token broad learned-prior guard as free of VLM timing-annotation evidence for the local `XFLOW` phrase.

## Session update (2026-05-07 KG fixture for broad semantic phrase visual exclusion)
- Tightened `semantic_prior_broad_phrase_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the one-token broad learned-prior guard as free of visual-caption semantic hints for the local `XFLOW` phrase.
...
- Tightened `semantic_prior_broad_phrase_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and prose hint metrics.
- This locks the one-token broad learned-prior guard as free of table semantic hints for the local `XFLOW` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase mismatch timing-extraction absence)
- Tightened `semantic_prior_phrase_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the unmatched learned-prior guard as free of timing-diagram extraction support for the local `XFLOW` phrase.
...
- Tightened `semantic_prior_phrase_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the unmatched learned-prior guard as free of visual-caption semantic hints for the local `XFLOW` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase mismatch alias exclusion)
- Tightened `semantic_prior_phrase_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the unmatched learned-prior guard as free of alias-grounded prose semantic hints for the local `XFLOW` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase mismatch table exclusion)
- Tightened `semantic_prior_phrase_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and prose hint metrics.
- This locks the unmatched learned-prior guard as free of table semantic hints for the local `XFLOW` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase source-kind timing-extraction absence)
- Tightened `semantic_prior_source_kind_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the visual-caption-only prior guard as free of timing-diagram extraction support for the prose-local `XSTAGE` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase source-kind VLM exclusion)
- Tightened `semantic_prior_source_kind_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the visual-caption-only prior guard as free of VLM timing-annotation evidence for the prose-local `XSTAGE` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase source-kind visual exclusion)
- Tightened `semantic_prior_source_kind_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the visual-caption-only prior guard as free of visual-caption semantic hints for the prose-local `XSTAGE` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase source-kind alias exclusion)
- Tightened `semantic_prior_source_kind_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the visual-caption-only prior guard as free of alias-grounded prose hints for the prose-local `XSTAGE` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase source-kind table exclusion)
- Tightened `semantic_prior_source_kind_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and prose hint metrics.
- This locks the visual-caption-only prior guard as free of table semantic hints for the prose-local `XSTAGE` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase family-mismatch timing-extraction absence)
- Tightened `semantic_prior_protocol_family_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the unrelated APB prior guard as free of timing-diagram extraction support for the AXI-local `XREQ` phrase.
...
- Tightened `semantic_prior_protocol_family_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the unrelated APB prior guard as free of visual-caption hints for the AXI-local `XREQ` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase family-mismatch alias exclusion)
- Tightened `semantic_prior_protocol_family_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the unrelated APB prior guard as free of alias-grounded prose hints for the AXI-local `XREQ` phrase.

## Session update (2026-05-07 KG fixture for semantic phrase family-mismatch table exclusion)
- Tightened `semantic_prior_protocol_family_mismatch_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and prose hint metrics.
- This locks the unrelated APB prior guard as free of table semantic hints for the AXI-local `XREQ` phrase.

## Session update (2026-05-07 KG fixture for no-prior semantic phrase timing-extraction absence)
- Tightened `semantic_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the `XACK can receive the transfer` no-prior mirror as free of timing-diagram extraction support when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior semantic phrase VLM exclusion)
- Tightened `semantic_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the `XACK can receive the transfer` no-prior mirror as free of VLM timing-annotation evidence when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior semantic phrase visual exclusion)
- Tightened `semantic_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the `XACK can receive the transfer` no-prior mirror as free of visual-caption hints when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior semantic phrase alias exclusion)
- Tightened `semantic_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the `XACK can receive the transfer` no-prior mirror as free of alias-grounded prose hints when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior semantic phrase table exclusion)
- Tightened `semantic_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and prose hint metrics.
- This locks the `XACK can receive the transfer` no-prior mirror as free of table semantic hints when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior ready-sink phrase timing-extraction absence)
- Tightened `semantic_ready_sink_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the `XACK can sink the transfer` no-prior mirror as free of timing-diagram extraction support when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior ready-sink phrase VLM exclusion)
- Tightened `semantic_ready_sink_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the `XACK can sink the transfer` no-prior mirror as free of VLM timing-annotation evidence when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior ready-sink phrase visual exclusion)
- Tightened `semantic_ready_sink_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the `XACK can sink the transfer` no-prior mirror as free of visual-caption hints when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior ready-sink phrase alias exclusion)
- Tightened `semantic_ready_sink_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the `XACK can sink the transfer` no-prior mirror as free of alias-grounded prose hints when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior ready-sink phrase table exclusion)
- Tightened `semantic_ready_sink_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside total and prose hint metrics.
- This locks the `XACK can sink the transfer` no-prior mirror as free of table semantic hints when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior valid-like phrase timing-extraction absence)
- Tightened `semantic_valid_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the `XREQ can publish the beat` no-prior mirror as free of timing-diagram extraction support when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior valid-like phrase VLM exclusion)
- Tightened `semantic_valid_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the `XREQ can publish the beat` no-prior mirror as free of VLM timing-annotation evidence when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior valid-like phrase visual exclusion)
- Tightened `semantic_valid_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the `XREQ can publish the beat` no-prior mirror as free of visual-caption hints when learned memory is absent.

## Session update (2026-05-07 KG fixture for no-prior valid-like phrase alias exclusion)
- Tightened `semantic_valid_prior_guided_phrase_without_prior_negative` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the `XREQ can publish the beat` no-prior mirror as free of alias-grounded prose hints when learned memory is absent.
...
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the prior-guided `XREQ can publish the beat` semantic recovery as free of timing-diagram extraction support.

## Session update (2026-05-07 KG fixture for valid-like phrase prior VLM exclusion)
- Tightened `semantic_valid_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the prior-guided `XREQ can publish the beat` semantic recovery as free of VLM timing-annotation evidence.

## Session update (2026-05-07 KG fixture for valid-like phrase prior visual exclusion)
- Tightened `semantic_valid_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the prior-guided `XREQ can publish the beat` semantic recovery as free of visual-caption evidence.

## Session update (2026-05-07 KG fixture for valid-like phrase prior alias exclusion)
- Tightened `semantic_valid_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the prior-guided `XREQ can publish the beat` semantic recovery as free of alias-grounded prose evidence.

## Session update (2026-05-07 KG fixture for valid-like phrase prior table exclusion)
- Tightened `semantic_valid_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside its total and prose hint metrics.
- This locks the prior-guided `XREQ can publish the beat` semantic recovery as prose-only rather than table-derived.

## Session update (2026-05-07 KG fixture for ready-sink phrase prior timing-extraction absence)
- Tightened `semantic_ready_sink_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the prior-guided `XACK can sink the transfer` semantic recovery as free of timing-diagram extraction support.

## Session update (2026-05-07 KG fixture for ready-sink phrase prior VLM exclusion)
- Tightened `semantic_ready_sink_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the prior-guided `XACK can sink the transfer` semantic recovery as free of VLM timing-annotation evidence.

## Session update (2026-05-07 KG fixture for ready-sink phrase prior visual exclusion)
- Tightened `semantic_ready_sink_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the prior-guided `XACK can sink the transfer` semantic recovery as free of visual-caption evidence.
...
- Tightened `semantic_ready_sink_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside its total and prose hint metrics.
- This locks the prior-guided `XACK can sink the transfer` semantic recovery as prose-only rather than table-derived.

## Session update (2026-05-07 KG fixture for semantic phrase prior timing-extraction absence)
- Tightened `semantic_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the prior-guided `XACK can receive the transfer` semantic recovery as free of timing-diagram extraction support.

## Session update (2026-05-07 KG fixture for semantic phrase prior VLM exclusion)
- Tightened `semantic_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the prior-guided `XACK can receive the transfer` semantic recovery as free of VLM timing-annotation evidence.

## Session update (2026-05-07 KG fixture for semantic phrase prior visual exclusion)
- Tightened `semantic_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_visual_captions = 0` alongside aggregate, table, prose, and alias-grounded prose hint metrics.
- This locks the prior-guided `XACK can receive the transfer` semantic recovery as free of visual-caption evidence.

## Session update (2026-05-07 KG fixture for semantic phrase prior alias exclusion)
- Tightened `semantic_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the prior-guided `XACK can receive the transfer` semantic recovery as free of alias-grounded prose evidence.

## Session update (2026-05-07 KG fixture for semantic phrase prior table exclusion)
- Tightened `semantic_prior_guided_phrase_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 0` alongside its total and prose hint metrics.
- This locks the prior-guided `XACK can receive the transfer` semantic recovery as prose-only rather than table-derived.

## Session update (2026-05-07 KG fixture for AMBA-generic modality fallback timing-extraction absence)
- Tightened `semantic_modality_reliability_amba_generic_fallback_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the AMBA-generic fallback path as free of timing-diagram extraction support before learned modality reliability resolves `XCTRL`.

## Session update (2026-05-07 KG fixture for AMBA-generic modality fallback VLM exclusion)
- Tightened `semantic_modality_reliability_amba_generic_fallback_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the AMBA-generic fallback path as free of VLM timing-annotation evidence before learned modality reliability resolves `XCTRL`.
...
- Tightened `semantic_modality_reliability_amba_generic_fallback_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the AMBA-generic fallback path as free of alias-grounded prose before learned modality reliability resolves `XCTRL`.

## Session update (2026-05-07 KG fixture for AMBA-generic modality fallback prose split)
- Tightened `semantic_modality_reliability_amba_generic_fallback_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 1` alongside aggregate and table hint metrics.
- This locks the local prose-side observation as counted EvidenceIR before AMBA-generic semantic arbitration resolves `XCTRL`.
...
- Tightened `semantic_modality_reliability_prior_guided_conflict_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_vlm_timing_annotations = 0` alongside aggregate, table, prose, alias-grounded prose, and visual-caption hint metrics.
- This locks the prior-guided gold path as free of VLM timing-annotation evidence before learned modality reliability resolves `XCTRL`.
...
- Tightened `semantic_modality_reliability_prior_guided_conflict_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_alias_grounded_prose = 0` alongside aggregate, table, and prose hint metrics.
- This locks the prior-guided gold path as free of alias-grounded prose before learned modality reliability resolves `XCTRL`.

## Session update (2026-05-07 KG fixture for modality-prior gold prose split)
- Tightened `semantic_modality_reliability_prior_guided_conflict_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_prose = 1` alongside aggregate and table hint metrics.
- This locks the local prose-side observation as counted EvidenceIR before prior-guided semantic arbitration resolves `XCTRL`.

## Session update (2026-05-07 KG fixture for modality-prior gold table split)
- Tightened `semantic_modality_reliability_prior_guided_conflict_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints_from_tables = 1` alongside the aggregate hint metric.
- This locks the local table-side observation as counted EvidenceIR before prior-guided semantic arbitration resolves `XCTRL`.

## Session update (2026-05-07 KG fixture for modality-prior gold hint totals)
- Tightened `semantic_modality_reliability_prior_guided_conflict_gold` in `crates/specforge/test_data/kg_quality/`.
- The fixture now asserts evidence-stage `signal_semantic_hints = 2` for the prior-guided semantic-modality gold.
- This locks prior-guided role resolution as starting from exactly two counted local table/prose semantic hints before learned modality reliability breaks the tie.

## Session update (2026-05-07 KG fixture for no-prior modality timing-extraction absence)
...
- The fixture now asserts evidence-stage `timing_diagram_extractions = 0` alongside its full semantic-hint source split.
- This locks the local table/prose conflict as free of timing-diagram extraction support when modality memory is absent.
