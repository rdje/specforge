...
- the temporal-rule layer now also recovers bounded `cycle_window` latency from structured prose/timing text, including idiomatic one-cycle phrases like `next cycle`, `next tick`, `next rising edge`, and `next falling edge`, plus quantified tick/edge forms like `within 2 ticks` / `after 3 falling edges` and explicit later/ordinal forms like `two cycles later` / `on the third rising edge`, and exposes that coverage in validation
