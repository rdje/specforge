...
- that replay surface now spans both evidence-local and canonical-local loops: evidence-stage signal-semantic conflicts can drive a direct `nlp-enrich -> validate` plan on the current `EvidenceIR`, while canonical-stage gaps still use the bounded `EvidenceIR -> SemanticIR -> IntentIR? -> validate` rebuild lanes
