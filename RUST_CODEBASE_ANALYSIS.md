...
- the temporal-rule layer now also recovers bounded `cycle_window` latency from structured prose/timing text, including idiomatic one-cycle phrases like `next cycle`, `next tick`, `next rising edge`, `next falling edge`, `next posedge`, and `next negedge`, plus quantified tick/edge forms like `within 2 ticks` / `after 3 falling edges`, unit-first diagram labels like `tick T3` / `posedge T4`, and explicit later/ordinal forms like `two cycles later` / `on the third rising edge`; explicit shorthand edge phrases now also preserve the named edge itself for signal/conditional/timing temporal rules instead of silently falling back to the default clock edge, explicit local timing text like `rising edge of HCLK`, `HCLK rising edge`, or `HCLK posedge` can now override the document default `clock_signal`, named local cycle/tick phrasing like `same ACLK cycle` now inherits the same bounded rising-edge temporal anchor already used for default-clock timing, and known-signal-aware one-cycle phrasing like `next ACLK cycle` or `next HCLK rising edge` now recovers the same `1..1` window as the unnamed forms instead of getting stranded behind the inserted clock token
