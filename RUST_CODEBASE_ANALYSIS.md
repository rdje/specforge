...
- the latest unseen protocol stress run is now AXI-Stream: it converges in `2` full pipeline iterations, validates at `80/100 GOOD`, and the latest truthfulness fixes remove both the spurious `TVALID` valid/ready conflict and the bogus `control information` producer while leaving graph-direction coverage and unresolved consumers as the dominant remaining gaps
