...
- `ir::clarification` now normalizes governed unresolved observations as `ClarificationNeed` and plans at most
  one blocking plus one advisory schema-1 packet through `plan_clarifications`.
- Exact equivalence/revision/reason/schema bindings govern grouping. Evidence and alternatives canonicalize;
  identical downstream targets merge with blocking as a conservative OR.
- Available autonomous actions and transitive dependents are withheld until replan. Remaining dependency
  components classify together, topologically order, reject cycles, and rank by structural reach only.
- Fifteen focused clarification tests pass (five planner cases added). The compiled boundary is 79 modules /
  41 families / 168 fields / 141 declared rows; current flow is 2,347 functions / 14,294 helper edges / 12,514
  decision sites / 1,459 semantic macros, with six declared non-authoritative regions. Three reason-bearing
  Rust-1.95 Clippy exceptions preserve intentional test-support proof-persistence fallbacks; no product
