...
- the first `R15g` corpus knowledge-base plane now exists too: tracked `corpus_kb/` pages can preserve reviewable cross-document synthesis, and `specforge corpus-kb` refreshes a managed validation-finding page from validation report sidecars without mutating canonical IR or typed priors
...
- the workspace currently validates through `bash scripts/run_ci.sh`, which runs Rust formatting, Clippy with `-D warnings`, Rust tests with `RUSTFLAGS="-D warnings"`, rustdoc with `RUSTDOCFLAGS="-D warnings"`, and the mdBook docs build; after the `R15g` corpus-KB slice the full local CI path reports clean Clippy, 307 passing Rust tests, clean Rust API docs, and a successful mdBook build
