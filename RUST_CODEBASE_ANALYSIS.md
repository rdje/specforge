...
- the workspace currently validates through `bash scripts/run_ci.sh`, which runs Rust formatting, Clippy with `-D warnings`, Rust tests with `RUSTFLAGS="-D warnings"`, rustdoc with `RUSTDOCFLAGS="-D warnings"`, and the mdBook docs build; after the signal-table inventory authority slice the full local CI path reports clean formatting, clean Clippy, `319` passing Rust tests, clean Rust API docs, and a successful mdBook build
