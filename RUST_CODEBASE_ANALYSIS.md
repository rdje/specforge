...
- latest local validation snapshot is now AXI `85/100 GOOD`, APB `94/100 EXCELLENT`, AHB `94/100 EXCELLENT`, and AXI-Stream `90/100 EXCELLENT`; this refresh replaced a stale optimistic snapshot, fresh `SourceIR -> EvidenceIR -> SemanticIR -> IntentIR` rebuilds restored APB/AHB to the excellent lane, and the latest AXI truthfulness fixes removed the last blocked-handshake residual, the false `AWAKEUP` / `CRVALID` semantic-role conflicts, and the false `ACLK` / `ARESETN` interface-direction conflicts while leaving AXI as the main remaining live quality outlier
