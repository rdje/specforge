...
- `specforge validate` now reports `table_signal_declaration_provenance` for `EvidenceIR` plus `with_table_support` for `SemanticIR` and `IntentIR`, making both evidence-stage table-declaration provenance and canonical table-backed signal coverage visible without replacing exact per-signal provenance checks
