...
- resolved signal polarity now also survives directly on canonical `InterfaceSignalRecord`s and is visible in validator metrics as `with_resolved_polarity`, but the current projected AMBA artifacts still report `0`, which is the honest signal that real polarity extraction coverage still needs to improve
