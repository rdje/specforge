...
- the latest unseen protocol stress run is now AXI-Stream: it converges in `2` full pipeline iterations, validates at `88/100 GOOD`, and the latest truthfulness fixes now recover parity-check signal ownership from the local `Check Signal / Signals Covered` table, bringing graph-direction coverage to `21/21`; the dominant remaining structural gap is now missing widths on `TDESTCHK` / `TIDCHK` / `TSTRBCHK` / `TUSERCHK` / `TWAKEUPCHK` plus unresolved producers for `ACLK` / `ARESETN`
