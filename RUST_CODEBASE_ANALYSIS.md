...
- the temporal-rule layer now also recovers bounded `cycle_window` latency from structured prose/timing text, including idiomatic one-cycle phrases like `next cycle`, `next tick`, and `next rising edge` plus explicit later/ordinal forms like `two cycles later` and `on the third rising edge`, and exposes that coverage in validation
