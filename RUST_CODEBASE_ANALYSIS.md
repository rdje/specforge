...
- the latest unseen protocol stress run is now AXI-Stream: it converges in `2` full pipeline iterations, validates at `80/100 GOOD`, and the latest truthfulness fixes remove the spurious `TVALID` valid/ready conflict, the bogus `control information` producer, and the missing consumer-side connectivity while leaving graph-direction coverage as the dominant remaining gap
