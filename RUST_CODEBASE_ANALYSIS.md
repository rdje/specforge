...
- the first `R15g` corpus knowledge-base plane now exists too: tracked `corpus_kb/` pages can preserve reviewable cross-document synthesis, and `specforge corpus-kb` refreshes managed validation-finding and KG fixture-result pages from validation report sidecars / benchmark fixture outcomes without mutating canonical IR or typed priors
- the tracked KG-quality benchmark surface currently contains 55 fixtures, including VLM timing spurious-annotation/sample-index-label rejection, waveform-motion rejection, VLM state-machine label-noise, undeclared-transition, duplicate-initial, multiple-initial, and missing-initial coverage, active-low VLM timing polarity-equivalence coverage, and collective, mixed clause-local, and detached mixed-polarity negative non-reset control polarity coverage
...
- the workspace currently validates through `bash scripts/run_ci.sh`, which runs Rust formatting, Clippy with `-D warnings`, Rust tests with `RUSTFLAGS="-D warnings"`, rustdoc with `RUSTDOCFLAGS="-D warnings"`, and the mdBook docs build; after the `R15g` KG fixture-result corpus-KB projection slice the full local CI path reports clean Clippy, `311` passing Rust tests, clean Rust API docs, and a successful mdBook build
...
- One remaining quality caveat is explicit: the corpus-KB benchmark projection currently reuses the validator path that prints detailed stage reports while it computes fixture expectations; future cleanup should separate silent report construction from command-line validation printing.
