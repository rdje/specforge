...
## Current snapshot
- ROADMAP-TASKTREE-COVERAGE task tree (R0, governance) — **ACTIVE** (opened `2026-05-31`, user doctrine directive): turning the WHOLE roadmap into owned, audited task-trees + locking ROADMAP↔codebase↔mdBook (zero drift). `.1` DONE — created the missing `docs/TASK_TREE_README.md` (system + 5-point doctrine) + measured the coverage gap (16 uncovered milestones: R1–R5, R8–R13, R15c–g) + registered the umbrella. Frontier `.2` (backfill+audit R1–R5), then `.3` (R8–R13), `.4` (R15c–g), `.5` (alignment lock + close): In Progress
