...
## Current snapshot
- CVE-PROSE-EXTRACTION task tree (R16) — **ACTIVE** (opened `2026-05-30`, user-greenlit): wiring the live prose→`ActorContract` extractor into the dormant R16 constrained-verified surface (`schema_rejects=0 entailment_fails=0 template_hits=0` today). New `extract-contracts` command (parallel to `nlp_enrich`) asks Qwen via Ollama for `ActorContract` JSON → fails-closed `parse_constrained_contract` → `apply_entailment_to_contract` → survivors fold into `actor_contracts` before fusion/fidelity → IntentIR carries → validate counts light up. `.1` design + `.2` typed-surface/fold + `.3` producer/command DONE. `.3` added `commands/extract_contracts.rs` + the `extract-contracts` CLI command (Qwen via Ollama): pure `classify_response` (none→skip / malformed→schema_reject / signals-present→Lowerable / signals-absent→entailment-Residual) + 5 tests; provenance overridden to the real statement, `contract_id` namespaced `cve:<id>`; lib `1154 → 1159`. Frontier `.4` (live Qwen e2e on a tiny synthetic spec + book method-doc + close). Safe-by-construction (malformed→schema_reject; unverifiable→Residual; never fabricated): In Progress
