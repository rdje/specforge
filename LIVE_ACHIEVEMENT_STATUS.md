...
## Current snapshot
- 🔣 TEMPORAL-RULE-LTL-RENDER (R6/R15e, `2026-06-04`) — **`.1` design DONE** (the Tier-1 headline adopt item from the `LITERATURE-GROUNDING` backlog): render mined `temporal_rules` in **standard LTL/MTL notation** — `G(antecedent → consequent)` with `X` (next tick) / `F[min,max]` (cycle window) — grounding SpecForge's temporal model in the spec-mining formalism (Pnueli/GoldMine/Texada `G(ante→cons)` templates). A new pure `ir/temporal_ltl.rs` renderer + an atom vocabulary for all 7 `TemporalPredicateRecord` variants (`sig==val`, `drive(actor,sig)`, `stable(…)`, `sample(…)`, `handshake(v,r)`); **opt-in** via `validate --ltl` (default output unchanged). **No IR persistence** (zero `kg-bench` fixture churn), **no `.isf`/PSL-SVA export** (that touches the FSMGen contract → explicit separate downstream tree). `.2` = renderer + tests + `validate --ltl` + book subsection (`domain/temporal-semantics.md`) + a Knowledge Map card + close. Docs-only: In Progress
