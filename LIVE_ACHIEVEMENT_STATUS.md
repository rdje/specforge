...
## Current snapshot
- CVE-PROSE-EXTRACTION task tree (R16) — **ACTIVE** (opened `2026-05-30`, user-greenlit): wiring the live prose→`ActorContract` extractor into the dormant R16 constrained-verified surface (`schema_rejects=0 entailment_fails=0 template_hits=0` today). New `extract-contracts` command (parallel to `nlp_enrich`) asks Qwen via Ollama for `ActorContract` JSON → fails-closed `parse_constrained_contract` → `apply_entailment_to_contract` → survivors fold into `actor_contracts` before fusion/fidelity → IntentIR carries → validate counts light up. `.1` design + `.2` typed-surface/fold DONE (additive `EvidenceIR.extracted_contracts` + `ConstrainedExtractionStats` carried to Semantic/Intent; fold before fusion; validate reads `schema_rejects`; lib `1151 → 1154`, zero artifact churn); frontier `.3` (producer + command, mock-tested). Safe-by-construction (malformed→schema_reject; unverifiable→Residual; never fabricated): In Progress
