...
## Current snapshot
- BOOK-METHOD-DOC (R0, active) — standing convention+close-rule: every task-tree's implementation & verification documented thoroughly in the topically-correct mdBook chapter (placement map + memory; `.1` done, `.2` backfill pending); honest fix that the in-chat methodology was not previously in the book: In Progress
