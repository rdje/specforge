...
## Current snapshot
- R14-SIGNAL-RESOLVE task tree (R14) — **ACTIVE** (opened `2026-05-31`, user-delegated): Tier-3 LLM `signal_relation` extraction via a new additive `signal-resolve` command (Qwen via Ollama) — mines actor→signal `drives`/`reads` edges from the hard prose Tier-1 (tables) + Tier-2 (verb patterns) miss, appending `ActorSignalRelation` records to the EXISTING `EvidenceIR.actor_signal_relations` KG field (already consumed by SemanticIR → actor_ports/graph-direction). No new IR types; does NOT touch `nlp_enrich`. Grounding-gated (anti-fabrication) + deduped vs existing edges. `.1` design + `.2` producer/command DONE — `commands/signal_resolve.rs` + the `signal-resolve` CLI command; pure `classify_relation_response` (none/malformed/ungrounded→skip; valid drives|reads→`ActorSignalRelation`) tested over 5 paths; lib `1159 → 1164`. Frontier `.3` (verify skip-mode + book method-doc + close): In Progress
