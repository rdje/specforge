...
- The refreshed live AMBA baseline is now AXI `85/100 GOOD`, APB `94/100 EXCELLENT`, AHB `94/100 EXCELLENT`, and AXI-Stream `90/100 EXCELLENT`; the old `94/95/95/90` snapshot was stale relative to the current semantic/intent stack, fresh rebuilds from current `SourceIR` / `EvidenceIR` restored APB and AHB to the excellent lane, and the latest AXI semantic-hint hygiene fix removed the last blocked-handshake residual plus the false `AWAKEUP` / `CRVALID` semantic-role conflicts without inflating the score
