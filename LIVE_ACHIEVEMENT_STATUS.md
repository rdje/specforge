...
## Current snapshot
- 🔁 TEMPORAL-ANTECEDENT-RECALL (R8/R15e, `2026-06-02`) — **`.1` design DONE** (the **second catch** of the measure→catch→fix loop, surfaced by the just-closed `TEMPORAL-RULE-EVAL`): root-caused the temporal antecedent **under-capture** to `parse_temporal_condition_predicates` (`ir/semantic.rs`) — a coordinated signal list drops its leading members because the shared trailing value isn't distributed (*"PBUSER valid when PSEL, PENABLE, **and** PREADY are asserted"* → antecedent only PREADY). Confirmed against the real APB `sigcon_0026` (the `condition_text` is complete; the parser drops PSEL+PENABLE). `.2` = distribute the single shared trailing value across the list (tightly guarded: ≥2 signals, one distinct value, fill only value-less clauses) + unit test + **eval re-run** (expected P 0.400→0.600, R 0.667→1.000, verifiable now at SemanticIR-build time — no re-ingest) + book refresh + close. The FP cohort (degenerate header / `*_WIDTH` subject) is upstream constraint-tier + stale-artifact (`CONSTRAINT-SUBJECT-PRECISION` class), out of scope. `.1` docs-only: In Progress
