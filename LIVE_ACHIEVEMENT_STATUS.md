...
## Current snapshot
- 📐 TEMPORAL-RULE-EVAL (R15d/R15e, `2026-06-02`) — **`.1` design + `.2` scorer DONE** (first Tier-1 item promoted from the `LITERATURE-GROUNDING` reach-full-potential backlog): a supervised precision/recall/F1 surface for **mined temporal rules** — SpecForge's previously-unmeasured third extraction surface (after `signal_constraint` + `actor_signal_relation`). `.2` landed in `eval.rs`: `EvalTask::TemporalRule` + `GoldFact::TemporalRule` (authored with the IR's own `TemporalPredicateRecord` shapes) + `temporal_predicate_key`/`temporal_rule_key`/`temporal_rule_record_key` + `index_temporal_rule_predictions`. Canonical key is **semantic, provenance-free** (clock edge + **sorted** antecedent/consequent predicate-keys + cycle_window; excludes rule_id/source_text/provenance/confidence → order- + case-insensitive). The closed-world `score_dataset` needed **no change** (already generic). 3 new unit tests; runner kept compiling honestly (LLM path errors for temporal → `.4`). **Zero extraction-behavior change; full CI GREEN (1211→1214).** Remaining: `.3` hand-labeled APB temporal gold seed (gold drafted independently from prose, ≥2 negatives) · `.4` runner (deterministic producer) + report + book + close. Continues the measure→catch→fix loop that found `CONSTRAINT-SUBJECT-PRECISION`: In Progress
