...
- Continue replacing the remaining direct `direction_hint` consumers with actor-relative graph semantics beyond the first `.fsm` explicit-module/top-composition, target-actor-aware standalone-direct, true-FSM control-read, and explicit top-link topology consumers; the latest adapter slices preserve recovered context in blocked artifacts and lock true-FSM graph-read recovery, so the next work should keep moving unrecovered consumers rather than widening backend scope prematurely
