...
- Continue replacing the remaining direct `direction_hint` consumers with actor-relative graph semantics beyond the first `.fsm` explicit-module actor-port/control-read, top-composition, target-actor-aware standalone-direct, true-FSM control-read, explicit top-link topology, and child-link topology consumers; the latest adapter slices preserve recovered context in blocked artifacts, extend module-local read recovery, guard that recovery against contradictory evidence, and now let explicit top links recover existing child-module port directions, so the next work should keep moving unrecovered consumers rather than widening backend scope prematurely
