...
- 🅿️ TEMPORAL-RULE-SVA-RENDER (R6/R15e, `2026-06-04`) — **DEFERRED / logged** (user direction: "log it for later, we'll decide"): the SpecForge-side `.isf`/IntentIR → **SVA** export (LTL renderer's tool-consumable successor — renders `temporal_rules` as IEEE 1800 SVA via `$stable`/`$isunknown`, `|=>` / `|-> ##[min:max]`, active-high convention, dropping non-SVA-expressible drive/sample predicates). `.1` design **captured** (the log); `.2` implementation **deferred pending the user's decision** between this and FSMGen-native LTL/MTL-in-ISF (`FSMGEN-LTL-MTL-SUGGESTION`) — if ISF gains LTL/MTL, this export may be unnecessary: In Progress (parked)
