...
## Current snapshot
- ROADMAP-TASKTREE-COVERAGE task tree (R0, governance) — **ACTIVE** (opened `2026-05-31`, user doctrine directive): turning the WHOLE roadmap into owned, audited task-trees + locking ROADMAP↔codebase↔mdBook (zero drift). `.1` DONE (created the missing `docs/TASK_TREE_README.md` + measured the coverage gap: 16 uncovered milestones R1–R5, R8–R13, R15c–g + registered the umbrella). `.2` DONE — `R1-R5-FOUNDATION-BACKFILL` owns + audits R1–R5 (all delivered: CLI/IntentIR pivot, SourceIR, EvidenceIR, SemanticIR, IntentIR; tested + book-covered; ROADMAP criteria met). Frontier `.3` (backfill+audit R8–R13), then `.4` (R15c–g), `.5` (ROADMAP↔code↔mdBook alignment lock + close): In Progress
