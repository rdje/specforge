...
## Current snapshot
- 📚 LITERATURE-GROUNDING (R0/R15e, `2026-06-02`) — **`.4`–`.12` DONE** (program now `.1`–`.12` complete; only `.13` synthesis remains): grounded all 9 remaining SpecForge aspects in verified published research via a **9-agent parallel research sweep**, every citation web-verified with a resolvable id, and the 4 highest-fabrication-risk recent works **personally spot-verified by the orchestrator** before commit (AssertLLM arXiv:2402.00386/ASP-DAC'25; Hybrid-NL2SVA 2506.21569/MLCAD'25; QiMeng-CodeV-SVA 2603.14239; MLLM-VRDU survey 2507.09861 — all real) + 1 unverifiable secondary survey (2408.01287) **DROPPED** per the anti-hallucination discipline. 9 docs under `docs/research/grounding/`: staged-ir (LLVM/MLIR/nanopass), requirements-extraction (RFC 2119/NLP4RE/NoRBERT/ACE), knowledge-graph-relation-extraction (OpenIE/ReVerb/distant-supervision/KBP/Hogan), multimodal-fusion (LayoutLM v1–3/Donut/Dempster–Shafer), neuro-symbolic-bounded-llm (grammar-constrained decoding/NLI/RAG/SelfCheckGPT/neurosymbolic), cross-document-learning (Yarowsky/Riloff-Jones/NELL/Snorkel/continual), extraction-evaluation (MUC/van Rijsbergen/Cohen-κ/Chao/capture–recapture-in-inspection), uncertainty-residual-honesty (Chow reject-option/selective-classification/conformal-prediction/open-set), spec-to-hardware (PSL/SVA/AssertLLM/HLS). Each doc separates *adopt-from-literature* vs *SpecForge's claimed novelty*. Docs-only; no code change: Done
