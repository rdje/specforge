...
## Current snapshot
- ROADMAP-TASKTREE-COVERAGE task tree (R0, governance) — **ACTIVE** (opened `2026-05-31`, user doctrine directive): turning the WHOLE roadmap into owned, audited task-trees + locking ROADMAP↔codebase↔mdBook (zero drift). `.1` DONE (created the missing `docs/TASK_TREE_README.md` + measured the coverage gap: 16 uncovered milestones R1–R5, R8–R13, R15c–g + registered the umbrella). `.2` DONE — `R1-R5-FOUNDATION-BACKFILL` owns + audits R1–R5 (all delivered: CLI/IntentIR pivot, SourceIR, EvidenceIR, SemanticIR, IntentIR; tested + book-covered; ROADMAP criteria met). `.3` DONE — `R8-R13-EXTRACTION-BACKFILL` owns + audits R8–R13 (SourceIR Tier-1 capture, EvidenceIR Tier-2 typed evidence, Tier-3 VLM, NLP-L3, multi-spec validation, Tier-2 relations; all delivered + book-covered; ROADMAP criteria met; R9 = Mostly Done, residual robustness-hardening noted). `.4` DONE — `R15C-R15G-LEARNING-PLANE-BACKFILL` (stays `active` — open lanes) owns + audits R15c–g (KG-guided multimodal rescans, cross-modality conflict arbitration, KG-quality benchmark hardening / `kg-bench`+153 fixtures, cross-document learning plane / `learn-priors`+`CorpusMemory`, corpus KB / `corpus-kb`+11 families); each leaf records delivered surface + remaining open scope; all book-covered. Frontier `.5` (ROADMAP↔code↔mdBook alignment lock + close umbrella): In Progress
