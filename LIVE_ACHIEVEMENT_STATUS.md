...
## Current snapshot
- 🔎 AMBIGUITY-PHRASE-DETECTOR (R8/R15e, `2026-06-04`) — **`.1` design DONE** (grounded gap from the `LITERATURE-GROUNDING` backlog, `requirements-extraction.md`; NASA ARM ICSE'97 / Berry-Kamsties): a **flag-only** detector (`ir/ambiguity.rs`) that scans EvidenceIR statement prose for vagueness / under-specification markers — NASA ARM weak phrases ("as appropriate", "if necessary", "and/or", "TBD", "but not limited to") + chip-spec "implementation-defined"/"vendor-specific" — and surfaces them in `validate` (Info finding `evidence_ambiguous_statements` + `ambiguous_statements` metric). **Modal verbs (MUST/SHALL/SHOULD/MAY) excluded** (normative strength, not vagueness). Residual-honesty: say where the spec is vague instead of treating it as precise. **Extraction-neutral** (report finding + metric, like the region-accounting detector); typed-residual routing deferred to a follow-up. `.2` = `ir/ambiguity.rs` (lexicon + detector + tests) + validate surface + book note + KM card + close. Docs-only: In Progress
