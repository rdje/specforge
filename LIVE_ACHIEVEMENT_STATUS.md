...
- R15-GRAPH-DIRECTION-MIGRATION task tree closed — 3 leaves resolved: audit of all production-code consumers across adapters.rs/semantic.rs/validate.rs/kg_bench.rs/learn_priors.rs (leaf .1), conversion of top-port resolved direction to graph-first with conflict-aware resolution mirroring preferred_signal_direction_hint() (leaf .2), validation/semantic consumers confirmed as structural/metric/test only with no TO-MIGRATE sites (leaf .3). All 1014 Rust tests pass, all 151 kg-bench fixtures pass, clippy clean: Done
- Continue replacing the remaining direct `direction_hint` consumers with actor-relative graph semantics beyond the first `.fsm` explicit-module actor-port/control-read, top-composition, target-actor-aware standalone-direct, true-FSM control-read, explicit top-link topology, explicit top actor-port boundary direction/width recovery, child-link topology, system-contract clock/reset consumers, semantic validation compatibility-lag reporting, and selected top signal-inventory projection; the latest adapter slices preserve recovered context in blocked artifacts, extend module-local read recovery, guard those recovery paths against contradictory evidence, directly regression-lock the shared preferred direction resolver as graph-first and conflict-sticky, lock renderable signal registration so graph-only directions can emit size entries while flat-vs-graph disagreement leaves no stale entry, lock system-contract signal validation so graph-only clock/reset inputs are accepted but stale flat disagreement blocks, lock init-assignment, typed assignment, dual-output assignment, registered dual-output assignment, delayed-pulse, and compound-update renderability so graph-backed output targets can lower while stale flat disagreement fails the role check, lock delayed-pulse delay gating so zero-delay intent blocks without losing graph-backed target metadata, lock dual-output assignment-kind gating so nonsequential dual-output intent blocks without losing graph-backed target metadata, lock compound-update public-output gating so shorthand exposure blocks without losing graph-backed target metadata, lock typed assignment values, width-cast typed assignment values, dual-output assignment values, width-cast dual-output assignment values, registered dual-output assignment values, width-cast registered dual-output assignment values, compound-update amounts, width-cast compound-update amounts, guard, comparison-guard, control-expression, width-cast control-expression, unary-expression, width-cast unary-expression, binary-expression, width-cast binary-expression, transition-guard, selector-block, width-cast selector-block, branch-predicate, and width-cast branch-predicate renderability so graph-backed read-side signals can emit size entries while stale flat disagreement leaves no stale entry, keep delayed-pulse values and width-cast delayed-pulse values signal-free while literal-only pulse lowering remains active, keep scalar, width-cast scalar, unary, width-cast unary, binary, width-cast binary, enum, width-cast enum, unary enum-member, width-cast unary enum-member, binary enum-member, and width-cast binary enum-member renderability signal-free even when graph-backed signal references exist, keep width-mismatched top-link endpoint metadata visible while composition stays blocked, keep missing child-module, duplicate, and missing-link child selected-inventory confidence plus duplicate and missing-link child declaration confidence visible while composition stays blocked, keep undeclared target/source selected-inventory and retained-record confidence visible while composition stays blocked, keep source-side, target-side, and legacy source-side unemitted child selected-inventory confidence plus source-side, target-side, and legacy source-side unemitted child retained record confidence visible while composition stays blocked, keep source-side and target-side child direction-role, child actor-port direction, child topology direction/width, and sibling child-link width selected-inventory confidence plus source-side and target-side child direction-role retained record confidence visible while composition stays blocked, keep top actor-port direction, top actor-port width, duplicate top-port direction/width, widthless and parametric top-port, recovered top-port missing-module, top-without-child inventory, missing child-module, duplicate child-instance and missing-link multi-child inventory, undeclared target/source inventory, source-side, target-side, and legacy source-side unemitted child inventory, source-side and target-side child direction-role inventory, child actor-port direction inventory, child topology direction/width inventory, sibling child-link width inventory, top-link direction, top-target direction-role, child actor-port direction, child topology direction/width, top child-link width, and sibling child-link width conflict retained record confidence visible while composition stays blocked, lock the top-boundary direction and width evidence mergers so conflicts cannot be resurrected by later hints, keep topology-linked child outputs under composition diagnostics instead of stale child-local undriven-output blockers, let explicit top links and matching top actor ports recover boundary directions and widths without accepting contradictory topology or graph claims, keep recovered selected top inventory directions graph-backed instead of flattening them into compatibility hints, keep selected top graph-vs-explicit direction conflicts sticky in `fsm.signal_inventory`, preserve recovered selected top width/direction provenance categories, supporting IDs, and automation confidence from actor-port and top-link topology evidence including child system-contract width endpoints, preserve standalone system-contract signal and renderable-module/source-document support/confidence before emission, preserve top-child declaration support/confidence/root-kind plus child system-contract endpoint and renderable-module/source-document support/confidence before top-link width recovery and emitted child roots consume it, preserve the same recovered direction/width support/confidence on resolved top ports and renderable top-root ports for actor-port direction/width, top-link direction/width, and child system-contract width evidence including the resolved top-port child endpoint seam, lock the selected top root-kind decision to recovered top-port confidence instead of raw low-confidence declarations, distinguish graph-direction, flat-direction, width-conflict, canonical parametric-width, and actor-port parametric-width evidence from missing-evidence diagnostics at renderability boundaries, preserve conflicting flat canonical direction evidence so graph recovery cannot override contradictory explicit declarations, require numeric top public IO width evidence before emission instead of accepting implicit 1-bit fallback syntax, require top links to target emitted child module ports instead of advisory-only inventory entries including system-contract endpoints, let top-link topology recover missing widths across top-boundary/child endpoints and sibling child endpoints, let direct and explicit-module control inputs recover width-only actor-port shape without importing external actor direction, collapse contradictory top-boundary plus duplicate top-port direction/width evidence to unresolved state, let canonical system contracts materialize missing clock/reset input/1-bit adapter shape without overriding contradictory local shape, and keep semantic compatibility lag scoped to signals whose actor-relative graph direction is already resolved, so the next work should keep moving unrecovered consumers rather than widening backend scope prematurely
