...
## Current snapshot
- 📐 TEMPORAL-RULE-EVAL (R15d/R15e, `2026-06-02`) — **`.1` design DONE** (first Tier-1 item promoted from the `LITERATURE-GROUNDING` reach-full-potential backlog): own + design a supervised precision/recall/F1 surface for **mined temporal rules** — SpecForge's unmeasured third extraction surface (after `signal_constraint` + `actor_signal_relation`). Reuses `eval.rs`'s closed-world scorer with a new **semantic, provenance-free** canonical key over `TemporalRuleRecord` (clock edge + sorted antecedent/consequent predicate-keys + cycle_window; excludes rule_id/source_text/provenance/confidence; antecedents+consequents sorted → order-insensitive). Producer = the deterministic temporal parser (EvidenceIR→SemanticIR lowering). 4 leaves: `.2` scorer types+keys+tests · `.3` hand-labeled APB temporal gold seed (gold drafted independently from prose, ≥2 negatives) · `.4` runner+report+book+close. Pure additive measurement; zero extraction-behavior change; continues the measure→catch→fix loop that found `CONSTRAINT-SUBJECT-PRECISION`. `.1` docs-only: In Progress
