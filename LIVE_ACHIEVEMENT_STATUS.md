...
- Continue replacing the remaining direct `direction_hint` consumers with actor-relative graph semantics beyond the first `.fsm` explicit-module actor-port/control-read, top-composition, target-actor-aware standalone-direct, true-FSM control-read, explicit top-link topology, explicit top actor-port boundary direction/width recovery, child-link topology, and system-contract clock/reset consumers; the latest adapter slices preserve recovered context in blocked artifacts, extend module-local read recovery, guard those recovery paths against contradictory evidence, let explicit top links and matching top actor ports recover boundary directions and widths without accepting contradictory topology or graph claims, let top-link topology recover missing widths across top-boundary/child endpoints and sibling child endpoints, let direct and explicit-module control inputs recover width-only actor-port shape without importing external actor direction, collapse contradictory top-boundary plus duplicate top-port direction/width evidence to unresolved state, and let canonical system contracts recover existing clock/reset input/1-bit adapter shape without overriding contradictory local shape, so the next work should keep moving unrecovered consumers rather than widening backend scope prematurely
