...
- 🛠️ LLM-EXTRACTION-EVAL (R15d/R16, `2026-06-01`) — **In Progress** (`.1`–`.3` done; `.4` next): the missing labeled precision/recall/F1 eval for the LLM extraction passes, so model A/Bs (qwen2.5vl vs qwen3-vl:8b) are rigorous not eyeballed. `.2` DONE — `crates/specforge/src/eval.rs`: pure scorer (task canonical keys, TP/FP/FN→P/R/F1, closed-world over labeled statements) + `EvalItem`/`GoldFact` format + loader; 6 tests. `.3` DONE — `test_data/llm_eval/seed_apb.json`: 16 hand-labeled AMBA APB items (8/task, ≥2 negatives), gold drafted independently from prose; loader accepts file-or-dir. `.4` NEXT — provider-gated `eval-extraction` runner over the real command path (design captured: extract a reusable no-write extraction core from `nlp-enrich`/`signal-resolve`). v1 = 2 text tasks; ActorContract + VLM enrich deferred. CI green (1206 tests): In Progress
