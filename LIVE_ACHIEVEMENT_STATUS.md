...
## Current snapshot
- 📜 SPEC-MINING-PROVENANCE (R0/R15e, `2026-06-04`) — **`.1` design DONE** (user directive): (1) name SpecForge's discipline **"specification mining"** (Ammons/Bodík/Larus POPL'02) with the forward-vs-backward novelty explicit — SpecForge mines intent *from the human-authored spec* (forward), not a spec *from an implementation* (the literature's backward direction); (2) build a per-author **adopt/defer provenance ledger** (`docs/research/grounding/adopt-defer-ledger.md`): for each leveraged author/work — *Take* (abstraction adopted) / *Leave-out + why* / *Instantiated-at* (where in SpecForge it lives) — in KM cards + book + the ledger. Sharpens the per-aspect grounding into per-author provenance so "what do we take from GoldMine?" is one lookup. `.2` = temporal trio (Pnueli/GoldMine/Texada — the `G(ante→cons)` LTL template our `temporal_rules`/`ir/temporal_ltl.rs` instantiate) + adopt the framing in README/book + KM card(s); `.3`+ = the rest of the swept authors + synthesis + close. Verify-every-citation carries over; advisory (no code change). Docs-only: In Progress
