...
## Current snapshot
- ISF-SYMBOL-SURFACE-EMIT.1 (R6, active) — FSMGen **answered** the type↔enum clarity request (upstream `c0b7eaa7`, locked by `t/1378`): emit **both** `(types (type NAME (bits k)))` + `(enums (NAME …))` per enum (`k = ceil(log2(members))`); unreferenced decls valid — which **validates SpecForge's existing build**. Bumped `subs/fsmgen` `88a7af9c → c0b7eaa7` (the answer boundary, not the mid-flight tip) to ingest it; full `scripts/run_ci.sh` green (`1148/0`, `.isf` still strict-valid); README pin + FSMGEN_FEEDBACK reconciled (request RESOLVED). Promoted the tree; emission of the built-but-discarded `(constants)`/`(types)`/`(enums)` surface (closing the count-vs-emission honesty gap) is `.2`: Done
