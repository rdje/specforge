...
## Current snapshot
- 🗺️ KNOWLEDGE-MAP-ADOPTION (R0, `2026-06-04`) — **`.1` design DONE** (user-directed: adopt the portable `KNOWLEDGE_MAP_ARCHITECTURE.md` bundle, the same way `MEMORY_ARCHITECTURE.md` was adopted): an **additive, derived, question-keyed retrieval layer** over durable facts that makes *archaeology* (re-deriving an already-logged fact from code/runtime) a structural impossibility. A fact = one front-mattered `.md` card with an `answers:` list (the questions an agent would grep) + `id`/`title`/`date` + `evidence`/`reverify`; a portable POSIX/awk generator derives `KNOWLEDGE_MAP.md` (Questions→fact); a derive-and-diff check gates pre-commit + CI so it can't drift. Composes with `MEMORY_ARCHITECTURE.md` + `docs/decisions/` + task-trees; **replaces nothing, converts nothing**; cards added lazily. `.1` read the bundle end-to-end + mapped it onto SpecForge's existing enforcement (defaults already match → no config override; pre-commit `exec`→run-both; `run_ci.sh`/`ci.yml` gates). `.2` = copy bundle verbatim + wire both gates + register in bootstrap + seed 3 archaeology-trap cards (docling-cpu, llm-provider-default, temporal-eval-stale-FPs) + verify + close. Docs-only: In Progress
