...
- latest_commit (baseline): **`ef89d3e4`** `CORPUS-COVERAGE.2 — re-ingest frontier unblocked via git-ignored chipdoc symlink (ownership/provisioning)`. **A CXS re-ingest slice commit is being created now.** **194 ahead** of `origin/main` (→ 195 after; push at ~200). Tree clean except untracked `.claude/settings.json`.
...
- next_action: **re-ingest doc #2 = AMBA GFB (`ihi0083`, 0.47 MB)** from `.cache/local-references/chipdoc/arm/amba/specialized/gfb/current/` via `DOCLING_DEVICE=cpu ./target/release/specforge ingest <symlink path>`; then evidence→semantic→intent→`adapt --target isf`; FSMGen `--strict --check` spot-check; record the result row in `CORPUS-COVERAGE.md` `.2` log; commit. **Doc #1 CXS DONE** (renderable `.isf`, 0 fsmgen diagnostics). PNT order: CXS✓→GFB→ACC→ATP→TileLink×2→LPI→DTI→CHI-C2C, then register/TRM/ISA docs. ONE doc per slice; commit each. Per-doc command pattern proven; each ~2 min Docling + fast cascade.
