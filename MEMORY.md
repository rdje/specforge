...
- trees + priority order: (1) `FSMGEN-ISSUE-REPORTING` (R0, user-directed 2026-05-18 — file the 2 genuine FSMGen findings via `subs/fsmgen/docs/DOWNSTREAM_ISSUE_REPORTING.md`; bundles under `docs/fsmgen-issues/`, submodule NEVER written), (2) `ISF-TEMPORAL-LOWERING` (R15b, critical — temporal_rules→`.isf`; transaction_count misreports), (3) `ISF-ONLY-IR-PRUNE` (R6 — prune ONLY `init_assignments`/`decision_tree_fragments`; `.3` SUPERSEDED, "keep" regular_states/state_transitions, no regression), (4) `AUDIT-DOC-RECONCILE` (R0 — R15 text + book converge flags)
...
- completed_count: `.1`(`f557baba`) `.2`-split(`b92f29ef`) `.2.1`(`5be3b278`) IR-PRUNE.3-supersede(`3e3fe823`) `.2.2`(`0ccdc8f0` windowed temporal_rules→`(contract …)` live-verified) FSMGEN-ISSUE-REPORTING-tree(`1c3a5d37`) FSMGEN-ISSUE-REPORTING.1(`1b028453` F1 bundle built+reproduced, submodule clean) FSMGEN-ISSUE-REPORTING.2(committing now: F2 stage bundle built+reproduced, submodule clean). Next frontier: `FSMGEN-ISSUE-REPORTING.3` (reference both bundle ids in `docs/FSMGEN_FEEDBACK.md` + record 2-not-3 + close), then resume `ISF-TEMPORAL-LOWERING.2.3`
...
- ACTIVE PNT (not exhausted): `FSMGEN-ISSUE-REPORTING` (frontier `.2`), `ISF-TEMPORAL-LOWERING` (`.2.3`), `ISF-ONLY-IR-PRUNE` (`.1`), `AUDIT-DOC-RECONCILE` (`.1`). SpecForge pipeline: `SourceIR→EvidenceIR→SemanticIR→IntentIR→.isf` only.
