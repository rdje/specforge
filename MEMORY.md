...
- latest_commit (baseline): **`20ec8e78`** `BOOK-COMMAND-COVERAGE.2 — dedicated nli-verify section in the Commands chapter`. **62 ahead** of `origin/main` (push at ~200 → HOLD); this MEMORY post-commit refresh → **63 ahead**. Untracked (stays): `.claude/settings.json`, `.cache/` (gitignored symlink).
