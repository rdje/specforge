...
- latest_commit: **`addc9fef`, 33 ahead of `origin/main`, NOT pushed** (owner hold; push ~200 or on ask). CI green = **1301 tests** (post-`.3a`; commit pending). KM 21 facts. `subs/fsmgen` pinned. Local models: `qwen2.5:14b-instruct` (text/NLI) + `qwen2.5vl:7b` (vision); `ollama serve` may be needed.
...
- next_action: **`WIRE-BASED-100.3`** remaining — the **2 prose residuals** (partial normative statements) are APB's genuine remaining completeness items; then `.2` value-constraint recall, `.4` temporal completeness, `.5` roll AHB→AXI→SWD. Future: content-based column detection / rotation handling in the EXTRACTOR itself (`.5`, where a misaligned table is the SOLE signal source = a genuine miss not a duplicate); `table_0018` header-row recovery or docling re-ingest. ROOT still open: **tier-overlap degeneracy** (Pattern×Nlp DISJOINT facts) blocks conformal + capture-recapture. Owner principle (NON-NEGOTIABLE): every score objectively measured + demonstrated PER-FACT — no fake/gamed scoring (`feedback_scoring_rigor`).
- paused_work: none. in_flight_uncommitted: `.3a` code+docs staged-not-committed (run COMMIT.md next). blockers: none.
