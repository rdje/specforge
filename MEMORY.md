...
- Active unit: `LIVE-DOCUMENT-PRESSURE-HEADROOM.2c`. Open: `SOURCE-IR-REPRODUCIBILITY`
...
- Current state: `.2a`/`.2b` removed the 160-file cap on `docs/tasks/` as a DECLARED exemption, not a null, and
  retired the single-use authority after observing its banked-authority refusal RED on a clean tree.
...
- Next action: `LIVE-DOCUMENT-PRESSURE-HEADROOM.2c` — shard `docs/TASK_TREE.md` into a bounded landing plus
  derived catalog parts, following the two remedies this repo already proves: the Knowledge Map's landing plus
  15 question shards, and `fact_card_titles`' 5 title parts (both are derive-and-diff with a registry pinning
  each planned output's sha256/metrics, regenerated with a `--write` mode). Keep it generated with no
  hand-edited member list, keep every existing route into the index resolving, and replace the residual
  ~108-tree stop with a bound ordinary compliant work can pass without an authority edit. `.2a` relocated the
  stop here; until `.2c` lands, the directive is relocated, not delivered, and ADR 0045 says so.
