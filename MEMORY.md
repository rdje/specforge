...
- latest_commit_hash: `c156fa943cec833891fbffc1edc37470f25f3c67`
- latest_commit_brief_message: `test(adapter): keep top actor direction inventory provenance`
- note: this is the pre-slice-185 baseline for the active `BWFSC=200` batch; local commits are ahead of `origin/main`, and push is deferred until all 200 batch slices complete
...
- `13496c1` test(adapter): keep widthless top inventory provenance

## Current repository state
- active workspace member: `crates/specforge`
- branch: `main`
- branch state before current slice commit: `main` has local work for active `BWFSC=200` slice 185 and push is deferred; branch started this batch one docs-only commit ahead of `origin/main`
- files in flight:
...
- objective:
  ...
  - slice 185/200 extends the top actor-port width conflict regression to prove selected `ext_data` inventory keeps output direction and top-port provenance while width remains conflicted with actor-port width evidence
  - push is deferred until all `200` slices complete unless explicitly instructed otherwise or a real blocker stops the batch
- tracker effect:
  - live-status tracker now marks top actor-port width conflict selected inventory provenance coverage as `Done`
- verification status:
