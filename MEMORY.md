...
- next_action: **`.2` wire docs** — re-verify the gold battery on the already-promoted APB5/AHB/AXI canonical via `eval-extraction` (`seed_apb/ahb/axi` + `_temporal` + `seed_swd_derivation`, **`--provider skip` = NO model**, re-runs on a temp copy so the corpus is never mutated); then promote SWD `ihi0074_a` (1 cons) + re-verify `seed_swd_derivation`, revert-on-regress. Then **roll up** the corpus gauge summary + refresh `VALIDATION_SNAPSHOT.md`/`LIVE_ACHIEVEMENT_STATUS.md` and close `.3`/`.2`. Commit per slice.
