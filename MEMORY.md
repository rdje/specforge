...
- latest_commit_hash: `97bd187083281cedba29300bbdfc588a02a44550`
- latest_commit_brief_message: `test(rescan): lock replay-input rendering`
...
- objective:
  - add rescan-plan dry-run run-report coverage proving scoped pending and limited selection counters are preserved without execution
  - refresh live docs for the dry-run report count behavior
- tracker effect:
  - planned live-status row addition: `specforge rescan-plan` dry-run report selection counts are regression-locked: `Done`
- verification status:
  - initial focused test showed `pending_recommendations` is scoped by `--document-key`; assertion and docs were corrected to lock that contract
  - `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_dry_run_report_tracks_selection_counts` passed with `1` test
  - `cargo fmt --manifest-path Cargo.toml -- --check` passed
  - `bash scripts/run_docs_ci.sh` passed
  - `bash scripts/run_ci.sh` passed with `572` Rust tests plus warning-deny Clippy/rustdoc and mdBook validation
  - `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `130` fixtures and `0` failures
  - `git diff --check` passed
  - checkout-specific absolute path scan across tracked markdown passed
- current known local CI baseline:
  - `572` Rust tests plus warning-deny Clippy/rustdoc and mdBook validation
