...
- latest_commit (baseline): **`ab1be0ea`** `CORPUS-COVERAGE.2 — re-ingest #1: AMBA CXS (ihi0079) refreshed to a strict-valid .isf`. **A GFB re-ingest slice commit is being created now.** **195 ahead** of `origin/main` (→ 196 after; push at ~200). Tree clean except untracked `.claude/settings.json`.
...
- next_action: **re-ingest doc #3 = ACC (`ihi0076`, 0.62 MB)** at `.cache/local-references/chipdoc/arm/debug/interfaces/acc/current/IHI0076_A_2018-05-02_Advanced_Communications_Channel_Architecture_Specification.pdf`. Per-doc pattern (proven): `DOCLING_DEVICE=cpu ./target/release/specforge ingest <symlink path>` → evidence→semantic→intent→`adapt --target isf` → FSMGen `--strict --check` spot → record `.2` log row + advance this pointer → commit. **PNT order:** CXS✓ GFB✓ → ACC → ATP → TileLink 1.7.1 → TileLink 1.8.0 → LPI(`ihi0068`) → DTI(`ihi0088`) → CHI-C2C(`ihi0098_b`), then register/TRM/ISA docs. ONE doc/slice; commit each; ~2 min Docling + fast cascade per doc.
