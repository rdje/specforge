...
- latest_commit_hash: `dbb209ea059cdedd4adaaf5f69ddd59c734ef97d`
- latest_commit_brief_message: `fix(rescan): reject traversing replay paths`
...
- objective:
  - harden rescan-plan command-hint parsing so replay source/artifact paths cannot be explicit empty strings
  - refresh live docs and mdBook reference for non-empty relative path restrictions
- tracker effect:
  - planned live-status row addition: `specforge rescan-plan` empty replay artifact paths are rejected: `Done`
- verification status:
  - `cargo test --manifest-path Cargo.toml -p specforge rescan_plan_rejects_empty_replay_artifact_paths` passed with `1` test and `591` filtered out
  - `cargo fmt --manifest-path Cargo.toml -- --check` passed
  - `bash scripts/run_docs_ci.sh` passed
  - `bash scripts/run_ci.sh` passed with `592` Rust tests plus warning-deny Clippy/rustdoc and mdBook validation
  - `cargo run --manifest-path Cargo.toml -p specforge -- kg-bench` passed with `130` fixtures and `0` failures
  - `git diff --check` passed
  - checkout-specific absolute path scan across tracked markdown passed
- current known local CI baseline:
  - `592` Rust tests plus warning-deny Clippy/rustdoc and mdBook validation
  - `130/130` tracked KG fixtures

## Next exact steps
- commit slice 34 without pushing, then continue slice 35
