...
- latest_commit (baseline): **`a11e09ce`** `CORPUS-COVERAGE.2 — re-ingest #2: AMBA GFB (ihi0083)`. **An ACC re-ingest slice commit is being created now.** **196 ahead** of `origin/main` (→ 197 after; push at ~200 — push imminent). Tree clean except untracked `.claude/settings.json`.
...
- next_action: **re-ingest doc #4 = ATP (`ihi0082`, 0.69 MB)** at `.cache/local-references/chipdoc/arm/amba/specialized/atp/current/IHI0082_A_2019-03-15_AMBA_Adaptive_Traffic_Profiles_Specification.pdf`. Per-doc pattern (proven): `DOCLING_DEVICE=cpu ./target/release/specforge ingest <symlink path>` → evidence→semantic→intent→`adapt --target isf` → FSMGen `--strict --check`. **FSMGEN PATH GOTCHA:** compute `ABS="$PWD/$ISF"` BEFORE `cd subs/fsmgen` (a `$(cd ..; pwd)` inside the cd'd subshell resolves WRONG → spurious diagnostics). **PNT order:** CXS✓ GFB✓ ACC✓ → ATP → TileLink 1.7.1 → TileLink 1.8.0 → LPI(`ihi0068`) → DTI(`ihi0088`) → CHI-C2C(`ihi0098_b`), then register/TRM/ISA docs. ONE doc/slice; commit each. **At 197 ahead → PUSH at 200** (after ~3 more docs).
