...
- latest_commit (baseline): **`25f46b05`** `DOCTRINE-ENFORCEMENT-ADOPT.3 — fix check_task_acceptance.sh pipefail/SIGPIPE false-negative …`. **50 ahead** of `origin/main` (push at ~200 → HOLD). This `KG-ISF-COMPLETENESS.2a.iv` CODE commit → **51 ahead**. Untracked (stays): `.claude/settings.json`, `.cache/` (gitignored symlink).
