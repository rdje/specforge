...
- latest_commit_hash: `80b95a61cf0b520e3484b89318b570362aa51dbd`
- latest_commit_brief_message: `test(adapter): keep child width conflict provenance`
- note: this is the pre-slice-17 baseline for the active `BWFSC=100` batch; `main` is eighteen local commits ahead of `origin/main`, and push is deferred until the 100-slice batch completes unless the user explicitly redirects or a real blocker stops the batch
...
- branch: `main`
- branch state: `main` has local work for active `BWFSC=100` slice 17 and push is deferred until all 100 slices complete
- files in flight:
  - `CHANGES.md`
...
- objective:
  ...
  - slice 17/100 extends the child topology direction conflict regression to prove the conflicted child output inventory keeps retained interface provenance while stale `.fsm` emission remains blocked
