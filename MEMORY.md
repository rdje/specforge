...
- completed_count: `.1`(`f557baba`) `.2`-split(`b92f29ef`) `.2.1`(`5be3b278`) IR-PRUNE.3-supersede(`3e3fe823`) `.2.2`(`0ccdc8f0` windowed temporal_rules→`(contract …)` live-verified) FSMGEN-ISSUE-REPORTING-tree(`1c3a5d37`) FSMGEN-ISSUE-REPORTING.1(`1b028453` F1 bundle built+reproduced, submodule clean) FSMGEN-ISSUE-REPORTING.2(`c415fc25` F2 stage bundle) FSMGEN-ISSUE-REPORTING.3(`a687cbd0` tree CLOSED) ISF-TEMPORAL-LOWERING.2.3(`d4633e38` classifier Contract|Rule|Residual; live nvme strict-valid) ISF-TEMPORAL-LOWERING.2.4(committing now: txn/rule counts from emitted model; regression locks it; live nvme 109→0 / 250==250 / 92 residual; closes BOTH audit critical complaints). Next frontier: `ISF-TEMPORAL-LOWERING.3` (end-to-end regression: temporal-rules-bearing IntentIR via generic markdown pipeline → non-empty ISF temporal behavior + metric match + fsmgen-strict), then `.4` (close tree + book/ROADMAP sync)
...
- ACTIVE PNT (not exhausted): `FSMGEN-ISSUE-REPORTING` DONE/closed; remaining: `ISF-TEMPORAL-LOWERING` (frontier `.2.4` → `.3` → `.4`), `ISF-ONLY-IR-PRUNE` (`.1`), `AUDIT-DOC-RECONCILE` (`.1`). SpecForge pipeline: `SourceIR→EvidenceIR→SemanticIR→IntentIR→.isf` only.
