...
- next_action: PNT — NLI verifier CLOSED. Possible follow-up: **auto-route NotEntailed claims → residual inside `converge`** (the live gate; today `nli-verify` is a standalone report command). Otherwise backlog: gated (TEDS/GriTS [table gold], conformal [held-out set]) or marginal (MUC near-miss; per-relation P/R/F1; unguarded-`min>1` monitor). Local models now: `qwen2.5:14b-instruct` (text, for NLI) + `qwen2.5vl:7b` (vision); ollama daemon may need `ollama serve`. Reminder: grep `KNOWLEDGE_MAP.md`; **regularly sync mdBook (sole user surface)**; **no push until ~200 ahead or ask.**
- paused_work: none.
