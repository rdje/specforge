...
- completed_count: `.1`(`f557baba`) + `.2`-split(`b92f29ef`) + `.2.1`(`5be3b278`) + IR-PRUNE.3-supersede(`3e3fe823`) + `.2.2`(committing now: windowed temporal_rules→`(contract …)`, live on CXS corpus, `(within 0)` strict-reject caught+guarded). Next frontier: `ISF-TEMPORAL-LOWERING.2.3` (guard→drive `(rule …)`; residual for unrepresentable incl. `within 0`/HandshakeComplete)
