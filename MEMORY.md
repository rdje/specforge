...
- latest_commit (baseline): **`76525cef`** `CORPUS-COVERAGE.2 — re-ingest #7: AMBA LPI (ihi0068)`. **A DTI re-ingest slice commit is being created now → 2 ahead** of `origin/main`. Tree clean except untracked `.claude/settings.json`.
...
- next_action: **re-ingest doc #9 = CHI-C2C (`ihi0098_b`, 1.71 MB)** at `.cache/local-references/chipdoc/arm/amba/specialized/chi-c2c/current/IHI0098_B_2026-03-23_AMBA_CHI_Chip-to-Chip_C2C_Architecture_Specification.pdf`. Per-doc pattern (proven): `DOCLING_DEVICE=cpu ./target/release/specforge ingest <symlink path>` → evidence→semantic→intent→`adapt --target isf` → FSMGen `--strict --check` (**robust JSON parse** — read `success` + `diagnostics`; **compute `ABS="$PWD/$ISF"` BEFORE `cd subs/fsmgen`**). **PNT order:** CXS✓ GFB✓ ACC✓ ATP✓ TL1.7.1✓ TL1.8.0✓ LPI✓ DTI✓ → CHI-C2C → then register/TRM/ISA docs (big `.10` gains: GIC/SMMU/CoreSight/CCIX/AMD/NVMe-class). ONE doc/slice; commit each; push ~every 200.
