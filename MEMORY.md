...
- Next action: read `docs/tasks/RETAINED-BUNDLE-POPULATION-FROZEN.md`. `.2` decides what a newly
  retained document owes that frozen 24-row qualification (7/17 calibration/holdout,
  `declared_attempts: 51`, `final_signoff: deferred_to_SPEC-TO-INTENT-ALIGNMENT.6d.ii.f.v`). It needs
  that tree's owner and a decision record — it is a policy call, not a code edit, and `.3` waits on it.
