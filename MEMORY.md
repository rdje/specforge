...
- latest_commit_hash: `3827bd312358da36d2226dc2ea126e808e1d4626`
- latest_commit_brief_message: `test(adapter): keep duplicate child residual diagnostics`
- note: this is the pre-slice-31 baseline for the active `BWFSC=100` batch; `main` is thirty-two local commits ahead of `origin/main`, and push is deferred until the 100-slice batch completes unless the user explicitly redirects or a real blocker stops the batch
...
- `c182818` test(adapter): keep source unemitted residual diagnostics

## Current repository state
- active workspace member: `crates/specforge`
- branch: `main`
- branch state: `main` has local work for active `BWFSC=100` slice 31 and push is deferred until all 100 slices complete
- files in flight:
  - `CHANGES.md`
...
- objective:
  ...
  - slice 31/100 extends the undeclared top-target residual regression to prove child-module reference diagnostics remain visible alongside explicit top-port diagnostics while stale `.fsm` emission remains blocked
