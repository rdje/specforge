...
- Blockers: none. Standing hazards: **a green score is evidence only about the facts its gold names** — AXI's
  ...
  **A relation can become a declaration**: `synthesize_directions_from_relations` is the one EvidenceIR path
  ...
  (`[[corpus-canonical-currency-and-ownership]]`); never infer ownership from a mention; attribute a
  ...
  this file's cap is 50 lines. `durability.stale_check` and
  `scripts/validate_canonical_recovery_contract.py` are run by no gate (`LIVE-DOCUMENT-PRESSURE-HEADROOM.18`).
