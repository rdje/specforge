...
- completed_count: `.1`(`f557baba`) `.2`-split(`b92f29ef`) `.2.1`(`5be3b278`) IR-PRUNE.3-supersede(`3e3fe823`) `.2.2`(`0ccdc8f0` windowed temporal_rules→`(contract …)` live-verified) FSMGEN-ISSUE-REPORTING-tree(`1c3a5d37`) FSMGEN-ISSUE-REPORTING.1(`1b028453` F1 bundle built+reproduced, submodule clean) FSMGEN-ISSUE-REPORTING.2(`c415fc25` F2 stage bundle) FSMGEN-ISSUE-REPORTING.3(committing now: FSMGEN_FEEDBACK.md "Filed issue bundles" section + index → tree CLOSED). Next frontier: `ISF-TEMPORAL-LOWERING.2.3` (guard→drive `(rule …)`; residual for unrepresentable incl. `within 0`/HandshakeComplete)
...
- ACTIVE PNT (not exhausted): `FSMGEN-ISSUE-REPORTING` DONE/closed; remaining: `ISF-TEMPORAL-LOWERING` (frontier `.2.3`), `ISF-ONLY-IR-PRUNE` (`.1`), `AUDIT-DOC-RECONCILE` (`.1`). SpecForge pipeline: `SourceIR→EvidenceIR→SemanticIR→IntentIR→.isf` only.
