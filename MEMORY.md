...
- latest_commit_hash: `7a916f5e67e78a6e0f7a0a1b13f9463e7cd5306a`
- latest_commit_brief_message: `test(adapter): keep source role guidance diagnostics`
- note: this is the pre-slice-36 baseline for the active `BWFSC=100` batch; `main` is thirty-seven local commits ahead of `origin/main`, and push is deferred until the 100-slice batch completes unless the user explicitly redirects or a real blocker stops the batch
...
- `3827bd3` test(adapter): keep duplicate child residual diagnostics

## Current repository state
- active workspace member: `crates/specforge`
- branch: `main`
- branch state: `main` has local work for active `BWFSC=100` slice 36 and push is deferred until all 100 slices complete
- files in flight:
  - `CHANGES.md`
...
- objective:
  ...
  - slice 36/100 extends the target-side child direction-role guidance regression to prove explicit top-port diagnostics remain visible alongside renderable child-module diagnostics while stale `.fsm` emission remains blocked
