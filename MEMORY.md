...
- Active unit: **`SIGNAL-DECLARATION-ROW-DROP.2h` CLOSED `2026-09-15` (`.2h.0` adjudication + `.2h.1`
  CODE) — a column whose CELLS are the literal direction words IS the direction column.** That
...
- Next action: **`SIGNAL-DECLARATION-ROW-DROP.2f`** — 122 recoveries, blocked on the `Unused` refusal
  (the repeated-name candidate refuses real signals: `AxPROT`, `BRESP`, `RRESP`, `CXSCNTL`,
...
- Blockers: none. Push cadence **400** (directive `2026-09-13`, FIXED), none due at 269; directive 16
  ...
  `EXTRACTION-QUALITY-GAUGE.md` is at 2,878 of 3,000 lines; **`DOCTRINE_ENFORCEMENT.md` line 394 is
  845 of the 1,024-byte `line_bytes_each` ceiling** — route new detail to §10 prose.
  **Any new production Rust FUNCTION moves `flow_census.json`** — re-derive via `aggregate_change`,
  never edit the literal; the boundary counts (144 / 9) must NOT move. **A book insert re-pins
  line-anchored regions in three claim registries — relocate BY CONTENT** — and moves
  ...
  **[[declaration-replay-reads-the-legacy-stratum]]** (51 of 78 legacy),
  **[[prior-phrase-utf8-byte-as-char]]** (a corpus `--dry-run` sweep is ~6 min, not hung),
  **[[live-surface-edit-bookkeeping-chain]]**.
