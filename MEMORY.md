...
- latest_commit: `NLI-ENTAILMENT-VERIFIER.1` design commit (this `.2` module commit pending → ~44 ahead of pushed `d92a73e3`). **PUSH THRESHOLD = 200**. CI green baseline = **1247 tests** (+4 NLI). KM 17 facts. `subs/fsmgen` pinned `92d7036b`. **STANDING USER REMINDER (2026-06-05): regularly sync/align the mdBook with the codebase — the book is the SOLE user-facing surface; every tree's close MUST refresh its book subsection (BOOK-METHOD-DOC).**
- active_work_unit: `NLI-ENTAILMENT-VERIFIER` — `.1` design + `.2` module DONE. `ir/nli_verify.rs`: a semantic entailment grounding gate (premise=source, hypothesis=claim → keep only ENTAILED) catching the condition-vs-obligation hallucination string-match misses. `NliVerdict`/`gate_action` (NotEntailed→residual, Entailed→keep, **Unknown→ABSTAIN**), `entailment_prompt`, fail-closed `parse_nli_verdict`, `verify_entailment` (reuses `commands::llm_text::call_text_provider` + `SPECFORGE_VLM_HELPER` hermetic hook). `DEFAULT_NLI_MODEL=qwen2.5:14b-instruct` (TEXT). 4 pure tests, NO Ollama dep; book (`architecture-rationale.md`) + KM `nli-entailment-verifier`. CI green 1247. **No behavior change yet.**
...
- next_action: PNT — **`NLI-ENTAILMENT-VERIFIER.3` (live wiring + CLOSE):** wire `verify_entailment` into the extraction/grounding path — for each extracted claim with a source statement, when a text provider is configured, run NLI and route `NotEntailed` → a residual decision; surface an `nli_*` metric; gate it behind a `converge`/`validate` flag (default off until proven, since it makes runtime LLM calls). Hermetic tests via the `SPECFORGE_VLM_HELPER` mock + serialized (env-var) like the other LLM-call tests. **Refresh the book subsection** (the live gate, the flag, the metric) per the BOOK-METHOD-DOC close-rule. Then CLOSE. Reminder: grep `KNOWLEDGE_MAP.md`; **no push until ~200 ahead or ask.**
- paused_work: none.
