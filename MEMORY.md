...
- latest_commit_hash: `5e2fe07292cfef756914dc0d1e253d1e1b8a1f3c`
- latest_commit_brief_message: `test(adapter): keep source role residual diagnostics`
- note: this is the pre-slice-22 baseline for the active `BWFSC=100` batch; `main` is twenty-three local commits ahead of `origin/main`, and push is deferred until the 100-slice batch completes unless the user explicitly redirects or a real blocker stops the batch
...
- branch: `main`
- branch state: `main` has local work for active `BWFSC=100` slice 22 and push is deferred until all 100 slices complete
- files in flight:
  - `CHANGES.md`
...
- objective:
  ...
  - slice 22/100 extends the target-side child direction-role residual regression to prove explicit top-port diagnostics remain visible alongside child-module diagnostics while stale `.fsm` emission remains blocked
