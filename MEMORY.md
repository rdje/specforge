...
- latest_commit_hash: `3429129c59660ad56cb26a2927f1a952c1c90213`
- latest_commit_brief_message: `test(adapter): keep source sibling width provenance`
- note: this is the pre-slice-14 baseline for the active `BWFSC=100` batch; `main` is fifteen local commits ahead of `origin/main`, and push is deferred until the 100-slice batch completes unless the user explicitly redirects or a real blocker stops the batch
...
- branch: `main`
- branch state: `main` has local work for active `BWFSC=100` slice 14 and push is deferred until all 100 slices complete
- files in flight:
  - `CHANGES.md`
...
- objective:
  ...
  - slice 14/100 extends the transitive child-width recovery regression to prove selected producer and consumer child inventories keep retained interface provenance while renderable recovery stays residual-clean
