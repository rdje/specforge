...
- latest_commit: **`2a07e87e` ("PDF-VARIANT-DIGESTION.4a — split into eval-surface + per-doc gold leaves"); ~3 local commits ahead of `origin/main` after `.4a.1` (`625a02ea` reconcile, `2a07e87e` split, then `.4a.1`)**. Push ~every 200 commits or on ask (`feedback_push_cadence`). CI green = **1363 tests** (`cargo test -p specforge --lib`); kg-bench 151/151. KM 38 facts — **systematically write a KM card per durable fact** (`feedback_systematic_km_cards`). `subs/fsmgen` pinned (`c0b7eaa7`/`d31b0b91`). Local models: `qwen2.5:14b-instruct` (text/NLI) + `qwen2.5vl:7b` (vision); `ollama serve` may be needed. Disk reclaimed ~31.8 GiB `2026-06-07`.
...
- next_action: **`PDF-VARIANT-DIGESTION.4a.2`** — RISC-V Debug register-field gold. `.4a.1` DONE (register-field per-fact eval surface: `EvalTask::RegisterField` + `GoldFact::RegisterField` + normalized `register|field|offset|width` key + `index_register_field_predictions` + `eval-extraction` branch over EvidenceIR `register_records`; additive, 3 hermetic tests). `.4a.2`: author a source-verified gold for a bounded set of RISC-V Debug registers (e.g. `dmcontrol`/`dmstatus`/`abstractcs`) from the `corpus/` PDF, FRESH re-ingest (`DOCLING_DEVICE=cpu` → evidence), `eval-extraction` per-fact P/R/F1 (no faking, `feedback_scoring_rigor`). Then `.4a.3` NVMe, `.4a.4` declared-signal eval surface, `.4a.5` I2C prose-signal gold. Book note for the register-field eval surface lands with `.4a.2` (first real number).
- paused_work: none. in_flight_uncommitted: `.4a.1` code (`eval.rs`+`eval_extraction.rs`) + live-docs staged for the `PDF-VARIANT-DIGESTION.4a.1` commit (this MEMORY refresh reflects the pre-commit baseline `2a07e87e`). blockers: none.
