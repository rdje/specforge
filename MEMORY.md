...
- latest_commit (baseline): **`f82ebccb`** `CORPUS-COVERAGE.2 — re-ingest #5: TileLink 1.7.1`. **A TileLink-1.8.0 re-ingest slice commit + a PUSH are happening now (200 ahead).** Tree clean except untracked `.claude/settings.json`.
...
- next_action: **PUSH first (200 ahead), then re-ingest doc #7 = LPI (`ihi0068`, 0.81 MB)** at `.cache/local-references/chipdoc/arm/amba/specialized/lpi/current/IHI0068_D_2021-10_AMBA_Low_Power_Interface_Specification.pdf`. Per-doc pattern (proven): `DOCLING_DEVICE=cpu ./target/release/specforge ingest <symlink path>` → evidence→semantic→intent→`adapt --target isf` → FSMGen `--strict --check`. **FSMGEN PATH GOTCHA:** compute `ABS="$PWD/$ISF"` BEFORE `cd subs/fsmgen`. **PNT order:** CXS✓ GFB✓ ACC✓ ATP✓ TL1.7.1✓ TL1.8.0✓ → LPI → DTI(`ihi0088`) → CHI-C2C(`ihi0098_b`), then the register/TRM/ISA docs (big `.10` gains expected: GIC/SMMU/CoreSight/CCIX/AMD/NVMe-class). ONE doc/slice; commit each; push ~every 200.
