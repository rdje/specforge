...
- push policy: PNT → push around every ~30 local commits since last push (pushed at `4d88ccd2`; currently ~9 ahead — under ~30, no push unless user requests)
...
- ACTIVE PNT (not exhausted): `FSMGEN-ISSUE-REPORTING` DONE; `ISF-TEMPORAL-LOWERING` DONE/closed (audit critical finding remediated); remaining: `ISF-ONLY-IR-PRUNE` (frontier `.1`; `.3` superseded), `AUDIT-DOC-RECONCILE` (`.1` + `.2`). SpecForge pipeline: `SourceIR→EvidenceIR→SemanticIR→IntentIR→.isf` only.
