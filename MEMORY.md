...
- latest_commit_hash: `7e765e59000d9d76344726251bfeffe1abecdcc9`
- latest_commit_brief_message: `test(adapter): lock top-before-child provenance`
- note: new local `N=200` batch is active; slice 2 is committed, slice 3 is in flight, and push is deferred until all 200 slices complete
...
- objective:
  - regression-lock single-FSM-child top source-document provenance
  - prove the selected top candidate and renderable top root keep the `ACC` top-port support IDs and the `controller.ACC -> ACC` topology-link support IDs
  - preserve the existing FSM child root-kind and child support-ID checks
  - keep the mdBook adapter artifact description aligned with this provenance surface
- tracker effect:
  - expected live-status tracker gain: `.fsm single FSM-child top emission now preserves top-port and topology-link provenance: Done`
- verification status:
...
## Next exact steps
- run final docs CI, KG bench, full CI, final guards, commit slice 3 without pushing, and continue slice 4
