...
- Active unit: **`COMMIT-GATE-SINGLE-RUN`** `.0`-`.3a` done. **NEVER run the full driver by hand** (`G+G` vs `G`).
...
- Next action: **STOP RISK** — `docs/tasks/EXTRACTION-QUALITY-GAUGE.md` is **2,995 of 3,000** `lines_each`; a
  ...
  partition — **too full to record its own remedy**. Open `LIVE-DOCUMENT-PRESSURE-HEADROOM.31` in
  `current-and-open-work.md` first (the pressure tree owns the transaction: `.21`, `.30`), then migrate. It
  is the FIRST `post_migration` OPEN leaf; 5-file chain, precedent `git show 474de7f2`; `--write` refuses.
