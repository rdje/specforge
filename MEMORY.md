...
- Blockers: none. Push cadence is **400** per director directive `2026-09-13`, FIXED there, so no push is due at 236; directive 16 still
  ...
  Rebuild is `evidence → validate → semantic → validate → intent → validate → adapt`, each validated exactly once, upstream-first;
  **the evidence stage needs the normalized bundle**, so restore from
