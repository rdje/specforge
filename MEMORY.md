...
- latest_commit_hash: `f2e714dde90f2f6bd8a45a0e429cc8f54b31f450`
- latest_commit_brief_message: `test(adapter): lock top fsm child roots`
- note: the latest committed baseline locks renderable top source documents preserving FSM child root kind through `?fsmc` references and child `?fsm` direct roots
...
- objective:
  - lock renderable source-document direct-root de-duplication for reused structured FSM child modules
- tracker effect:
  - added a renderable source-document direct-root `Done` row for reused FSM child module de-duplication while preserving each `?fsmc` instance
- verification status:
...
- current known local CI baseline:
  - `517` Rust tests plus warning-deny Clippy/rustdoc and the mdBook build

## Next exact steps
- rerun final lightweight formatting, docs, and whitespace checks after validation-doc updates
- commit the reused-FSM-child direct-root de-duplication regression slice
