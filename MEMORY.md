...
- latest_commit (baseline): **`2544d3e8`** `CORPUS-COVERAGE.2 — re-ingest #6: TileLink 1.8.0` (PUSHED — 0 ahead of `origin/main`). **An LPI re-ingest slice commit is being created now → 1 ahead.** Tree clean except untracked `.claude/settings.json`.
...
- next_action: **re-ingest doc #8 = DTI (`ihi0088`, 1.0 MB)** at `.cache/local-references/chipdoc/arm/amba/specialized/dti/current/IHI0088_G_2024-06_AMBA_DTI_Protocol_Specification.pdf` — EXPECT a big `.10f` gain: `message_field_records` 0→~159 (DTI is THE `.10f` section-heading message-field doc). Per-doc pattern (proven): `DOCLING_DEVICE=cpu ./target/release/specforge ingest <symlink path>` → evidence→semantic→intent→`adapt --target isf` → FSMGen `--strict --check`. **FSMGEN PATH GOTCHA:** compute `ABS="$PWD/$ISF"` BEFORE `cd subs/fsmgen`. **PNT order:** CXS✓ GFB✓ ACC✓ ATP✓ TL1.7.1✓ TL1.8.0✓ LPI✓ → DTI → CHI-C2C(`ihi0098_b`), then register/TRM/ISA docs (big `.10` gains: GIC/SMMU/CoreSight/CCIX/AMD/NVMe-class). ONE doc/slice; commit each; push ~every 200. **RULE-CONFLICT RESIDUALS:** LPI surfaced `isf_conflicting_rule_writes` (the `ISF-RULE-CONFLICT-RESIDUAL` family); track such per-doc as honest residuals, candidates for that tree — do NOT do emitter surgery inside a re-ingest slice.
