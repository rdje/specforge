...
- latest_commit_hash: `af3962a`
- latest_commit_brief_message: `fix(adapter): surface width conflicts`
- note: the latest committed baseline preserves `.fsm` width-conflict provenance in adapter artifacts and reports conflicted width evidence separately from missing width evidence
...
- objective:
  - require explicit numeric `.fsm` top public IO width evidence after all top actor-port and child-link width recovery has run
- tracker effect:
  - added a `Done` live-status row for blocking missing or parametric top public IO widths instead of rendering implicit 1-bit top ports
- verification status:
...
- current known local CI baseline:
  - `503` Rust tests plus warning-deny Clippy/rustdoc and the mdBook build

## Next exact steps
- rerun final lightweight formatting, docs, and whitespace checks after validation-doc updates
- commit the top public IO width renderability gate slice
