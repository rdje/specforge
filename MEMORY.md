...
- latest_commit (baseline): **`46f9603f`** `CORPUS-COVERAGE.2 — re-ingest #4: ATP (ihi0082)`. **A TileLink-1.7.1 re-ingest slice commit is being created now.** **198 ahead** of `origin/main` (→ 199 after; PUSH after the next slice hits 200). Tree clean except untracked `.claude/settings.json`.
...
- next_action: **re-ingest doc #6 = TileLink 1.8.0 (`tilelink_1_8_0`, 0.75 MB)** at `.cache/local-references/chipdoc/risc-v/interfaces/tilelink/current/TileLink-1.8.0_Specification.pdf`. Per-doc pattern (proven): `DOCLING_DEVICE=cpu ./target/release/specforge ingest <symlink path>` → evidence→semantic→intent→`adapt --target isf` → FSMGen `--strict --check`. **FSMGEN PATH GOTCHA:** compute `ABS="$PWD/$ISF"` BEFORE `cd subs/fsmgen`. **PNT order:** CXS✓ GFB✓ ACC✓ ATP✓ TileLink1.7.1✓ → TileLink 1.8.0 → LPI(`ihi0068`) → DTI(`ihi0088`) → CHI-C2C(`ihi0098_b`), then register/TRM/ISA docs. ONE doc/slice; commit each. **This slice → 200 ahead → PUSH** (`git push origin main`) after committing, then resume PNT.
