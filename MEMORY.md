...
- Active unit: **`LIVE-DOCUMENT-PRESSURE-HEADROOM`** — **`.25`** stopped a FULL collection being the one state
  that reports nothing. `check_live_document_size.pl` skipped the `files` dimension whenever actual == target,
  so `rolling_ledger_archive_indexes` sat at **4/4, 100%, silent**. The first narrowing was WRONG and the tree
  said so: exempting only derived projections unsilenced **24** single-file surfaces forever. The measured
  discriminator is `files` bound **==** glob-free target count (40 surfaces enumerate, 21 glob, but 5 of the 40
  bound ABOVE what they list). Final rule exempts a derived projection or an exactly-enumerated surface and
  reports every other 100%; **113/113** with a RED control per arm.
...
- Next action: **`.26`** — `rolling_ledger_archive_indexes` is **4 of 4**, health == ceiling, no band, and the
  count is really ONE INDEX PER DECLARED LEDGER (`rolling_ledgers.jsonl` declares exactly 4). Declare the
  derivation or size it from the ledger population under an authority — do NOT just raise it to quiet the
  warning `.25` made visible. Trigger is near: a 5th ledger (`VALIDATION_SNAPSHOT.md`, `.4d.ii`) is refused.
